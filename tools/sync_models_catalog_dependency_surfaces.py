...
BACKEND_OPERATIONS = [
    ...
    ("POST", "/backend/v3/api/ai/models/refresh", "models.refresh", "getModelsBackendSdkClient().ai.models.refresh", True),
    ...
    ("GET", "/backend/v3/api/ai/resources", "aiResources.list", "getModelsBackendSdkClient().ai.aiResources.list", True),
    ("POST", "/backend/v3/api/ai/resources", "aiResources.create", "getModelsBackendSdkClient().ai.aiResources.create", False),
    ("PUT", "/backend/v3/api/ai/resources/{resourceId}", "aiResources.update", "getModelsBackendSdkClient().ai.aiResources.update", False),
    ("GET", "/backend/v3/api/ai/resource_groups", "aiResourceGroups.list", "getModelsBackendSdkClient().ai.aiResourceGroups.list", True),
    ("POST", "/backend/v3/api/ai/resource_groups", "aiResourceGroups.create", "getModelsBackendSdkClient().ai.aiResourceGroups.create", False),
    ("PATCH", "/backend/v3/api/ai/resource_groups/{groupId}", "aiResourceGroups.update", "getModelsBackendSdkClient().ai.aiResourceGroups.update", False),
    ("DELETE", "/backend/v3/api/ai/resource_groups/{groupId}", "aiResourceGroups.delete", "getModelsBackendSdkClient().ai.aiResourceGroups.delete", False),
    ("GET", "/backend/v3/api/ai/resource_groups/{groupIdOrCode}/resources", "aiResourceGroups.resources.list", "getModelsBackendSdkClient().ai.aiResourceGroups.resources.list", False),
]
...
def same_origin_runtime(required_env: str) -> dict:
    return {
        ...
        "rustRouteContractCrate": {
            "crate": "sdkwork-router-catalog-backend-api",
        ...
        "mountCoverage": {
            "status": "verified",
            "evidence": [
                "Model catalog admin CRUD, mappings, sync, and rankings routes are owned by sdkwork-models route crates and mounted in sdkwork-router-backend-api through sdkwork_router_catalog_backend_api.",
                ...
                "cargo test -p sdkwork-router-catalog-backend-api --test route_manifest verifies backend route manifest alignment.",
            ],
        },
    }
...
def same_origin_app_runtime() -> dict:
    ...
    runtime["rustRouteContractCrate"] = {
        "crate": "sdkwork-router-catalog-app-api",
        "executableRouterExport": "admin_model_catalog_router",
    }
    runtime["handlerAdapterExports"] = []
    runtime["mountCoverage"]["evidence"] = [
        "Model catalog and rankings app routes are owned by sdkwork-models route crates and mounted in sdkwork-router-app-api through sdkwork_router_catalog_app_api.",
        ...
        "cargo test -p sdkwork-router-catalog-app-api --test route_manifest verifies app route manifest alignment.",
    ]
    return runtime
