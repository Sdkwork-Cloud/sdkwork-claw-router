...
#[tokio::test]
async fn appbase_commerce_app_routers_reject_malformed_upstream_request_id() {
    let pool = SqlitePool::connect("sqlite::memory:")
        .await
        .expect("sqlite pool");
    let router = app_commerce_foundation_router_with_sqlite_pool(pool.clone())
    ...
    assert!(
        is_canonical_uuid(&request_id),
        "bad request response returned non-canonical request id {request_id}"
    );
}

#[tokio::test]
async fn appbase_commerce_app_routers_echo_trusted_canonical_upstream_request_id() {
    let pool = SqlitePool::connect("sqlite::memory:")
        .await
        .expect("sqlite pool");
    let router = app_commerce_foundation_router_with_sqlite_pool(pool.clone())
