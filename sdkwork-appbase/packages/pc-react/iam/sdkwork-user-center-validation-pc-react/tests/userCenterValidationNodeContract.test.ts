...
test("user-center validation node contract creates preflight reports from local snapshots and fails closed when peer auth semantics drift", () => {
  ...
  const mismatchedPeerContract = {
    ...peerContract,
    allowAuthorizationFallbackToAccessToken: false,
    handshake: {
        ...
        mismatches: [
          {
            actual: true,
            expected: false,
            fieldPath: "allowAuthorizationFallbackToAccessToken",
          },
          {
  ...
  assert.throws(
    ...
    /allowAuthorizationFallbackToAccessToken|handshake\.headerNames\.signedAtHeaderName/u,
  );
});
...
test("user-center validation node contract documents and exports the independent validation package", () => {
  ...
  assert.match(validationSource, /allowAuthorizationFallbackToAccessToken/u);
