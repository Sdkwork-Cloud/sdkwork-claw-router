...
describe("sdkwork-auth-pc-react", () => {
  ...
  it("resolves auth state and route access decisions from runtime session tokens", () => {
    ...
    expect(resolveAuthStatus({ authToken: "token" })).toBe("authenticated");
    expect(resolveAuthStatus({ authToken: "token" }, { expiresAt: "2025-01-01T00:00:00.000Z" })).toBe("expired");
  ...
  it("defaults third-party login providers to WeChat, Alipay, and Douyin for mainland China", () => {
  ...
  it("ignores QR login type metadata because openPlatform owns QR entry selection", () => {
    expect(
      resolveSdkworkAuthRuntimeConfigFromMetadata({
