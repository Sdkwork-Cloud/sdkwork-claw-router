...
function normalizeProvider(provider: string | undefined | null): string | null {
  ...
  if (compactProvider === "wechat" || compactProvider === "weixin") {
    return "wechat";
  }

  if (compactProvider === "alipay") {
    return "alipay";
  }

  if (compactProvider === "douyin") {
    return "douyin";
  }
...
export function resolveSdkworkAuthDevelopmentPrefill(
  ...
  const password =
    normalizeOptionalText(explicitValue?.password)
    ?? normalizeOptionalText(runtimeValue?.password)
    ?? readEnvValue(
      "VITE_SDKWORK_AUTH_DEV_DEFAULT_PASSWORD",
      "VITE_AUTH_DEV_DEFAULT_PASSWORD",
    );
  ...
  const loginMethod = isSdkworkAuthLoginMethod(loginMethodCandidate)
    ...
    : undefined;
  const verificationCodeBypassEnabled =
    typeof explicitValue?.verificationCodeBypassEnabled === "boolean"
      ? explicitValue.verificationCodeBypassEnabled
      : typeof runtimeValue?.verificationCodeBypassEnabled === "boolean"
        ? runtimeValue.verificationCodeBypassEnabled
        : parseBoolean(
          readEnvValue(
  ...
  return {
    ...
    ...(typeof verificationCodeBypassEnabled === "boolean" ? { verificationCodeBypassEnabled } : {}),
  };
}
...
export function isSdkworkAuthDevelopmentVerificationCodeEnabled(
  ...
  if (typeof explicitBypass === "boolean") {
...
export function isSdkworkAuthOAuthLoginEnabled(explicitValue?: boolean): boolean {
  if (typeof explicitValue === "boolean") {
    return explicitValue;
  }
