...
goes through the standard app SDK open-platform resource
`openPlatform.qrAuth.sessions.create` with only `purpose: "login" | "register"`.
The platform QR auth service owns default account/entry selection for official
accounts, mini apps, and browser fallback URLs.
...
call `openPlatform.qrAuth.sessions.scans.create` before password completion;
provider webhooks record the provider scan before external completion. Direct
password or external completion without a scan is rejected so scanner source,
entry, external user, event, and login log data are always present.

Desktop QR polling uses `openPlatform.qrAuth.sessions.retrieve`. If the platform
returns `completed` without token data, the controller runs normal session
bootstrap; token issuing and storage stay owned by IAM/runtime rather than the
platform QR session.
