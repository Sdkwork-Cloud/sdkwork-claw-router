...
pub const USER_CENTER_AUTH_VERIFY_SEND_PATH: &str = "/app/v3/api/auth/verification_codes";
pub const USER_CENTER_AUTH_VERIFY_CHECK_PATH: &str = "/app/v3/api/auth/verification_codes/verify";
...
pub const USER_CENTER_DEFAULT_LOCAL_OWNER_USER_ID: &str = "100000000000000001";

const DECIMAL_IDENTIFIER_BASE: u64 = 100_000_000_000_000_000;
static NEXT_DECIMAL_IDENTIFIER: AtomicU64 = AtomicU64::new(DECIMAL_IDENTIFIER_BASE);

pub fn create_identifier(_entity_kind: &str) -> String {
    let timestamp_component = u64::try_from(
        SystemTime::now()
    ...
    let minimum_identifier = DECIMAL_IDENTIFIER_BASE.saturating_add(timestamp_component);

    loop {
        let observed_identifier = NEXT_DECIMAL_IDENTIFIER.load(Ordering::Relaxed);
        let next_identifier = minimum_identifier.max(observed_identifier.saturating_add(1));
        if NEXT_DECIMAL_IDENTIFIER
            ...
            .is_ok()
        {
            return next_identifier.to_string();
        }
    }
}
...
pub struct UserCenterLocalApiRoutes {
    ...
    pub auth_verify_check: String,
    pub auth_verify_send: String,
    pub account: String,
    pub health: String,
...
pub fn create_user_center_local_api_routes(base_path: Option<&str>) -> UserCenterLocalApiRoutes {
    ...
    let auth_verify_send_path = format!("{auth_base_path}/verification_codes");
    let auth_verify_check_path = format!("{auth_base_path}/verification_codes/verify");
    ...
    let auth_oauth_url_path = format!("{auth_base_path}/oauth_authorization_urls");
    let auth_oauth_login_path = format!("{auth_base_path}/oauth_sessions");

    UserCenterLocalApiRoutes {
        account: account_summary_path.clone(),
        account_summary: account_summary_path,
