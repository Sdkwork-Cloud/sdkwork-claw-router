...
const USER_CENTER_DEFAULT_APP_ID: &str = "sdkwork-user-center";
...
const SDKWORK_USER_CENTER_LOCAL_OAUTH_PROVIDERS_ENV: &str =
    "SDKWORK_USER_CENTER_LOCAL_OAUTH_PROVIDERS";
const SDKWORK_USER_CENTER_LOCAL_OAUTH_CODE_SECRET_ENV: &str =
    "SDKWORK_USER_CENTER_LOCAL_OAUTH_CODE_SECRET";
const SDKWORK_USER_CENTER_LOCAL_OAUTH_CODE_TTL_SECONDS_ENV: &str =
    "SDKWORK_USER_CENTER_LOCAL_OAUTH_CODE_TTL_SECONDS";
const SDKWORK_USER_CENTER_LOCAL_BOOTSTRAP_EMAIL_ENV: &str =
    "SDKWORK_USER_CENTER_LOCAL_BOOTSTRAP_EMAIL";
const SDKWORK_USER_CENTER_LOCAL_BOOTSTRAP_PHONE_ENV: &str =
    "SDKWORK_USER_CENTER_LOCAL_BOOTSTRAP_PHONE";
const SDKWORK_USER_CENTER_LOCAL_BOOTSTRAP_PASSWORD_ENV: &str =
    "SDKWORK_USER_CENTER_LOCAL_BOOTSTRAP_PASSWORD";
const SDKWORK_USER_CENTER_LOCAL_VERIFY_CODE_FIXED_ENV: &str =
    "SDKWORK_USER_CENTER_LOCAL_VERIFY_CODE_FIXED";
const SDKWORK_USER_CENTER_LOCAL_VERIFY_CODE_TTL_SECONDS_ENV: &str =
    "SDKWORK_USER_CENTER_LOCAL_VERIFY_CODE_TTL_SECONDS";
...
CREATE TABLE IF NOT EXISTS iam_tenant (
    ...
    biz_type TEXT NULL,
    biz_id INTEGER NULL,
    jwt_secret_key TEXT NOT NULL DEFAULT 'sdkwork-user-center-local-tenant-secret',
...
CREATE TABLE IF NOT EXISTS iam_vip_membership (
    id INTEGER PRIMARY KEY,
    uuid TEXT NOT NULL UNIQUE,
    tenant_id INTEGER NOT NULL DEFAULT 0,
    organization_id INTEGER NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS idx_iam_verification_code_target
ON iam_verification_code(target, verify_type, scene, status, is_deleted);
...
const DEFAULT_LOCAL_BOOTSTRAP_EMAIL: &str = "local-default@sdkwork-user-center.local";
...
const DEFAULT_LOCAL_TENANT_CODE: &str = "sdkwork-user-center-local";
...
const LOCAL_PHONE_SHADOW_EMAIL_SUFFIX: &str = "@sms.sdkwork-user-center.local";
const EXTERNAL_ACCOUNT_SHADOW_EMAIL_SUFFIX: &str = "@external.sdkwork-user-center.local";
const LOCAL_OAUTH_SHADOW_EMAIL_SUFFIX: &str = "@oauth.sdkwork-user-center.local";
...
fn ensure_default_local_tenant(connection: &mut Connection) -> Result<(), String> {
    ...
    let default_tenant_name = "SDKWork User Center Local Tenant";
    ...
    let default_tenant_secret_key = "sdkwork-user-center-local-tenant-secret";
    ...
    let canonical_default_id_exists: i64 = connection
        .query_row(
            "SELECT EXISTS(SELECT 1 FROM iam_tenant WHERE CAST(id AS TEXT) = ?1)",
            params![DEFAULT_LOCAL_TENANT_ID],
            |row| row.get(0),
        )
        .map_err(|error| format!("probe canonical default iam_tenant failed: {error}"))?;
    if canonical_default_id_exists > 0 {
        let transaction = connection
            .transaction()
            .map_err(|error| format!("open default iam_tenant canonicalization failed: {error}"))?;
        transaction
            .execute(
                r#"
                UPDATE iam_tenant
            ...
            .map_err(|error| {
                format!("release duplicate default iam_tenant code failed: {error}")
            })?;
        let updated_canonical_default = transaction
            ...
            .map_err(|error| format!("canonicalize default iam_tenant failed: {error}"))?;
        transaction.commit().map_err(|error| {
            format!("commit default iam_tenant canonicalization failed: {error}")
        })?;
    ...
    let updated_existing_default_code = connection
        ...
        .map_err(|error| format!("ensure default iam_tenant failed: {error}"))?;
    ...
    connection
        .execute(
            ...
            ON CONFLICT(id) DO UPDATE SET
                uuid = COALESCE(NULLIF(iam_tenant.uuid, ''), excluded.uuid),
                code = excluded.code,
                name = excluded.name,
                type = excluded.type,
                jwt_secret_key = COALESCE(NULLIF(iam_tenant.jwt_secret_key, ''), excluded.jwt_secret_key),
                token_expiration_ms = COALESCE(iam_tenant.token_expiration_ms, excluded.token_expiration_ms),
                refresh_token_expiration_ms = COALESCE(iam_tenant.refresh_token_expiration_ms, excluded.refresh_token_expiration_ms),
            ...
            params![
                DEFAULT_LOCAL_TENANT_ID,
                stable_entity_uuid("iam_tenant", DEFAULT_LOCAL_TENANT_ID),
                &now,
                &now,
                default_tenant_name,
                DEFAULT_LOCAL_TENANT_CODE,
...
fn backfill_iam_user_business_columns(connection: &mut Connection) -> Result<(), String> {
    backfill_global_business_columns(connection, "iam_user")
}

fn migrate_legacy_iam_user_password_credentials(connection: &mut Connection) -> Result<(), String> {
    if !crate::sqlite_table_exists(connection, "iam_user")? {
        return Ok(());
    }
    if !crate::sqlite_column_exists(connection, "iam_user", "password")? {
        return Ok(());
    }
    ...
    let rows = {
        let mut statement = connection
            .prepare(
                r#"
                SELECT id, tenant_id, organization_id, password, status, created_at, updated_at
                FROM iam_user
                WHERE is_deleted = 0
            ...
            .map_err(|error| {
                format!("prepare legacy iam_user password migration failed: {error}")
            })?;
        let mut rows = statement
            .query([])
            .map_err(|error| format!("query legacy iam_user password migration failed: {error}"))?;
        let mut records = Vec::new();
        while let Some(row) = rows.next().map_err(|error| {
            format!("read legacy iam_user password migration row failed: {error}")
        })? {
            records.push((
                sqlite_row_required_string_value(row, 0, "iam_user.id")
                    .map_err(|error| format!("read legacy iam_user id failed: {error}"))?,
                sqlite_row_optional_string_value(row, 1, "iam_user.tenant_id")
                    .map_err(|error| format!("read legacy iam_user tenant_id failed: {error}"))?,
                sqlite_row_optional_string_value(row, 2, "iam_user.organization_id").map_err(
                    |error| format!("read legacy iam_user organization_id failed: {error}"),
                )?,
                row.get::<_, String>(3)
                    .map_err(|error| format!("read legacy iam_user password failed: {error}"))?,
                row.get::<_, String>(4)
                    .map_err(|error| format!("read legacy iam_user status failed: {error}"))?,
                row.get::<_, String>(5)
                    .map_err(|error| format!("read legacy iam_user created_at failed: {error}"))?,
                row.get::<_, String>(6)
                    .map_err(|error| format!("read legacy iam_user updated_at failed: {error}"))?,
            ));
        }
        records
    };

    for (user_id, tenant_id, organization_id, password_hash, status, created_at, updated_at) in rows
    {
        let Some(normalized_password_hash) = normalize_optional_text(Some(password_hash.as_str()))
        else {
            continue;
        };
        let resolved_tenant_id = normalize_optional_text(tenant_id.as_deref())
            .unwrap_or_else(|| DEFAULT_LOCAL_TENANT_ID.to_owned());
        let resolved_organization_id = resolve_local_organization_id(organization_id.as_deref());
        let credential_id = stable_numeric_identifier("iam_credential", &user_id);
        connection
            .execute(
                r#"
                ...
                VALUES (?1, ?2, ?3, ?4, 1, ?5, 'password', ?6, ?7, NULL, ?8, ?9, 0, 0)
            ...
            .map_err(|error| {
                format!("migrate legacy iam_user password credential for {user_id} failed: {error}")
            })?;
    }
...
fn rebuild_iam_user_without_legacy_password_column(
    connection: &mut Connection,
) -> Result<(), String> {
    if !crate::sqlite_table_exists(connection, "iam_user")? {
        return Ok(());
    ...
    connection
        .execute("DROP TABLE IF EXISTS iam_user__legacy_password_column", [])
        .map_err(|error| format!("drop stale iam_user legacy password table failed: {error}"))?;
    connection
        .execute(
            "ALTER TABLE iam_user RENAME TO iam_user__legacy_password_column",
            [],
        )
        .map_err(|error| format!("rename iam_user legacy password table failed: {error}"))?;
    connection
        .execute_batch(USER_CENTER_SQLITE_SCHEMA)
        .map_err(|error| format!("recreate iam_user without password column failed: {error}"))?;
    connection
        .execute(
            r#"
            INSERT INTO iam_user (
                id, uuid, tenant_id, organization_id, data_scope, username, nickname, salt,
                platform, type, gender, face_image, face_video, scene, email, phone,
                country_code, province_code, city_code, district_code, address, bio, birth_date,
                oauth_user_info, metadata, social_info_list, avatar_url, provider_key,
            ...
            SELECT
                ...
                oauth_user_info, metadata, social_info_list, avatar_url, provider_key,
                external_subject, metadata_json, status, created_at, updated_at, version, is_deleted
            FROM iam_user__legacy_password_column
            "#,
            [],
        )
        .map_err(|error| format!("copy iam_user rows without password column failed: {error}"))?;
    connection
        .execute("DROP TABLE iam_user__legacy_password_column", [])
        .map_err(|error| format!("drop iam_user legacy password table failed: {error}"))?;
    Ok(())
}
...
fn backfill_user_owned_business_columns(
    ...
    let query = format!(
        ...
        LEFT JOIN iam_user
            ON iam_user.id = child.{user_id_column}
...
fn backfill_login_qr_business_columns(connection: &mut Connection) -> Result<(), String> {
        ...
        let mut statement = connection
            .prepare(
                ...
                SELECT
                    iam_login_qr.id,
                    iam_login_qr.uuid,
                    iam_login_qr.tenant_id,
                    iam_login_qr.organization_id,
                    iam_user.tenant_id,
                    iam_user.organization_id
                FROM iam_login_qr
                LEFT JOIN iam_user
                    ON iam_user.id = iam_login_qr.user_id
                   AND iam_user.is_deleted = 0
                WHERE iam_login_qr.is_deleted = 0
                "#,
            )
            .map_err(|error| format!("prepare iam_login_qr canonical migration failed: {error}"))?;
        let mut rows = statement
            .query([])
            .map_err(|error| format!("query iam_login_qr canonical migration failed: {error}"))?;
        let mut records = Vec::new();
        while let Some(row) = rows
            .next()
            .map_err(|error| format!("read iam_login_qr canonical migration row failed: {error}"))?
        {
            records.push((
                sqlite_row_required_string_value(row, 0, "iam_login_qr.id")
                    .map_err(|error| format!("read iam_login_qr id failed: {error}"))?,
                row.get::<_, Option<String>>(1)
                    .map_err(|error| format!("read iam_login_qr uuid failed: {error}"))?,
                sqlite_row_optional_string_value(row, 2, "iam_login_qr.tenant_id")
                    .map_err(|error| format!("read iam_login_qr tenant_id failed: {error}"))?,
                sqlite_row_optional_string_value(row, 3, "iam_login_qr.organization_id").map_err(
                    |error| format!("read iam_login_qr organization_id failed: {error}"),
                )?,
                sqlite_row_optional_string_value(row, 4, "iam_login_qr.parent_tenant_id").map_err(
                    |error| format!("read iam_login_qr parent tenant_id failed: {error}"),
                )?,
                sqlite_row_optional_string_value(row, 5, "iam_login_qr.parent_organization_id")
                    .map_err(|error| {
                        format!("read iam_login_qr parent organization_id failed: {error}")
                    })?,
            ));
        }
        records
    ...
    for (id, uuid, tenant_id, organization_id, user_tenant_id, user_organization_id) in rows {
        let resolved_uuid = normalize_optional_text(uuid.as_deref())
            .unwrap_or_else(|| stable_entity_uuid("iam_login_qr", &id));
        let resolved_tenant_id = normalize_optional_text(tenant_id.as_deref())
        ...
        connection
            ...
            .map_err(|error| {
                format!("backfill iam_login_qr canonical fields {id} failed: {error}")
            })?;
    }
...
const USER_CENTER_INTEGER_IDENTIFIER_TABLE_RULES: &[(&str, &[(&str, bool)])] = &[
    ("iam_tenant", &[("id", false)]),
    ("iam_user", &[("id", false), ("tenant_id", true)]),
    (
        "iam_credential",
...
fn ensure_sqlite_user_center_integer_identifier_upgrade(
    ...
    let iam_tenant_legacy_table = "iam_tenant__legacy_integer_identifiers";
    let iam_user_legacy_table = "iam_user__legacy_integer_identifiers";
    let iam_user_identity_legacy_table = "iam_user_identity__legacy_integer_identifiers";
    let iam_vip_membership_legacy_table = "iam_vip_membership__legacy_integer_identifiers";
    let iam_session_legacy_table = "iam_session__legacy_integer_identifiers";
    let iam_verification_code_legacy_table = "iam_verification_code__legacy_integer_identifiers";
    let iam_login_qr_legacy_table = "iam_login_qr__legacy_integer_identifiers";
    let iam_organization_legacy_table = "iam_organization__legacy_integer_identifiers";
    ...
    let _ = upgrade_sqlite_user_center_integer_identifier_table(
        ...
        format!(
            r#"
            INSERT INTO iam_user (
                id, uuid, tenant_id, organization_id, username, nickname, salt, platform, type, scene,
    ...
    let _ = upgrade_sqlite_user_center_integer_identifier_table(
        ...
        format!(
            ...
            FROM {iam_user_identity_legacy_table}
            "#
        ),
    )?;
    let _ = upgrade_sqlite_user_center_integer_identifier_table(
        ...
        format!(
            r#"
            INSERT INTO iam_vip_membership (
            ...
            FROM {iam_vip_membership_legacy_table}
            "#
        ),
    )?;
    let _ = upgrade_sqlite_user_center_integer_identifier_table(
    ...
    let _ = upgrade_sqlite_user_center_integer_identifier_table(
        ...
        format!(
            ...
            FROM {iam_verification_code_legacy_table}
            "#
        ),
    )?;
    let _ = upgrade_sqlite_user_center_integer_identifier_table(
        ...
        format!(
            ...
            SELECT
                ...
                END,
                scanned_at, confirmed_at, expires_at, metadata_json, created_at, updated_at, version, is_deleted
            FROM {iam_login_qr_legacy_table}
            "#
        ),
    )?;
    let _ = upgrade_sqlite_user_center_integer_identifier_table(
        ...
        format!(
            ...
            FROM {iam_organization_legacy_table}
            "#
        ),
    )?;
...
fn ensure_sqlite_user_center_business_columns(connection: &Connection) -> Result<(), String> {
    if crate::sqlite_table_exists(connection, "iam_tenant")? {
    ...
    if crate::sqlite_table_exists(connection, "iam_organization")? {
        crate::ensure_sqlite_table_columns(
            connection,
            "iam_organization",
            &[
    ...
    if crate::sqlite_table_exists(connection, "iam_user_identity")? {
        crate::ensure_sqlite_table_columns(
            connection,
            "iam_user_identity",
            &[
    ...
    if crate::sqlite_table_exists(connection, "iam_vip_membership")? {
        crate::ensure_sqlite_table_columns(
            connection,
            "iam_vip_membership",
            &[
    ...
    if crate::sqlite_table_exists(connection, "iam_session")? {
        crate::ensure_sqlite_table_columns(
            connection,
            "iam_session",
            &[
    ...
    if crate::sqlite_table_exists(connection, "iam_verification_code")? {
        crate::ensure_sqlite_table_columns(
            connection,
            "iam_verification_code",
            &[
    ...
    if crate::sqlite_table_exists(connection, "iam_login_qr")? {
        crate::ensure_sqlite_table_columns(
            connection,
            "iam_login_qr",
            &[
...
pub fn ensure_sqlite_user_center_schema(connection: &mut Connection) -> Result<(), String> {
    ...
    ensure_sqlite_user_center_business_columns(connection)?;
    migrate_legacy_iam_user_password_credentials(connection)?;
    rebuild_iam_user_without_legacy_password_column(connection)?;

    ensure_default_local_tenant(connection)?;
    backfill_iam_user_business_columns(connection)?;
    backfill_user_owned_business_columns(connection, "iam_credential", "user_id")?;
    backfill_user_owned_business_columns(connection, "iam_user_identity", "user_id")?;
    backfill_user_owned_business_columns(connection, "iam_vip_membership", "user_id")?;
    backfill_user_owned_business_columns(connection, "iam_session", "user_id")?;
    backfill_global_business_columns(connection, "iam_verification_code")?;
...
fn resolve_local_bootstrap_email() -> String {
    read_env_trimmed(SDKWORK_USER_CENTER_LOCAL_BOOTSTRAP_EMAIL_ENV)
        .filter(|value| value.contains('@'))
        .unwrap_or_else(|| DEFAULT_LOCAL_BOOTSTRAP_EMAIL.to_owned())
}

fn resolve_local_bootstrap_phone() -> Option<String> {
    read_env_trimmed(SDKWORK_USER_CENTER_LOCAL_BOOTSTRAP_PHONE_ENV)
...
fn resolve_local_fixed_verify_code(seed_policy: &UserCenterSeedPolicy) -> Option<String> {
    ...
    read_env_trimmed(SDKWORK_USER_CENTER_LOCAL_VERIFY_CODE_FIXED_ENV)
}
...
fn resolve_external_app_api_config_from_env(
    ...
    let handshake = resolve_external_app_api_handshake_config(
        read_env_trimmed(SDKWORK_USER_CENTER_APP_ID_ENV).as_deref(),
...
fn resolve_local_bootstrap_password() -> String {
    std::env::var(SDKWORK_USER_CENTER_LOCAL_BOOTSTRAP_PASSWORD_ENV)
        .ok()
        .map(|value| value.trim().to_owned())
        .filter(|value| !value.is_empty())
...
fn stable_numeric_identifier(entity_name: &str, id: &str) -> String {
    let identity_seed = format!(
        "sdkwork-user-center:{entity_name}:{}",
...
fn resolve_display_name(email: &str, explicit_name: Option<&str>) -> String {
    normalize_optional_text(explicit_name)
        ...
        .filter(|value| !value.is_empty())
        .unwrap_or_else(|| "SDKWork User Center User".to_owned())
}
    ...
    normalize_optional_text(explicit_name)
        .or_else(|| {
            ...
            if suffix.is_empty() {
                ...
                Some(format!("SDKWork User Center User {}", suffix))
            }
        })
        .unwrap_or_else(|| "SDKWork User Center User".to_owned())
}
...
fn resolve_local_oauth_providers_from_env() -> Vec<String> {
    match std::env::var(SDKWORK_USER_CENTER_LOCAL_OAUTH_PROVIDERS_ENV) {
        Ok(value) => collect_normalized_oauth_provider_identifiers(&value),
...
fn resolve_local_oauth_code_secret(provider_key: &str) -> String {
    read_env_trimmed(SDKWORK_USER_CENTER_LOCAL_OAUTH_CODE_SECRET_ENV)
        .unwrap_or_else(|| format!("{provider_key}:local-oauth"))
}

fn resolve_local_oauth_code_ttl() -> Duration {
    std::env::var(SDKWORK_USER_CENTER_LOCAL_OAUTH_CODE_TTL_SECONDS_ENV)
        .ok()
        .and_then(|value| value.trim().parse::<u64>().ok())
        .filter(|value| *value > 0)
...
fn local_oauth_provider_env_key(provider: &str, suffix: &str) -> Result<String, String> {
    ...
    Ok(format!(
        "SDKWORK_USER_CENTER_LOCAL_OAUTH_{provider_segment}_{suffix}"
    ))
}
...
fn load_vip_membership_record(
    connection: &Connection,
    user_id: &str,
) -> Result<Option<VipMembershipRecord>, String> {
    connection
        .query_row(
            r#"
            SELECT
                ...
                last_active_time,
                remark
            FROM iam_vip_membership
            WHERE user_id = ?1 AND is_deleted = 0
            LIMIT 1
            "#,
            params![user_id],
            |row| {
                Ok(VipMembershipRecord {
                    uuid: row.get(0)?,
                    tenant_id: sqlite_row_optional_string_value(
                        row,
                        1,
                        "iam_vip_membership.tenant_id",
                    )?,
                    organization_id: sqlite_row_optional_string_value(
                        row,
                        2,
                        "iam_vip_membership.organization_id",
                    )?,
                    ...
                    user_id: sqlite_row_required_string_value(
                        row,
                        5,
                        "iam_vip_membership.user_id",
                    )?,
                    vip_level_id: sqlite_row_optional_string_value(
                        row,
                        6,
                        "iam_vip_membership.vip_level_id",
                    )?,
                    status: row.get(7)?,
        ...
        .optional()
        .map_err(|error| format!("load iam_vip_membership {user_id} failed: {error}"))
}
...
fn ensure_local_credentials(
    ...
    connection
        .execute(
            ...
            SELECT
                ?2, ?3, COALESCE(u.tenant_id, ?4), COALESCE(u.organization_id, ?5), 1,
                u.id, 'password', ?6, 'active', NULL, ?7, ?8, 0, 0
            FROM iam_user u
            ...
            params![
                user_id,
                stable_numeric_identifier("iam_credential", user_id),
                stable_entity_uuid("iam_credential", user_id),
                DEFAULT_LOCAL_TENANT_ID,
...
fn create_verify_code_record(
    ...
    connection
        .execute(
            ...
            params![
                &id,
                stable_entity_uuid("iam_verification_code", &id),
                DEFAULT_LOCAL_TENANT_ID,
                DEFAULT_LOCAL_ORGANIZATION_ID,
                provider_key,
                verify_type,
...
fn load_latest_pending_verify_code(
    ...
    connection
        .query_row(
            ...
            |row| {
                Ok(VerifyCodeRecord {
                    id: sqlite_row_required_string_value(row, 0, "iam_verification_code.id")?,
                    code: row.get(4)?,
...
fn upsert_vip_membership_shadow(
    connection: &mut Connection,
    user_id: &str,
    ...
    remark: Option<&str>,
) -> Result<VipMembershipRecord, String> {
    let now = crate::current_storage_timestamp();
    let existing = load_vip_membership_record(connection, user_id)?;
    ...
    connection
        .execute(
            ...
            "#,
            params![
                crate::create_identifier("vip-membership"),
                stable_entity_uuid("iam_vip_membership", user_id),
    ...
    load_vip_membership_record(connection, user_id)?
...
fn build_vip_membership_payload(
    user: &UserCenterUserPayload,
    membership: Option<VipMembershipRecord>,
) -> UserCenterVipMembershipPayload {
    let resolved = membership.unwrap_or(VipMembershipRecord {
        uuid: user.uuid.clone(),
        tenant_id: user.tenant_id.clone(),
        organization_id: user.organization_id.clone(),
        created_at: user.created_at.clone(),
...
fn upsert_vip_membership_record(
    ...
    Ok(build_vip_membership_payload(
        &session.user,
        load_vip_membership_record(connection, &session.user.id)?,
    ))
}
...
impl LocalUserCenterProvider {
    fn new(provider_key: String) -> Self {
        ...
        let verify_code_ttl = std::env::var(SDKWORK_USER_CENTER_LOCAL_VERIFY_CODE_TTL_SECONDS_ENV)
            .ok()
            .and_then(|value| value.trim().parse::<u64>().ok())
            .filter(|value| *value > 0)
            .map(Duration::from_secs)
    ...
    fn register(
        ...
        let user = match normalized_channel.as_deref() {
            Some("PHONE") | Some("SMS") => {
                ...
                if let Some(verification_code) = verification_code.as_deref() {
                    consume_verify_code(
                        connection,
                        &normalized_phone,
                        "PHONE",
...
impl UserCenterProvider for SdkworkCloudAppApiExternalUserCenterProvider {
    ...
    fn read_vip_membership(
        ...
        let membership = upsert_vip_membership_shadow(
            connection,
...
mod tests {
    ...
    fn local_user_center_registers_email_without_verification_code_by_default() {
        ...
        let provider = LocalUserCenterProvider::new("sdkwork-test-local".to_owned());
        ...
        let session = provider
            .register(
                ...
                &UserCenterRegisterRequest {
                    ...
                    confirm_password: Some("dev123456".to_owned()),
                    email: Some("new-user@sdkwork.test".to_owned()),
                    name: Some("New User".to_owned()),
                    password: Some("dev123456".to_owned()),
                    phone: None,
    ...
    #[test]
    fn default_metadata_does_not_advertise_phone_code_login() {
        let local_metadata =
            LocalUserCenterProvider::new("sdkwork-test-local".to_owned()).metadata();
        assert_eq!(
            local_metadata.login_methods,
    ...
    fn resolve_user_center_seed_policy_from_env_enables_builtin_local_seed_contracts() {
        ...
        std::env::remove_var(SDKWORK_USER_CENTER_MODE_ENV);
        std::env::remove_var(SDKWORK_USER_CENTER_APP_API_BASE_URL_ENV);
        std::env::remove_var(SDKWORK_USER_CENTER_LOCAL_VERIFY_CODE_FIXED_ENV);
    ...
    fn resolve_user_center_seed_policy_from_env_disables_local_seed_contracts_for_cloud_mode() {
        let _guard = ENV_LOCK.lock().expect("lock env");
        std::env::set_var(SDKWORK_USER_CENTER_MODE_ENV, "sdkwork-cloud-app-api");
        std::env::set_var(
            SDKWORK_USER_CENTER_APP_API_BASE_URL_ENV,
            "https://cloud.sdkwork.test/app",
        );
        std::env::set_var(SDKWORK_USER_CENTER_LOCAL_VERIFY_CODE_FIXED_ENV, "123456");
    ...
    fn ensure_sqlite_user_center_schema_tolerates_non_decimal_legacy_tenant_ids() {
        ...
        connection
            ...
            .expect("create legacy non-decimal iam_tenant");
        ...
        let tenant_id: String = connection
            .query_row(
                "SELECT id FROM iam_tenant WHERE code = ?1",
    ...
    fn ensure_sqlite_user_center_schema_adopts_case_variant_legacy_local_tenant_code() {
        ...
        connection
            .execute_batch(
                ...
                CREATE UNIQUE INDEX idx_plus_tenant_code_legacy_nocase
                    ON iam_tenant(code COLLATE NOCASE);
            ...
            .expect("create legacy case-variant iam_tenant");
        ...
        let tenant: (String, String) = connection
            .query_row(
                "SELECT id, code FROM iam_tenant WHERE code = ?1 COLLATE NOCASE",
                params![DEFAULT_LOCAL_TENANT_CODE],
        ...
        let tenant_count: i64 = connection
            .query_row(
                "SELECT COUNT(*) FROM iam_tenant WHERE code = ?1 COLLATE NOCASE",
                params![DEFAULT_LOCAL_TENANT_CODE],
                |row| row.get(0),
            )
    ...
    fn ensure_sqlite_user_center_schema_canonicalizes_default_tenant_id_when_code_is_duplicated() {
        ...
        let canonical_tenant: (String, String) = connection
            .query_row(
                "SELECT CAST(id AS TEXT), code FROM iam_tenant WHERE id = ?1",
                params![DEFAULT_LOCAL_TENANT_ID],
                |row| Ok((row.get(0)?, row.get(1)?)),
            )
    ...
    fn ensure_sqlite_user_center_schema_upgrades_legacy_oauth_accounts_missing_tenant_id() {
        ...
        connection
            .execute_batch(
                ...
                ALTER TABLE legacy_plus_oauth_account RENAME TO iam_user_identity;
                "#,
            )
            .expect("create legacy iam_user_identity without tenant_id");
        ...
        let migrated: (i64, Option<i64>, i64) = connection
            .query_row(
                "SELECT id, tenant_id, user_id FROM iam_user_identity WHERE open_id = 'legacy-open-id'",
                [],
