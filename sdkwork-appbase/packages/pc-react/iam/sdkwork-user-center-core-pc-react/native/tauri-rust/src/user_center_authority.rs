...
struct MembershipRecord {
    ...
    membership_level_id: Option<String>,
    uuid: String,
}
...
#[derive(Clone, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct UpdateUserCenterMembershipRequest {
    pub membership_level_id: Option<String>,
...
#[derive(Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct UserCenterMembershipPayload {
    pub uuid: String,
    pub tenant_id: Option<String>,
    pub organization_id: Option<String>,
    pub created_at: String,
...
#[derive(Clone, Deserialize, Serialize)]
#[serde(rename_all = "camelCase")]
struct UpstreamAppApiMembershipPayload {
    membership_level_id: Option<String>,
...
trait UserCenterProvider: Send + Sync {
    ...
    fn read_membership(
        ...
        connection: &mut Connection,
        session: &UserCenterSessionPayload,
    ) -> Result<UserCenterMembershipPayload, String>;
    ...
    fn update_membership(
        ...
        connection: &mut Connection,
        session: &UserCenterSessionPayload,
        request: &UpdateUserCenterMembershipRequest,
    ) -> Result<UserCenterMembershipPayload, String>;
}
...
impl UserCenterState {
    ...
    pub fn read_membership(
        ...
        self.provider.read_membership(connection, session)
    }
    ...
    pub fn update_membership(
        ...
        self.provider
            .update_membership(connection, session, request)
    }
}
...
fn load_membership_record(
    connection: &Connection,
    user_id: &str,
) -> Result<Option<MembershipRecord>, String> {
    connection
        .query_row(
            r#"
            SELECT
            ...
            |row| {
                Ok(MembershipRecord {
                    ...
                    organization_id: sqlite_row_optional_string_value(
                        row,
                        2,
                        "iam_membership.organization_id",
                    )?,
                    created_at: row.get(3)?,
                    updated_at: row.get(4)?,
                    user_id: sqlite_row_required_string_value(row, 5, "iam_membership.user_id")?,
                    membership_level_id: sqlite_row_optional_string_value(
                        row,
                        6,
                        "iam_membership.membership_level_id",
                    )?,
                    status: row.get(7)?,
        ...
        .optional()
        .map_err(|error| format!("load iam_membership {user_id} failed: {error}"))
}
...
fn upsert_membership_shadow(
    connection: &mut Connection,
    user_id: &str,
    membership_level_id: Option<&str>,
    status: Option<&str>,
    point_balance: Option<i64>,
    total_recharged_points: Option<i64>,
    ...
    let now = crate::current_storage_timestamp();
    let existing = load_membership_record(connection, user_id)?;
    let resolved_membership_level_id = normalize_optional_text(membership_level_id).or_else(|| {
        existing
            .as_ref()
            .and_then(|record| record.membership_level_id.clone())
    });
    ...
    let membership_scope_user = load_user_by_id(connection, user_id)?
        .ok_or_else(|| format!("user {user_id} was not found for membership upsert"))?;

    connection
        .execute(
            r#"
            INSERT INTO iam_membership (
                id, uuid, tenant_id, organization_id, user_id, membership_level_id, status, point_balance,
            ...
            "#,
            params![
                crate::create_identifier("membership"),
                stable_entity_uuid("iam_membership", user_id),
        ...
        .map_err(|error| format!("upsert membership shadow {user_id} failed: {error}"))?;

    load_membership_record(connection, user_id)?
        .ok_or_else(|| format!("membership shadow {user_id} was not found after upsert"))
}
...
fn build_membership_payload(
    user: &UserCenterUserPayload,
    membership: Option<MembershipRecord>,
) -> UserCenterMembershipPayload {
    let resolved = membership.unwrap_or(MembershipRecord {
        uuid: user.uuid.clone(),
        tenant_id: user.tenant_id.clone(),
        organization_id: user.organization_id.clone(),
        created_at: user.created_at.clone(),
    ...
    UserCenterMembershipPayload {
        ...
        membership_level_id: resolved.membership_level_id,
...
fn upsert_membership_record(
    ...
    upsert_membership_shadow(
        connection,
        &session.user.id,
        request.membership_level_id.as_deref(),
    ...
    Ok(build_membership_payload(
        &session.user,
        load_membership_record(connection, &session.user.id)?,
    ))
}
...
impl UserCenterProvider for MisconfiguredUserCenterProvider {
    ...
    fn update_membership(
        ...
        _connection: &mut Connection,
        _session: &UserCenterSessionPayload,
        _request: &UpdateUserCenterMembershipRequest,
    ) -> Result<UserCenterMembershipPayload, String> {
        Err(self.message.clone())
    }
}
...
impl UserCenterProvider for LocalUserCenterProvider {
    ...
    fn update_membership(
        ...
        upsert_membership_record(connection, session, request)
    }
}
...
impl SdkworkCloudAppApiExternalUserCenterProvider {
    ...
    fn request_membership_with_state(
        &self,
        session_id: Option<&str>,
        upstream_state: &PersistedUpstreamSessionState,
    ) -> Result<UpstreamAppApiMembershipPayload, String> {
        let headers = self.build_request_headers(
            "GET",
            "/memberships/current",
            session_id,
        ...
        upstream_request_json::<UpstreamAppApiMembershipPayload>(
            &self.config,
            "GET",
            "/memberships/current",
            &headers,
        ...
        .ok_or_else(|| "Upstream user center returned an empty membership payload.".to_owned())
    }
...
impl UserCenterProvider for SdkworkCloudAppApiExternalUserCenterProvider {
    ...
    fn read_membership(
        ...
        let membership_payload = match self
            .request_membership_with_state(Some(session_record.id.as_str()), &upstream_state)
        {
            Ok(membership_payload) => membership_payload,
            Err(error) if error.contains("status 401") || error.contains("status 403") => {
                upstream_state = self.refresh_session_state(connection, &session_record)?;
                self.request_membership_with_state(
                    Some(session_record.id.as_str()),
                    &upstream_state,
                )?
            }
        ...
        let membership_level_id =
            normalize_optional_text(membership_payload.membership_level_id.as_deref());
        let normalized_status = normalize_optional_text(membership_payload.status.as_deref())
            .map(|value| value.to_ascii_lowercase())
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| {
                if membership_level_id.is_some() {
                    "active".to_owned()
                } else {
                    "inactive".to_owned()
                }
            });
        let point_balance = membership_payload.point_balance.unwrap_or(0);
        let membership = upsert_membership_shadow(
            connection,
            &session_record.user_id,
            membership_level_id.as_deref(),
            Some(normalized_status.as_str()),
            Some(point_balance),
            Some(
                membership_payload
                    .total_recharged_points
                    .unwrap_or(point_balance),
            ),
            None,
            membership_payload.valid_to.as_deref(),
            None,
            None,
        )?;
        Ok(build_membership_payload(&session.user, Some(membership)))
    }
    ...
    fn update_membership(
        ...
        Err("Membership is managed by the external sdkwork-cloud-app-api user center.".to_owned())
    }
}
