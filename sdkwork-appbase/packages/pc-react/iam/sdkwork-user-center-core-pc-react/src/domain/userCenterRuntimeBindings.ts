...
function resolveWindowPrefix(options: UserCenterRuntimeBindingOptions): string {
  if (normalizeRuntimeText(options.windowPrefix)) {
    return options.windowPrefix!.trim();
  }
...
export function resolveUserCenterRuntimeConfigInput<T extends UserCenterRuntimeConfigInputLike>(
  ...
  const runtimeAllowAuthorizationFallbackToAccessToken = parseRuntimeBoolean(
    readRuntimeValue(
      USER_CENTER_DEPLOYMENT_VARIABLE_NAMES.allowAuthorizationFallbackToAccessToken,
      bindings,
    ),
  );
  ...
  const runtimeAuthOverrides: UserCenterAuthProfileInput = {
    ...(runtimeAllowAuthorizationFallbackToAccessToken === undefined
      ? {}
      : {
          allowAuthorizationFallbackToAccessToken:
            runtimeAllowAuthorizationFallbackToAccessToken,
        }),
