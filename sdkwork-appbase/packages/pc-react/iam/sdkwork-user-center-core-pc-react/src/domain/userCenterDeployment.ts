...
function createBaseDeploymentVariables(options: {
  ...
  return [
    ...
    },
    {
      ...
      defaultValue: String(options.auth.allowAuthorizationFallbackToAccessToken),
      description: "Controls whether requests may reuse the AccessToken when the AuthToken is temporarily unavailable.",
      required: false,
      targets,
    },
  ];
