...
describe("sdkwork-search-pc-react", () => {
  it("creates a normalized search catalog with deterministic metadata", () => {
  ...
  it("rejects duplicate search document ids", () => {
    expect(() =>
      createSdkworkSearchCatalog([
        ...documents,
        {
  ...
  it("filters search catalogs by capability and scope and preserves group order in grouped results", () => {
    const catalog = createSdkworkSearchCatalog(documents);
  ...
  it("creates a search manifest from the normalized catalog", () => {
    const catalog = createSdkworkSearchCatalog(documents);
    ...
    expect(manifest.packageNames).toEqual(["@sdkwork/search-pc-react"]);
