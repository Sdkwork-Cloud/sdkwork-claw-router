...
export type SdkworkRtcCallType = "audio" | "video";
...
export function resolveRtcControlState(
  ...
  return {
    ...
    canToggleCamera: session.callType === "video",
    reason: undefined,
  };
}
...
export function evaluateRtcJoinReadiness(
  ...
  const canUseCamera =
    ...
    !cameraDenied &&
    !cameraMissing;
  const blockedByCamera = session.callType === "video" && cameraRequired && !canUseCamera;
  ...
  const degraded =
    ...
    (session.callType === "video" && !cameraRequired && (cameraDenied || cameraMissing));
...
export function createRtcWorkspaceManifest({
  description = "Realtime calling workspace for voice, video, and call-window routing.",
  host,
  id = "sdkwork-rtc",
  launchMode = "panel",
