...
export type SdkworkNewsItemStatus = "draft" | "published" | "scheduled";
...
export type SdkworkNewsEditorialAction =
  | "open-detail"
  | "publish"
  | "schedule";
...
export interface SdkworkNewsWorkspaceManifest extends SdkworkAppCapabilityManifest {
  capability: "news";
...
function getPrimaryTimestamp(item: SdkworkNewsItem): number {
  ...
  return Math.max(parseTimestamp(item.publishedAt), parseTimestamp(item.updatedAt));
}
...
export function filterNewsItems(
  ...
  const statuses = options.statuses?.length ? new Set(options.statuses) : new Set(["published"]);
...
export function evaluateNewsEditorialReadiness(
  ...
  const checklist: SdkworkNewsEditorialChecklist = {
    ...
    hasScheduleDate: parseTimestamp(options.scheduledFor) > 0,
  ...
  const blockedIssues = new Set<SdkworkNewsEditorialIssue>([
    "category-disabled",
    ...(action === "open-detail" ? (["unpublished"] as const) : []),
