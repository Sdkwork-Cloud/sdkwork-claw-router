...
export function createEmptySdkworkIotCatalog(
  ...
  const nodes = sortSdkworkIotNodes(options.nodes ?? createDefaultSdkworkIotNodes());
  const alerts = sortSdkworkIotAlerts(options.alerts ?? createDefaultSdkworkIotAlerts());
