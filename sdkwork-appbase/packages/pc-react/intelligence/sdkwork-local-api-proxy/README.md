...
- product-specific secrets backends
- product-owned settings wrappers
...
- provider-route compatibility helpers for product-side route editors and migrations, including legacy provider id/model-ref normalization
- provider runtime config contracts and normalizers
- provider request-override draft format/parse helpers for product-side editors
...
They should not re-implement config models, route normalization, schema shape, or host command catalogs in product code.
