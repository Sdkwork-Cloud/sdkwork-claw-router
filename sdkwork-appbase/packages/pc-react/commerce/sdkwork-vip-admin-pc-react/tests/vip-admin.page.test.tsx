...
import { createSdkworkVipAdminController, SdkworkVipAdminPage } from "../src";

describe("sdkwork-vip-admin-pc-react page", () => {
  it("renders the standard admin VIP workspace with package group operations and assignment controls", () => {
    const controller = createSdkworkVipAdminController({
      initialState: {
        activeView: "packages",
        dashboard: {
          entitlements: [
          ...
          packageGroups: [
            ...
            },
            {
              billingCycle: "annual",
              code: "annual",
              description: "Annual VIP package group",
              durationDays: 365,
              id: "group-annual",
              name: "Annual",
              sortWeight: 20,
    ...
    render(<SdkworkVipAdminPage controller={controller} />);

    expect(screen.getByRole("heading", { name: "VIP Admin" })).toBeInTheDocument();
  ...
  it("submits professional level, package, package group, and membership mutations through the controller", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Create VIP level" }));
