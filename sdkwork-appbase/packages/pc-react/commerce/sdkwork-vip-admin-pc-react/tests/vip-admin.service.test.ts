...
import {
  createSdkworkVipAdminRouteIntent,
  createSdkworkVipAdminService,
  createSdkworkVipAdminWorkspaceManifest,
} from "../src";

describe("sdkwork-vip-admin-pc-react service", () => {
  beforeEach(() => {
    configureCommerceServiceMockSession({ authToken: "admin-vip-token" });
  });
  ...
  it("loads standard admin VIP levels, package groups, packages, memberships, and entitlements through commerce admin runtime", async () => {
    ...
    const packageGroupCreate = vi.fn().mockResolvedValue({
      ...
      data: {
        ...
        code: "monthly",
        description: "Monthly VIP package group",
        durationDays: 30,
        id: "group-monthly",
        name: "Monthly",
        sortWeight: 10,
    ...
    const service = createSdkworkVipAdminService({
      commerceService,
    });
  ...
  it("returns a guest-safe empty dashboard and localizes mutation auth failures", async () => {
    ...
    ).rejects.toThrow("\u8bf7\u5148\u767b\u5f55\u540e\u518d\u7ba1\u7406 VIP \u540e\u53f0\u3002");
  });

  it("creates route intents and a standalone admin VIP workspace manifest", () => {
    expect(createSdkworkVipAdminRouteIntent({
      view: "packages",
    })).toMatchObject({
      route: "/admin/vip?view=packages",
      source: "vip-admin-workspace",
      view: "packages",
    });
    expect(createSdkworkVipAdminWorkspaceManifest()).toMatchObject({
