...
import { resolveSdkworkVipPurchaseMode } from "../vip-purchase";
import { useSdkworkVipPurchaseIntl } from "../vip-purchase-intl";
import {
  sdkworkVipPurchaseService,
  type SdkworkVipPurchaseService,
} from "../vip-purchase-service";

export interface SdkworkVipPurchaseMenuProps {
  controller: SdkworkVipController;
  onOpenCenter?: () => void;
  onPurchased?: (plan: SdkworkVipPlan) => void;
  purchaseService?: Pick<SdkworkVipPurchaseService, "submitPackagePurchase">;
}

const PAYMENT_METHODS = ["WECHAT", "ALIPAY"] as const;

export function SdkworkVipPurchaseMenu({
  controller,
  onOpenCenter,
  onPurchased,
  purchaseService,
}: SdkworkVipPurchaseMenuProps) {
  ...
  const selectedPlan = state.dashboard.plans.find((plan) => plan.packageId === selectedPackageId)
    ...
    ?? null;
  const mode = resolveSdkworkVipPurchaseMode({
    plan: selectedPlan,
  ...
  function submitSelectedPlan(): void {
    ...
    const request = (purchaseService ?? sdkworkVipPurchaseService).submitPackagePurchase({
      ...input,
      mode,
      plan: {
    ...
    void request.then(() => {
      ...
      setPurchaseError(error instanceof Error ? error.message : "VIP purchase failed.");
    }).finally(() => {
  ...
  return (
    <div
      ...
      style={createSdkworkVipPanelStyle("brand", {
        backgroundWeight: 4,
        borderWeight: 18,
      })}
    >
        ...
        {state.dashboard.plans.length === 0 ? (
          ...
          return (
            <button
              ...
              style={createSdkworkVipPanelStyle(isSelected ? "accent" : "neutral", {
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-2 text-sm text-[var(--sdk-color-text-secondary)]">
                    {formatSdkworkVipDurationLabel(plan.durationDays, locale)}
                    {" · "}
                    {formatSdkworkVipIncludedPointsLabel(plan.includedPoints, locale)}
                  </div>
                </div>
