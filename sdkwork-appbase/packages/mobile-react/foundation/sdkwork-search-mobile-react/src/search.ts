...
export function createSdkworkSearchCatalog(
  ...
  for (const document of normalizedDocuments) {
    ...
    const groupId = slugify(document.group ?? "General");
...
export function filterSdkworkSearchCatalog(
  ...
  return createSdkworkSearchCatalog(
    catalog.documents.filter((document) => {
      ...
      if (groupFilter.size > 0 && !groupFilter.has(slugify(document.group ?? "General"))) {
        return false;
      }
...
export function createSdkworkSearchManifest({
  ...
  host,
  id = "sdkwork-search",
  packageNames = ["@sdkwork/search-mobile-react"],
  theme,
  title = "Search",
...
export const searchPackageMeta = {
  ...
  domain: "foundation",
  package: "@sdkwork/search-mobile-react",
  status: "ready",
} as const;
