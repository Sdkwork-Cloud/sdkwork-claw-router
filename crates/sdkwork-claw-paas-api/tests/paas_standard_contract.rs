...
#[tokio::test]
async fn builtin_provider_plugin_returns_explicit_not_configured_until_native_adapter_exists() {
    let registry = default_paas_plugin_registry();
    let plugin = registry
        .resolve(&PaasProviderRoutingKey {
            provider_code: "baidu",
    ...
    let error = plugin
        ...
        .await
        .expect_err("built-in metadata plugin must not pretend native calls are implemented");
