...
> 依据：`DATABASE_SPEC.md`、`apps/sdkwork-clawrouter-pc`、`spring-ai-plus-backend-api` 与 `spring-ai-plus-app-api` API 标准、`spring-ai-plus-business-entity` 既有实体表结构。
...
1. `sdkwork-claw-router` 不按旧版多个前端应用继续拆分，产品面统一到 `apps/sdkwork-clawrouter-pc`，内部通过 public、console、admin 三个路由域隔离。
