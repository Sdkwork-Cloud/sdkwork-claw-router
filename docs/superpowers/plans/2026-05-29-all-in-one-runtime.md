...
Run: `cargo test -p sdkwork-claw-gateway --test edge_server edge_server_can_dispatch_to_in_process_upstreams --offline`
...
Modify `services/sdkwork-claw-gateway/src/edge_server.rs` to add `EdgeInProcessUpstreams` and a new router constructor. Dispatch matching API paths through cloned Axum routers with `tower::ServiceExt::oneshot`; keep existing HTTP forwarding as the default.
...
- Modify: `services/sdkwork-claw-gateway/src/main.rs`
...
Run: `cargo test -p sdkwork-claw-gateway --test edge_server_sqlite_smoke all_in_one_edge_router_serves_sqlite_gateway_admin_and_app_without_service_ports --offline`
...
Run:
- `cargo test -p sdkwork-claw-gateway --test edge_server edge_server_can_dispatch_to_in_process_upstreams --offline`
- `cargo test -p sdkwork-claw-gateway --test edge_server_sqlite_smoke all_in_one_edge_router_serves_sqlite_gateway_admin_and_app_without_service_ports --offline`
