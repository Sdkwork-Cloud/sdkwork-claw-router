...
2. Public app and admin API paths must remain Java-compatible:
...
8. User, VIP, account, coupon, order, payment, points recharge, PlusApp, AgentSkills, and PlusCategory domains must prefer Java-owned contracts and table models. New Claw tables are only allowed where Java does not already own the model.
...
2. align the owning Java app/backend controller and OpenAPI snapshot
...
- `sdkwork-claw-gateway`: `/v1/**` gateway runtime and health
- `sdkwork-claw-admin-api`: `/backend/v3/api/**` admin surface
- `sdkwork-claw-app-api`: `/app/v3/api/**` app/console/public surface
- `sdkwork-claw-product`: product composition entrypoint
...
The model and pricing product base lives in `sdkwork-claw-product`. It resolves `ModelVendor`, model catalog rows, provider-specific upstream costs, official reference prices, customer charge prices, `ai_pricing_plan`, and `iam_gateway_api_key_group` multipliers through a `PricingCatalog` port. API Key group binding is part of the pricing resolution path because `iam_gateway_api_key.group_id` chooses the business group and its default pricing plan.
...
- `/v1/**` runtime routes must not return Java `PlusApiResult<T>` envelopes
