...
- `sdkwork-clawrouter-gateway`: `/v1/**` gateway runtime and health
- `sdkwork-clawrouter-admin-api-server`: `/backend/v3/api/**` admin surface
- `sdkwork-clawrouter-app-api-server`: `/app/v3/api/**` app/console/public surface
...
`sdkwork-clawrouter-admin-api-server` exposes a catalog-injected router entrypoint for this route. The default service router stays on manifest 501 until bootstrap supplies a real `PricingCatalog`, which prevents accidental production mock data while still allowing route-level tests with deterministic in-memory adapters.
