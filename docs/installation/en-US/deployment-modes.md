...
| `source` | source checkout | gateway-backed client dev; PostgreSQL for explicit server dev | `pnpm dev` / `pnpm server:dev` / `pnpm start` | development, validation, private builds |
