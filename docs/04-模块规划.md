# sdkwork-claw-router 模块规划

## 1. 模块规划目标

模块规划需要服务两个方向：

1. 产品模块：与 `apps/sdkwork-claw-router-portal` 的 public、console、admin 模块对齐。
...
| 包 | 产品能力 | API 面 |
| --- | --- | --- |
| `sdkwork-claw-router-home` | 首页、能力介绍、下载入口 | app public |
| `sdkwork-claw-router-models` | 模型目录、模型详情 | app public |
| `sdkwork-claw-router-rankings` | 模型排行 | app public |
| `sdkwork-claw-router-app-center` | 应用中心 | app public |
| `sdkwork-claw-router-skills-hub` | 技能中心 | app public |
| `sdkwork-claw-router-api-reference` | API 文档、在线请求 | gateway + app public |
| `sdkwork-claw-router-sdk-reference` | SDK 文档、SDK 生成 | app public |
| `sdkwork-claw-router-playground` | 多模态调试 | gateway + app |
| `sdkwork-claw-router-forum` | 论坛 | app |
| `sdkwork-claw-router-courses` | 课程 | app |
...
| `console-routing` | 本地路由、策略、fallback、日志 | ai_routing、integration_channel |
| `console-api-keys` | Key 创建、轮换、权限 | iam_gateway_api_key |
| `console-user` | 用户资料 | existing `plus_user` |
| `console-billing` | 钱包、套餐、支付 | existing commerce/account tables |
...
| `console-messages` | 消息中心 | content/comms/ops notifications |
