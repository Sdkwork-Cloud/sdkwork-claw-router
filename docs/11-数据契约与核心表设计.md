...
说明：`provider_code` 解决“怎么接入”的问题，`vendor_code` 解决“模型是谁发布的”的问题。OpenRouter、Azure、AWS Bedrock、GCP Vertex AI 这类平台通常是 `Provider`，不是 `ModelVendor`；它们托管的模型通过 `ai_channel_model.vendor_code` 和 `ai_model.vendor_code` 标明原厂。
...
- `uk_ai_channel_model_uuid(uuid)`
- `uk_ai_channel_model_active(tenant_id, organization_id, channel_id, model, provider_model, capability, effective_from)`
- `idx_ai_channel_model_tenant_gateway_status(tenant_id, organization_id, model, status)`
...
| 某模型可通过哪些渠道调用 | `ai_channel_model` | `model_id` 或 `model + vendor_code` |
