# sdkwork-claw-router 数据契约与核心表设计

> 版本：v0.1
> 日期：2026-04-28
> 范围：`sdkwork-claw-router` 新增数据域、存量 `plus_*` 表复用边界、核心表契约、索引、留存、事件一致性和 CI 校验。
> 依据：[DATABASE_SPEC.md](../DATABASE_SPEC.md)、[05-数据库设计.md](./05-数据库设计.md)、[12-前端功能模块与数据库表结构映射.md](./12-前端功能模块与数据库表结构映射.md)、[13-页面级数据结构覆盖与SchemaRegistry落地设计.md](./13-页面级数据结构覆盖与SchemaRegistry落地设计.md)、`spring-ai-plus-business-entity` 既有实体、`spring-ai-plus-app-api`、`spring-ai-plus-backend-api`。
...
本轮只打磨设计，不修改 `spring-ai-plus-business-entity` 既有表结构，不生成生产迁移。

当前 portal 前端 public、console、admin 模块到数据库表和字段的完整映射见 [12-前端功能模块与数据库表结构映射.md](./12-前端功能模块与数据库表结构映射.md)。页面级覆盖验收、字段级复核和机器可校验表注册表见 [13-页面级数据结构覆盖与SchemaRegistry落地设计.md](./13-页面级数据结构覆盖与SchemaRegistry落地设计.md)、[14-数据结构细节复核与补强记录.md](./14-数据结构细节复核与补强记录.md) 与 [schema-registry/sdkwork-claw-router.tables.yaml](./schema-registry/sdkwork-claw-router.tables.yaml)。本文负责核心数据契约，12 号文档负责从前端产品面反推完整表结构覆盖，13 号文档负责页面级覆盖闭环和 Registry 落地规则，14 号文档负责 service/interface/mock data 字段级缺口复核。

核心目标：

- 保持用户、VIP、账户、优惠券、积分充值、订单、支付、退款、发票等 `plus_*` 表结构完全一致。
- 数据库模型采用 Java Entity first：任何新增数据模型先查 `spring-ai-plus-business-entity`；只要存在 `Plus*` Entity，就必须沿用对应 `plus_*` 表和 Java app/backend API，不得在 claw-router 下新建同义主数据表。
...
| --- | --- | --- | --- | --- |
| 存量主数据层 | Java 业务实体已有事实来源 | `plus_user`、`plus_account`、`plus_coupon`、`plus_vip_*`、`plus_order`、`plus_payment` | 既有 Java service/repository | 保持现状，claw-router 不直接改结构 |
| 控制面配置层 | 网关域配置、Provider、模型厂家、模型、策略、Key 扩展 | `iam_*`、`integration_*`、`ai_model_vendor`、`ai_model`、`ai_routing_*` | claw-router control-plane | 强一致写入，变更发布到缓存 |
...
| 结算投影层 | 用量到资金/积分账户的桥接证据 | `commerce_usage_settlement`、`commerce_billing_export` | settlement worker | 幂等，不能替代 `plus_account_history` |
...
以下业务域不在 claw-router 中创建替代表。表结构、字段、索引、枚举转换、加密转换、审计字段均以 `spring-ai-plus-business-entity` 为准。
...
| 优惠券营销 | `plus_coupon`、`plus_coupon_template`、`plus_user_coupon`、`plus_invitation_code`、`plus_invitation_relation`、`plus_partner` | 券模板、用户券、邀请码、伙伴 |
...
| `plus_coupon*` | 券模板、券实例、用户券 | Billing/redeem 功能只调用既有营销能力 |
...
| `commerce_coupon`、`commerce_coupon_template`、`commerce_user_coupon` | 会形成双券事实 |
...
| 名称 | 类型 | 字段 |
| --- | --- | --- |
| `uk_iam_gateway_api_key_group_uuid` | unique | `uuid` |
| `uk_iam_gateway_api_key_group_tenant_code` | unique | `tenant_id, organization_id, code` |
| `idx_iam_gateway_api_key_group_provider_status` | index | `tenant_id, organization_id, provider_code, status, updated_at, id` |
| `idx_iam_gateway_api_key_group_tenant_status_updated` | index | `tenant_id, organization_id, status, updated_at, id` |
| `idx_iam_gateway_api_key_group_pricing` | index | `tenant_id, organization_id, pricing_plan_id, status, updated_at, id` |

#### 6.1.1 `iam_gateway_api_key_group_metric_snapshot`
...
| `idx_iam_gateway_api_key_group_status` | index | `tenant_id, organization_id, group_id, status` |
...
| `Channel` | `integration_channel.channel_code` | 租户/组织可路由的具体接入实例 | 某个 Azure region、某个 OpenRouter 账号、某个本地 Ollama 节点 |
...
| `icon_url` | string(512) | 否 | Provider 图标 |
...
| `base_url_template` | string(512) | 否 | 默认 base URL 模板，不含 secret |
...
说明：`provider_code` 解决“怎么接入”的问题，`vendor_code` 解决“模型是谁发布的”的问题。OpenRouter、Azure、AWS Bedrock、GCP Vertex AI 这类平台通常是 `Provider`，不是 `ModelVendor`；它们托管的模型通过 `integration_channel_model.vendor_code` 和 `ai_model.vendor_code` 标明原厂。

### 7.2 `integration_channel`
...
| `base_url_override` | string(512) | 否 | 渠道级 base URL 覆盖，不保存密钥 |
...
- `uk_integration_channel_uuid(uuid)`
...
### 7.4 `integration_channel_model`

用途：渠道与网关模型、供应商模型的映射关系。

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `channel_id` | int64 | 是 | 渠道 |
| `model_id` | int64 | 是 | `ai_model.id` |
| `model` | string(128) | 是 | 网关模型名快照 |
| `vendor_code` | string(64) | 否 | 实际模型厂家编码，来自 `ModelVendor` |
| `provider_model` | string(128) | 是 | 供应商模型名 |
| `capability` | enum_int32 | 是 | chat、embedding、image、audio、video 等 |
| `model_aliases` | json | 否 | 别名 |
| `default_parameters` | json | 否 | 默认参数 |
| `max_input_tokens` | int64 | 否 | 输入上限 |
| `max_output_tokens` | int64 | 否 | 输出上限 |
| `supports_streaming` | bool | 是 | 是否支持流式 |
| `supports_tools` | bool | 是 | 是否支持工具调用 |
| `effective_from` | instant | 是 | 生效时间 |
| `effective_to` | instant | 否 | 失效时间 |

约束：

- `uk_integration_channel_model_uuid(uuid)`
- `uk_integration_channel_model_active(tenant_id, organization_id, channel_id, model, provider_model, capability, effective_from)`
- `idx_integration_channel_model_tenant_gateway_status(tenant_id, organization_id, model, status)`
- `idx_integration_channel_model_vendor_status(tenant_id, organization_id, vendor_code, status, effective_from, id)`

### 7.5 `integration_proxy`
...
| `logo_url` | string(512) | 否 | 品牌 logo |
| `icon_url` | string(512) | 否 | 小图标 |
...
用途：模型价格簿。新表必须使用 decimal，不允许 float/double。它用 `price_side` 区分官方参考价、供应商上游成本价、客户销售价和内部结算价，用 `pricing_scope` 表示 global、tenant、organization、sku、api_key_group、provider、channel 等生效范围。一个 `ai_model` 可以有多条 `upstream_cost` 价格，对应不同 `provider_code/channel_id/provider_model`；也可以有多条 `customer_charge` 价格，对应不同定价方案、租户或 SKU。
...
| `channel_id` | int64 | 否 | 渠道级价格时关联 `integration_channel.id` |
...
| `pricing_scope` | enum_int32 | 是 | global、tenant、organization、sku、api_key_group、provider、channel |
...
| 某模型可通过哪些渠道调用 | `integration_channel_model` | `model_id` 或 `model + vendor_code` |
...
用途：定价方案主表。它不是用户组，也不是 API Key 分组本身，而是“如何从参考价计算销售价”的策略集合。`iam_gateway_api_key_group` 是创建 API Key 时选择的业务分组事实来源，可以直接挂默认 `pricing_plan_id`；更复杂场景通过 `ai_pricing_plan_binding` 把定价方案绑定到用户、VIP、SKU、租户或单个 API Key。
...
| `description` | string(512) | 否 | 说明 |
| `plan_scope` | enum_int32 | 是 | global、tenant、organization、api_key_group、api_key、vip、sku、user |
...
用途：定价方案绑定表，解决 sub2api 中 API Key 绑定 group、account 绑定 group、用户专属 group rate 的需求，同时避免修改 `plus_user`、`plus_vip_user`、`plus_account` 等存量事实表。业务分组关系仍由 `iam_gateway_api_key.group_id` 和 `iam_gateway_api_key_group` 表达；该表只处理“某个主体临时或专属使用哪套定价方案”。

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `pricing_plan_id` | int64 | 是 | `ai_pricing_plan.id` |
| `pricing_plan_code` | string(64) | 是 | 定价方案编码快照 |
| `subject_type` | enum_int32 | 是 | tenant、organization、api_key_group、api_key、user、vip_level、sku、account |
...
| `api_key_group_snapshot` | string(128) | 否 | Key 分组快照 |
...
| `provider_account_id` | int64 | 否 | Provider 账号 |
| `requested_model` | string(128) | 是 | 请求模型 |
...
| `api_key_name_snapshot` | string(128) | 否 | Key 名称快照 |
| `api_key_group_id` | int64 | 否 | Key 分组 ID 快照 |
| `api_key_group_snapshot` | string(128) | 否 | Key 分组快照 |
...
| `provider_id` | int64 | 否 | Provider |
| `channel_id` | int64 | 否 | 渠道 |
| `provider_account_id` | int64 | 否 | 账号 |
...
- 结算到 `plus_account_history` 后，必须把账户流水 ID 记录到 `commerce_usage_settlement`。
...
| `request_id` | string(128) | 是 | 请求 ID |
| `account_id` | int64 | 否 | `plus_account.id` |
| `account_history_id` | int64 | 否 | `plus_account_history.id` |
| `order_id` | int64 | 否 | `plus_order.id` |
| `payment_id` | int64 | 否 | `plus_payment.id` |
| `asset_type` | enum_int32 | 是 | balance、points、token、free_quota |
| `direction` | enum_int32 | 是 | debit、credit、refund、compensate |
...
| 表 | 用途 | 关键字段 |
| --- | --- | --- |
| `plus_app` | AppCenter/PlusApp 主数据 | `name`、`icon`、`resource_list`、`project_id`、`description`、`version`、`icon_url`、`access_url`、`config`、`status`、`app_type`、`platforms`、`install_platforms`、`install_skill`、`install_config`、`release_notes`、`package_name`、`bundle_id`、`store_url`、`download_url` |
| `plus_agent_skill` | SkillsHub/AgentSkill 主数据 | `skill_key`、`name`、`summary`、`description`、`icon`、`cover_image`、`category_id`、`package_id`、`provider`、`version`、`manifest_url`、`license_name`、`market_status`、`visibility`、`review_status`、`install_count`、`rating_avg`、`capabilities`、`default_config`、`latest_published_at` |
| `plus_agent_skill_package` | 技能包/集合 | `package_key`、`name`、`summary`、`description`、`icon`、`cover_image`、`category_id`、`enabled`、`featured`、`sort_weight`、`tags`、`latest_published_at` |
...
| `content_course` | 课程 | `course_code`、`title`、`summary`、`cover_url`、`level`、`published_at` |
| `content_course_section` | 课程章节分组 | `course_id`、`section_no`、`title`、`sort_order`、`lesson_count`、`duration_seconds` |
| `content_course_lesson` | 课程课时 | `course_id`、`section_id`、`lesson_no`、`title`、`video_url`、`external_bvid`、`duration_seconds` |
...
/v1 request
  ...
  -> plus_account / plus_account_history by account service
```
...
- `plus_account_history` 是资金/积分最终流水事实，不能被 `commerce_usage_settlement` 替代。
...
| 账户扣减成功但回写 settlement 失败 | 通过 `plus_account_history.transaction_id` 和 `settlement_no` 对账修复 |
...
- 新表中只保存必要的 user_id、owner_id、account_id、account_history_id 引用，不复制手机号、邮箱、地址、支付明细。
...
- 优惠券替代表：`commerce_coupon`、`commerce_coupon_template`、`commerce_user_coupon`。
...
- 账务闭环：`ai_usage_fact` 是用量事实，`commerce_usage_settlement` 是结算桥接，`plus_account_history` 是最终账户流水事实。
