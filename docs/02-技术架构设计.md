# sdkwork-claw-router 技术架构设计

## 1. 架构目标

`sdkwork-claw-router` 是 `apps/sdkwork-api-router` 的 Rust-first 高性能重构项目。它不是对旧系统的轻量修补，而是围绕统一数据库标准、Java-compatible API 路径、生成 SDK 边界、统一 portal 产品体验重新建立的标准化运行时。
...
apps/sdkwork-claw-router-portal
  ...
  -> /app/v3/api/**      sdkwork-claw-app-api
  -> /backend/v3/api/**  sdkwork-claw-admin-api

OpenAI-compatible SDK / curl / third-party clients
  -> /v1/**              sdkwork-claw-gateway
...
`spring-ai-plus-app-api` 和 `spring-ai-plus-backend-api` 仍然是 API 路径、OpenAPI snapshot、业务实体兼容和 SDK 生成标准的参考源。`sdkwork-claw-router` 以同一路径实现、代理或装配能力，确保前端和外部调用方只切换 base URL 即可自由切换到 Java-compatible 或 Rust runtime。

## 3. Runtime Plane

| Plane | Rust 服务/模块 | 职责 | 公共路径 |
| --- | --- | --- | --- |
| Gateway Plane | `sdkwork-claw-gateway` | OpenAI-compatible 请求、streaming、provider relay、routing、quota、usage finalize | `/v1/**` |
| App Plane | `sdkwork-claw-app-api` | console/public 用户自助 API，用户上下文和资源归属校验 | `/app/v3/api/**` |
| Admin Plane | `sdkwork-claw-admin-api` | admin 控制台 API，后台 RBAC/ABAC、审计和配置治理 | `/backend/v3/api/**` |
| Product Plane | `sdkwork-claw-product` | 本地桌面/服务端组合入口、静态 portal 装配、部署 profile glue | internal |
...
3. 使用 API key hash 得到 tenant、organization、user、api_key、api_key_group 和 data_scope。
...
数据库以 `docs/schema-registry/sdkwork-claw-router.tables.yaml` 为唯一新增表契约来源。
...
`sdkwork-claw-router` 使用统一 Rust runtime，按 profile 变更基础设施装配：
