...
apps/sdkwork-clawrouter-pc
  ...
  -> /app/v3/api/**      sdkwork-claw-app
  -> /backend/v3/api/**  sdkwork-claw-admin
...
| App Plane | `sdkwork-claw-app` | console/public 用户自助 API，用户上下文和资源归属校验 | `/app/v3/api/**` |
| Admin Plane | `sdkwork-claw-admin` | admin 控制台 API，后台 RBAC/ABAC、审计和配置治理 | `/backend/v3/api/**` |
