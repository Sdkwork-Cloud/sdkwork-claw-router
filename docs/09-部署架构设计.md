# sdkwork-claw-router 部署架构设计

## 1. 部署目标

`sdkwork-claw-router` 使用 Rust-first Rust services 支持四种部署方式：
...
Desktop Shell / local launcher
  ...
  -> sdkwork-claw-product
      -> sdkwork-claw-gateway      127.0.0.1:18080
      -> sdkwork-claw-admin-api    127.0.0.1:18081
      -> sdkwork-claw-app-api      127.0.0.1:18082
      -> SQLite
      -> memory/moka cache
...
| Deployment | 扩缩容依据 |
| --- | --- |
| `sdkwork-claw-gateway` | QPS、并发、CPU、network、TTFT |
| `sdkwork-claw-app-api` | console/public 请求量 |
| `sdkwork-claw-admin-api` | admin 请求量、后台任务触发 |
