...
import type { FetchGenerationHistoryResult } from '../types';
...
/** List generation history */
  async fetchGenerationHistory(pageNo?: number, pageSize?: number, keyword?: string, status?: string, startTime?: string, endTime?: string): Promise<FetchGenerationHistoryResult> {
    ...
    return this.client.get<FetchGenerationHistoryResult>(appendQueryString(appApiPath(`/playground/history`), query));
  }
}

export function createPlaygroundApi(client: HttpClient): PlaygroundApi {
  return new PlaygroundApi(client);
}
