...
from ..models import EntriesCreateResult, EntriesListResult, EntriesRetrieveResult, MemoryEntryCreateRequest, MemorySpaceCreateRequest, SpacesCreateResult, SpacesListResult, SpacesRetrieveResult
...
class MemoryApi:
    """memory memory API client."""

    def __init__(self, client: HttpClient):
        self._client = client
        self.entries = MemoryEntriesApi(client)
        self.spaces = MemorySpacesApi(client)


class MemoryEntriesApi:
    """memory memory.entries API client."""

    def __init__(self, client: HttpClient):
        self._client = client


    def retrieve(self, entry_id: str) -> EntriesRetrieveResult:
    ...
    def create(self, space_id: str, body: MemoryEntryCreateRequest, idempotency_key: str) -> EntriesCreateResult:
        ...
        return self._client.post(f"/app/v3/api/memory/spaces/{serialize_path_parameter(space_id, {'name': 'spaceId', 'style': 'simple', 'explode': False})}/entries", json=body, headers=request_headers)

class MemorySpacesApi:
    """memory memory.spaces API client."""
    ...
    def create(self, body: MemorySpaceCreateRequest, idempotency_key: str) -> SpacesCreateResult:
        ...
        return self._client.post(f"/app/v3/api/memory/spaces", json=body, headers=request_headers)

    def retrieve(self, space_id: str) -> SpacesRetrieveResult:
        """Retrieve memory space"""
        return self._client.get(f"/app/v3/api/memory/spaces/{serialize_path_parameter(space_id, {'name': 'spaceId', 'style': 'simple', 'explode': False})}")
