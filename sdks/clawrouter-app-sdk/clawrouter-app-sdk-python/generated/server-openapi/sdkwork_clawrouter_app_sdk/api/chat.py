...
from ..models import ChatConversationCreateRequest, ChatTurnCreateRequest, ChatTurnResponseRequest, ConversationMessagesListResult, ConversationsCreateResult, ConversationsListResult, ConversationsRetrieveResult, TurnResponsesCreateResult, TurnsCreateResult
...
class ChatApi:
    ...
    def __init__(self, client: HttpClient):
        ...
        self.conversation_messages = ChatConversationMessagesApi(client)
        self.turns = ChatTurnsApi(client)
        self.turn_responses = ChatTurnResponsesApi(client)
...
class ChatConversationsApi:
    ...
    def create(self, body: ChatConversationCreateRequest, idempotency_key: str) -> ConversationsCreateResult:
        ...
        return self._client.post(f"/app/v3/api/chat/conversations", json=body, headers=request_headers)

    def retrieve(self, conversation_id: str) -> ConversationsRetrieveResult:
        """Retrieve product chat conversation"""
        return self._client.get(f"/app/v3/api/chat/conversations/{serialize_path_parameter(conversation_id, {'name': 'conversationId', 'style': 'simple', 'explode': False})}")

class ChatConversationMessagesApi:
    """chat chat.conversation_messages API client."""
    ...
    def list(self, conversation_id: str, limit: Optional[int] = None, order: Optional[str] = None) -> ConversationMessagesListResult:
        ...
        return self._client.get(_append_query_string(f"/app/v3/api/chat/conversations/{serialize_path_parameter(conversation_id, {'name': 'conversationId', 'style': 'simple', 'explode': False})}/messages", query))

class ChatTurnsApi:
    """chat chat.turns API client."""
    ...
    def create(self, conversation_id: str, body: ChatTurnCreateRequest, idempotency_key: str) -> TurnsCreateResult:
        ...
        return self._client.post(f"/app/v3/api/chat/conversations/{serialize_path_parameter(conversation_id, {'name': 'conversationId', 'style': 'simple', 'explode': False})}/turns", json=body, headers=request_headers)

class ChatTurnResponsesApi:
    """chat chat.turn_responses API client."""
    ...
    def create(self, conversation_id: str, turn_id: str, body: ChatTurnResponseRequest, idempotency_key: str) -> TurnResponsesCreateResult:
        ...
        return self._client.post(f"/app/v3/api/chat/conversations/{serialize_path_parameter(conversation_id, {'name': 'conversationId', 'style': 'simple', 'explode': False})}/turns/{serialize_path_parameter(turn_id, {'name': 'turnId', 'style': 'simple', 'explode': False})}/response", json=body, headers=request_headers)
