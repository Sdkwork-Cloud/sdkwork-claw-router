...
class ChatApi {
  ...
  Future<ConversationsListResult?> conversationsList([int? page, int? pageSize]) async {
    ...
    final response = await _client.get(ApiPaths.appendQueryString(ApiPaths.appPath('/chat/conversations'), query));
    return (() {
  ...
  Future<ConversationsCreateResult?> conversationsCreate(ChatConversationCreateRequest body, String idempotencyKey) async {
    ...
    final response = await _client.post(ApiPaths.appPath('/chat/conversations'), body: payload, headers: requestHeaders, contentType: 'application/json');
    return (() {
      final map = sdkworkResponseAsMap(response);
  ...
  Future<ConversationMessagesListResult?> conversationMessagesList(String conversationId, [int? limit, String? order]) async {
    ...
    final response = await _client.get(ApiPaths.appendQueryString(ApiPaths.appPath('/chat/conversations/${serializePathParameter(conversationId, const PathParameterSpec('conversationId', 'simple', false))}/messages'), query));
    return (() {
      final map = sdkworkResponseAsMap(response);
  ...
  Future<TurnsCreateResult?> turnsCreate(String conversationId, ChatTurnCreateRequest body, String idempotencyKey) async {
    ...
    final payload = body.toJson();
    final response = await _client.post(ApiPaths.appPath('/chat/conversations/${serializePathParameter(conversationId, const PathParameterSpec('conversationId', 'simple', false))}/turns'), body: payload, headers: requestHeaders, contentType: 'application/json');
    return (() {
      final map = sdkworkResponseAsMap(response);
  ...
  Future<TurnResponsesCreateResult?> turnResponsesCreate(String conversationId, String turnId, ChatTurnResponseRequest body, String idempotencyKey) async {
    ...
    final payload = body.toJson();
    final response = await _client.post(ApiPaths.appPath('/chat/conversations/${serializePathParameter(conversationId, const PathParameterSpec('conversationId', 'simple', false))}/turns/${serializePathParameter(turnId, const PathParameterSpec('turnId', 'simple', false))}/response'), body: payload, headers: requestHeaders, contentType: 'application/json');
    return (() {
      final map = sdkworkResponseAsMap(response);
