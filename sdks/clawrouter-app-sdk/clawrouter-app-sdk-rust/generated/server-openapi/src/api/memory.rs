...
use crate::models::{EntriesCreateResult, EntriesListResult, EntriesRetrieveResult, MemoryEntryCreateRequest, MemorySpaceCreateRequest, SpacesCreateResult, SpacesListResult, SpacesRetrieveResult};
...
impl MemoryApi {
    ...
    pub async fn entries_retrieve(&self, entry_id: &str) -> Result<EntriesRetrieveResult, SdkworkError> {
        let path = app_path(&format!("/memory/entries/{}", serialize_path_parameter(entry_id, PathParameterSpec::new("entryId", "simple", false))));
    ...
    pub async fn spaces_list(&self, page: Option<i64>, page_size: Option<i64>) -> Result<SpacesListResult, SdkworkError> {
        ...
        let path = append_query_string(app_path(&"/memory/spaces".to_string()), &query);
        self.client.get(&path, None, None).await
    }
    ...
    pub async fn spaces_retrieve(&self, space_id: &str) -> Result<SpacesRetrieveResult, SdkworkError> {
        let path = app_path(&format!("/memory/spaces/{}", serialize_path_parameter(space_id, PathParameterSpec::new("spaceId", "simple", false))));
        self.client.get(&path, None, None).await
    }
    ...
    pub async fn entries_list(&self, space_id: &str, page: Option<i64>, page_size: Option<i64>) -> Result<EntriesListResult, SdkworkError> {
        ...
        let path = append_query_string(app_path(&format!("/memory/spaces/{}/entries", serialize_path_parameter(space_id, PathParameterSpec::new("spaceId", "simple", false)))), &query);
        self.client.get(&path, None, None).await
    }
