...
impl AuthApi {
    ...
    pub async fn registrations_create(&self, body: &IamRegistrationCreateRequest) -> Result<RegistrationsCreateResult, SdkworkError> {
        let path = app_path(&"/auth/registrations".to_string());
        self.client.post(&path, Some(body), None, None, Some("application/json")).await
    }

    /// Create IAM session
    pub async fn sessions_create(&self, body: &IamSessionCreateRequest) -> Result<SessionsCreateResult, SdkworkError> {
