...
use crate::models::{AppSkillConfigRequest, SkillsCategoriesListResult, SkillsConfigUpdateResult, SkillsDisableResult, SkillsEnableResult, SkillsListResult, SkillsRetrieveResult, UsersCurrentSkillsListResult};
...
impl EcosystemApi {
    ...
    pub async fn skills_list(&self, q: Option<&str>, page: Option<i64>, page_size: Option<i64>, status: Option<&str>, start_time: Option<&str>, end_time: Option<&str>) -> Result<SkillsListResult, SdkworkError> {
        ...
        let path = append_query_string(app_path(&"/ecosystem/skills".to_string()), &query);
        self.client.get(&path, None, None).await
    }

    /// Get categories
    pub async fn skills_categories_list(&self) -> Result<SkillsCategoriesListResult, SdkworkError> {
        let path = app_path(&"/ecosystem/skills/categories".to_string());
        self.client.get(&path, None, None).await
    }

    /// Get skill by ID
    pub async fn skills_retrieve(&self, skill_id: &str) -> Result<SkillsRetrieveResult, SdkworkError> {
        let path = app_path(&format!("/ecosystem/skills/{}", serialize_path_parameter(skill_id, PathParameterSpec::new("skillId", "simple", false))));
        self.client.get(&path, None, None).await
    }

    /// Update skill config
    pub async fn skills_config_update(&self, skill_id: &str, body: &AppSkillConfigRequest, x_request_id: Option<&str>) -> Result<SkillsConfigUpdateResult, SdkworkError> {
        let path = app_path(&format!("/ecosystem/skills/{}/config", serialize_path_parameter(skill_id, PathParameterSpec::new("skillId", "simple", false))));
    ...
    pub async fn skills_disable(&self, skill_id: &str, x_request_id: Option<&str>) -> Result<SkillsDisableResult, SdkworkError> {
        let path = app_path(&format!("/ecosystem/skills/{}/disable", serialize_path_parameter(skill_id, PathParameterSpec::new("skillId", "simple", false))));
        let headers = build_request_headers(
            &[
                ("X-Request-Id", HeaderParameterSpec::new(x_request_id, "simple", false, None)),
            ],
    ...
    /// Enable skill
    pub async fn skills_enable(&self, skill_id: &str, body: &AppSkillConfigRequest, x_request_id: Option<&str>) -> Result<SkillsEnableResult, SdkworkError> {
        let path = app_path(&format!("/ecosystem/skills/{}/enable", serialize_path_parameter(skill_id, PathParameterSpec::new("skillId", "simple", false))));
        let headers = build_request_headers(
            &[
                ("X-Request-Id", HeaderParameterSpec::new(x_request_id, "simple", false, None)),
            ],
    ...
    /// Get my skills
    pub async fn users_current_skills_list(&self) -> Result<UsersCurrentSkillsListResult, SdkworkError> {
        let path = app_path(&"/ecosystem/users/current/skills".to_string());
        self.client.get(&path, None, None).await
    }
