...
impl SystemApi {
    ...
    pub async fn site_runtime_retrieve(&self, tenant_code: Option<&str>, organization_code: Option<&str>) -> Result<SiteRuntimeRetrieveResult, SdkworkError> {
        let query = build_query_string(&[
            QueryParameterSpec::new("tenant_code", tenant_code, "form", true, false, None),
            QueryParameterSpec::new("organization_code", organization_code, "form", true, false, None),
        ]);
        let path = append_query_string(app_path(&"/system/site/runtime".to_string()), &query);
        self.client.get(&path, None, None).await
    }
