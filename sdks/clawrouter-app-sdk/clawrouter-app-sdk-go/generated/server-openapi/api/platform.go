...
func (a *PlatformApi) AppsStoreList(page *int, pageSize *int, q *string, category *string, platformType *string, platformTypes []string, sort *string, status *string, startTime *string, endTime *string) (sdktypes.AppsStoreListResult, error) {
    ...
    raw, err := a.client.Get(AppendQueryString(AppApiPath("/platform/apps/store"), query), nil, nil)
    if err != nil {
        var zero sdktypes.AppsStoreListResult
        return zero, err
    }
    return decodeResult[sdktypes.AppsStoreListResult](raw)
}

// Get categories
func (a *PlatformApi) AppsStoreCategoriesList() (sdktypes.AppsStoreCategoriesListResult, error) {
    raw, err := a.client.Get(AppApiPath("/platform/apps/store/categories"), nil, nil)
    if err != nil {
        var zero sdktypes.AppsStoreCategoriesListResult
        return zero, err
    }
    return decodeResult[sdktypes.AppsStoreCategoriesListResult](raw)
}

// Get app by ID
func (a *PlatformApi) AppsStoreRetrieve(appId string) (sdktypes.AppsStoreRetrieveResult, error) {
    raw, err := a.client.Get(AppApiPath(fmt.Sprintf("/platform/apps/store/%s", SerializePathParameter(appId, PathParameterSpec{Name: "appId", Style: "simple", Explode: false}))), nil, nil)
    if err != nil {
        var zero sdktypes.AppsStoreRetrieveResult
        return zero, err
    }
    return decodeResult[sdktypes.AppsStoreRetrieveResult](raw)
}
