...
class PlatformApi(private val client: HttpClient) {
    ...
    suspend fun appsStoreList(page: Int? = null, pageSize: Int? = null, q: String? = null, category: String? = null, platformType: String? = null, platformTypes: List<String>? = null, sort: String? = null, status: String? = null, startTime: String? = null, endTime: String? = null): AppsStoreListResult? {
        ...
        val raw = client.get(ApiPaths.appendQueryString(ApiPaths.appPath("/platform/apps/store"), query))
        return client.convertValue(raw, object : TypeReference<AppsStoreListResult>() {})
    }

    /** Get categories */
    suspend fun appsStoreCategoriesList(): AppsStoreCategoriesListResult? {
        val raw = client.get(ApiPaths.appPath("/platform/apps/store/categories"))
        return client.convertValue(raw, object : TypeReference<AppsStoreCategoriesListResult>() {})
    }

    /** Get app by ID */
    suspend fun appsStoreRetrieve(appId: String): AppsStoreRetrieveResult? {
        val raw = client.get(ApiPaths.appPath("/platform/apps/store/${serializePathParameter(appId, PathParameterSpec("appId", "simple", false))}"))
        return client.convertValue(raw, object : TypeReference<AppsStoreRetrieveResult>() {})
    }
