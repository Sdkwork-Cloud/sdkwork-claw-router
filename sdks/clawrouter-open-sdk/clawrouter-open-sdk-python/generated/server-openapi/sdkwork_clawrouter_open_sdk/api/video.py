...
from ..models import DeleteResult, OpenAiVideo, OpenAiVideoCharacter, OpenAiVideoCharacterCreateRequest, OpenAiVideoCreateRequest, OpenAiVideoEditRequest, OpenAiVideoExtendRequest, OpenAiVideoList, OpenAiVideoRemixRequest
...
class VideoApi:
    ...
    def list(self, limit: Optional[int] = None, order: Optional[str] = None, after: Optional[str] = None, before: Optional[str] = None) -> OpenAiVideoList:
        """List videos"""
        ...
        return self._client.get(_append_query_string(f"/v1/videos", query))

    def create(self, body: OpenAiVideoCreateRequest) -> OpenAiVideo:
        """Create video"""
        return self._client.post(f"/v1/videos", json=body)

    def create_character(self, body: OpenAiVideoCharacterCreateRequest) -> OpenAiVideoCharacter:
        """Create video character"""
        return self._client.post(f"/v1/videos/characters", json=body)

    def list_characters(self, character_id: str) -> OpenAiVideoCharacter:
        """Retrieve video character"""
        return self._client.get(f"/v1/videos/characters/{serialize_path_parameter(character_id, {'name': 'character_id', 'style': 'simple', 'explode': False})}")

    def create_edit(self, body: OpenAiVideoEditRequest) -> OpenAiVideo:
        """Edit video"""
        return self._client.post(f"/v1/videos/edits", json=body)

    def create_extension(self, body: OpenAiVideoExtendRequest) -> OpenAiVideo:
        """Extend video"""
        return self._client.post(f"/v1/videos/extensions", json=body)

    def delete(self, video_id: str) -> DeleteResult:
        """Delete video"""
        return self._client.delete(f"/v1/videos/{serialize_path_parameter(video_id, {'name': 'video_id', 'style': 'simple', 'explode': False})}")
    ...
    def create_remix(self, video_id: str, body: OpenAiVideoRemixRequest) -> OpenAiVideo:
        """Remix video"""
        return self._client.post(f"/v1/videos/{serialize_path_parameter(video_id, {'name': 'video_id', 'style': 'simple', 'explode': False})}/remix", json=body)
