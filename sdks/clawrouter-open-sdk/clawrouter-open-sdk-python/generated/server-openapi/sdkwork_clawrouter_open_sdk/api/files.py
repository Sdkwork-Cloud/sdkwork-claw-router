...
from ..models import DeleteResult, OpenAiFile, OpenAiFileList, OpenAiFileUploadRequest
...
class FilesApi:
    ...
    def list(self, limit: Optional[int] = None, order: Optional[str] = None, after: Optional[str] = None, before: Optional[str] = None) -> OpenAiFileList:
        """List files"""
        ...
        return self._client.get(_append_query_string(f"/v1/files", query))

    def create(self, body: OpenAiFileUploadRequest) -> OpenAiFile:
        """Upload file"""
        return self._client.post(f"/v1/files", data=body)

    def delete(self, file_id: str) -> DeleteResult:
        """Delete file"""
        return self._client.delete(f"/v1/files/{serialize_path_parameter(file_id, {'name': 'file_id', 'style': 'simple', 'explode': False})}")
