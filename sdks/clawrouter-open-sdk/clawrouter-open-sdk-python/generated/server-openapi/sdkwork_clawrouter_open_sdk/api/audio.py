...
from ..models import DeleteResult, OpenAiAudioTranscription, OpenAiAudioTranscriptionRequest, OpenAiAudioTranslation, OpenAiAudioTranslationRequest, OpenAiSpeechCreateRequest, OpenAiVoice, OpenAiVoiceConsent, OpenAiVoiceConsentCreateRequest, OpenAiVoiceConsentList, OpenAiVoiceCreateRequest, OpenAiVoiceList
...
class AudioApi:
    ...
    def create_transcription(self, body: OpenAiAudioTranscriptionRequest) -> OpenAiAudioTranscription:
        ...
        return self._client.post(f"/v1/audio/transcriptions", json=body)

    def create_translation(self, body: OpenAiAudioTranslationRequest) -> OpenAiAudioTranslation:
        """Create translation"""
        return self._client.post(f"/v1/audio/translations", json=body)

    def list_voice_consents(self, limit: Optional[int] = None, order: Optional[str] = None, after: Optional[str] = None, before: Optional[str] = None) -> OpenAiVoiceConsentList:
        """List voice consents"""
        ...
        return self._client.get(_append_query_string(f"/v1/audio/voice_consents", query))

    def create_voice_consent(self, body: OpenAiVoiceConsentCreateRequest) -> OpenAiVoiceConsent:
        """Create voice consent"""
        return self._client.post(f"/v1/audio/voice_consents", json=body)

    def delete_voice_consents(self, consent_id: str) -> DeleteResult:
        """Delete voice consent"""
        return self._client.delete(f"/v1/audio/voice_consents/{serialize_path_parameter(consent_id, {'name': 'consent_id', 'style': 'simple', 'explode': False})}")

    def list_voices(self, limit: Optional[int] = None, order: Optional[str] = None, after: Optional[str] = None, before: Optional[str] = None) -> OpenAiVoiceList:
        """List voices"""
        query = build_query_string([
            {'name': 'limit', 'value': limit, 'style': 'form', 'explode': True, 'allow_reserved': False},
            {'name': 'order', 'value': order, 'style': 'form', 'explode': True, 'allow_reserved': False},
        ...
        return self._client.get(_append_query_string(f"/v1/audio/voices", query))

    def create_voice(self, body: OpenAiVoiceCreateRequest) -> OpenAiVoice:
        """Create voice"""
        return self._client.post(f"/v1/audio/voices", json=body)
