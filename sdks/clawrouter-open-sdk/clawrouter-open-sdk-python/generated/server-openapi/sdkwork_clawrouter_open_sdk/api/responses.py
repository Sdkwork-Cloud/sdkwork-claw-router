...
from ..models import DeleteResult, OpenAiResponse, OpenAiResponseCompactRequest, OpenAiResponseInputItemList, OpenAiResponseInputTokenCount, OpenAiResponseInputTokenCountRequest, OpenAiResponsesRequest
...
class ResponsesApi:
    ...
    def create(self, body: OpenAiResponsesRequest) -> OpenAiResponse:
        """Create response"""
        return self._client.post(f"/v1/responses", json=body)

    def compact(self, body: OpenAiResponseCompactRequest) -> OpenAiResponse:
        """Compact response"""
        return self._client.post(f"/v1/responses/compact", json=body)

    def create_input_token(self, body: OpenAiResponseInputTokenCountRequest) -> OpenAiResponseInputTokenCount:
        """Count response input tokens"""
        return self._client.post(f"/v1/responses/input_tokens", json=body)

    def delete(self, response_id: str) -> DeleteResult:
        """Delete response"""
        return self._client.delete(f"/v1/responses/{serialize_path_parameter(response_id, {'name': 'response_id', 'style': 'simple', 'explode': False})}")

    def retrieve(self, response_id: str, include: Optional[List[str]] = None) -> OpenAiResponse:
        """Retrieve response"""
        query = build_query_string([
            {'name': 'include[]', 'value': include, 'style': 'form', 'explode': True, 'allow_reserved': False},
        ])
        return self._client.get(_append_query_string(f"/v1/responses/{serialize_path_parameter(response_id, {'name': 'response_id', 'style': 'simple', 'explode': False})}", query))

    def cancel(self, response_id: str) -> OpenAiResponse:
        """Cancel response"""
        return self._client.post(f"/v1/responses/{serialize_path_parameter(response_id, {'name': 'response_id', 'style': 'simple', 'explode': False})}/cancel")
    ...
    def list_input_items(self, response_id: str, limit: Optional[int] = None, order: Optional[str] = None, after: Optional[str] = None, before: Optional[str] = None, include: Optional[List[str]] = None) -> OpenAiResponseInputItemList:
        ...
        return self._client.get(_append_query_string(f"/v1/responses/{serialize_path_parameter(response_id, {'name': 'response_id', 'style': 'simple', 'explode': False})}/input_items", query))
