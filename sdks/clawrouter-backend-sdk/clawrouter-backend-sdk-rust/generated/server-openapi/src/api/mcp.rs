...
impl McpApi {
    ...
    /// List MCP servers
    pub async fn servers_list(&self, page: Option<i64>, page_size: Option<i64>, q: Option<&str>, transport: Option<&str>, visibility: Option<&str>, status: Option<&str>, category_id: Option<&str>) -> Result<ServersListResult, SdkworkError> {
        let query = build_query_string(&[
            QueryParameterSpec::new("page", page, "form", true, false, None),
            QueryParameterSpec::new("page_size", page_size, "form", true, false, None),
            QueryParameterSpec::new("q", q, "form", true, false, None),
            QueryParameterSpec::new("transport", transport, "form", true, false, None),
        ...
        let path = append_query_string(backend_path(&"/mcp/servers".to_string()), &query);
    ...
    /// Retrieve MCP server
    pub async fn servers_retrieve(&self, server_id: &str) -> Result<ServersRetrieveResult, SdkworkError> {
    ...
    /// List MCP bindings
    pub async fn servers_bindings_list(&self, server_id: &str) -> Result<ServersBindingsListResult, SdkworkError> {
    ...
    pub async fn servers_revisions_list(&self, server_id: &str) -> Result<ServersRevisionsListResult, SdkworkError> {
    ...
    /// List MCP tools
    pub async fn servers_tools_list(&self, server_id: &str) -> Result<ServersToolsListResult, SdkworkError> {
        let path = backend_path(&format!("/mcp/servers/{}/tools", serialize_path_parameter(server_id, PathParameterSpec::new("serverId", "simple", false))));
        self.client.get(&path, None, None).await
    }
