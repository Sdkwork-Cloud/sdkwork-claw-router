...
class IntegrationApi(private val client: HttpClient) {
    ...
    suspend fun providerSecretsList(providerCode: String? = null, status: String? = null): ProviderSecretsListResult? {
        val query = buildQueryString(listOf(
            QueryParameterSpec("provider_code", providerCode, "form", true, false, null),
            QueryParameterSpec("status", status, "form", true, false, null)
        ))
        val raw = client.get(ApiPaths.appendQueryString(ApiPaths.backendPath("/integration/provider_secrets"), query))
