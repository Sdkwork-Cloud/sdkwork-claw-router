...
import type { AgentDefinitionsListResult, AgentDefinitionsRetrieveResult } from '../types';
...
/** List managed agents */
  async list(params?: AgentsAgentDefinitionsListParams): Promise<AgentDefinitionsListResult> {
    ...
    return this.client.get<AgentDefinitionsListResult>(appendQueryString(backendApiPath(`/agents`), query));
  }

/** Retrieve managed agent */
  async retrieve(agentId: string): Promise<AgentDefinitionsRetrieveResult> {
    return this.client.get<AgentDefinitionsRetrieveResult>(backendApiPath(`/agents/${serializePathParameter(agentId, { name: 'agentId', style: 'simple', explode: false })}`));
  }
}
