...
class OpenPlatformApi:
    ...
    def __init__(self, client: HttpClient):
        self._client = client
        self.accounts = OpenPlatformAccountsApi(client)
        self.manifests = OpenPlatformManifestsApi(client)
        self.providers = OpenPlatformProvidersApi(client)


class OpenPlatformAccountsApi:
    """open_platform open_platform.accounts API client."""

    def __init__(self, client: HttpClient):
        self._client = client
        self.entries = OpenPlatformAccountsEntriesApi(client)
...
class OpenPlatformAccountsEntriesApi:
    ...
    def list(self, account_id: str) -> AccountsEntriesListResult:
...
class OpenPlatformAccountsPayBindingsApi:
    ...
    def list(self, account_id: str) -> AccountsPayBindingsListResult:
...
class OpenPlatformManifestsApi:
    ...
    def list(self, provider: Optional[str] = None, type: Optional[str] = None) -> ManifestsListResult:
        """List open platform manifests"""
        query = build_query_string([
            {'name': 'provider', 'value': provider, 'style': 'form', 'explode': True, 'allow_reserved': False},
            {'name': 'type', 'value': type, 'style': 'form', 'explode': True, 'allow_reserved': False},
        ])
        return self._client.get(_append_query_string(f"/backend/v3/api/open_platform/manifests", query))

class OpenPlatformProvidersApi:
    """open_platform open_platform.providers API client."""

    def __init__(self, client: HttpClient):
        self._client = client


    def list(self, status: Optional[str] = None) -> ProvidersListResult:
        """List open platform providers"""
        query = build_query_string([
            {'name': 'status', 'value': status, 'style': 'form', 'explode': True, 'allow_reserved': False},
        ])
        return self._client.get(_append_query_string(f"/backend/v3/api/open_platform/providers", query))
