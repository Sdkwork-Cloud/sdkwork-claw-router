...
- Public read endpoints for app center, skill center, forum browsing, and course browsing must not require app auth unless the product contract explicitly says the action is user-private or mutating.
...
4. Let feature modules call semantic generated methods such as `client.course.fetchCourses`, `client.app.getApps`, `client.skill.getSkills`, `client.feed.fetchForumFeeds`, or `client.comment.fetchForumComments`.
