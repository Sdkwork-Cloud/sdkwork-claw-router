...
test("admin open platform management is registered in claw-router portal", () => {
  ...
  assert.ok(
    existsSync(new URL("./packages/sdkwork-clawrouter-pc-admin-open-platform/src/index.tsx", import.meta.url)),
...
test("admin open platform wrapper delegates to appbase standard package through backend SDK boundary", () => {
  const packageJson = JSON.parse(readPortalFile("./packages/sdkwork-clawrouter-pc-admin-open-platform/package.json")) as {
    dependencies: Record<string, string>;
  };
  ...
  assert.equal(packageJson.dependencies["sdkwork-clawrouter-pc-commons"], "workspace:*");
