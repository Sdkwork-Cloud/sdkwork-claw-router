...
import { readMediaResourceUrl, toExternalUrlMediaResource } from "./packages/sdkwork-clawrouter-pc-commons/src/media-resource.ts";
...
} from "./packages/sdkwork-clawrouter-pc-admin-skill/src/skillService.ts";
...
test("admin skill page exposes SDK-backed asset and artifact management surface", async () => {
  const source = await import("node:fs/promises").then((fs) =>
    fs.readFile(new URL("./packages/sdkwork-clawrouter-pc-admin-skill/src/index.tsx", import.meta.url), "utf8"),
  );
...
test("admin skill page uses tabbed management sections and page-scoped i18n keys", async () => {
  ...
  const source = await fs.readFile(
    new URL("./packages/sdkwork-clawrouter-pc-admin-skill/src/index.tsx", import.meta.url),
    "utf8",
  );
  const i18nSource = await fs.readFile(
    new URL("./packages/sdkwork-clawrouter-pc-i18n/src/resources/admin/skill.ts", import.meta.url),
    "utf8",
  );
