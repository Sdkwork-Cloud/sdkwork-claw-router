...
This package is a Claw Router admin adapter. The Commerce-owned implementation and product-center service facade live in `@sdkwork/commerce-pc-admin-product`; this package preserves Claw Router route/package imports, re-exports the Commerce public API, and adds host-side publish-system projections.
...
- `src/ProductPublishSystem.ts` owns Claw Router host-side publish projections.
- It maps Commerce readiness into publish stages, gates, actions, and metadata snapshots.
- It does not call raw HTTP, construct SDK clients, issue backend publish commands, or fork Commerce DTOs.
- Publishing commands and persistence remain Commerce-owned.
