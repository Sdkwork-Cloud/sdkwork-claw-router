...
import { CourseLessonForm } from '../forms/CourseLessonForm';
import type { CourseLessonFormValues } from '../forms/CourseLessonForm';
...
export function CourseLessonsPage() {
  ...
  const loadLessons = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await CourseAdminService.fetchLessons(activeCourseId);
      setRecords(extractRecordList(result));
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : t('admin.courses.states.errorTitle', 'Lessons could not be loaded'));
    } finally {
      setIsLoading(false);
    }
  }, [courseId, t]);
  ...
  const handleSave = async (input: CourseLessonFormValues) => {
    const payload = input as unknown as Record<string, unknown>;
    if (editingRecord) {
      const id = String(editingRecord.id ?? '');
      await CourseAdminService.updateLesson(id, payload as never);
    } else {
      await CourseAdminService.createLesson(courseId.trim() || 'default', payload as never);
    }
  ...
  return (
    <>
        ...
        actions={(
          <>
            ...
            >
              ...
              {t('admin.courses.lessons.add', 'Add Lesson')}
            </button>
          </>
        )}
      >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 dark:border-white/5">
                ...
                <th className="px-4 py-2.5 text-left font-medium text-slate-500">{t('admin.courses.table.sectionId', 'Section ID')}</th>
                <th className="px-4 py-2.5 text-left font-medium text-slate-500">{t('admin.courses.table.lessonNo', 'Lesson')}</th>
                ...
                <th className="px-4 py-2.5 text-left font-medium text-slate-500">{t('admin.courses.table.freePreview', 'Preview')}</th>
            ...
            <tbody>
              {filteredRecords.length === 0 ? (
                ...
                <tr key={recordKey(record, index)} className="border-b border-slate-50 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5">
                  ...
                  <td className="max-w-[100px] truncate px-4 py-2.5 text-xs text-slate-400">{formatCell(record.sectionId)}</td>
                  <td className="px-4 py-2.5 font-medium text-slate-700 dark:text-slate-200">{formatCell(record.lessonNo)}</td>
                  ...
                  <td className="px-4 py-2.5"><CourseStatusBadge status={String(record.freePreview ?? '')} /></td>
                  ...
                  <td className="px-4 py-2.5">
                    <CourseTableActions>
                      ...
                      <CourseDeleteConfirm
                        title={t('admin.courses.lessons.deleteTitle', 'Delete Lesson')}
                        description={t('admin.courses.lessons.deleteDesc', 'Are you sure you want to delete this lesson? This action cannot be undone.')}
      ...
      <CourseDrawer
        title={editingRecord
          ? t('admin.courses.lessons.editTitle', 'Edit Lesson')
          : t('admin.courses.lessons.addTitle', 'Add Lesson')}
        isOpen={isDrawerOpen}
      ...
      >
        <CourseLessonForm
          ...
          initialValue={editingRecord ? {
            sectionId: String(editingRecord.sectionId ?? ''),
            lessonNo: String(editingRecord.lessonNo ?? ''),
            title: String(editingRecord.title ?? ''),
            sourceProvider: String(editingRecord.sourceProvider ?? ''),
            sourceReference: String(editingRecord.sourceReference ?? ''),
            freePreview: String(editingRecord.freePreview ?? ''),
