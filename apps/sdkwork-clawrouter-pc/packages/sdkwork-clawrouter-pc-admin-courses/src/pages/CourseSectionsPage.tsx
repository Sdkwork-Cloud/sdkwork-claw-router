...
import type { CourseSectionFormValues } from '../forms/CourseSectionForm';
...
export function CourseSectionsPage() {
  ...
  const loadSections = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await CourseAdminService.fetchSections(activeCourseId);
      setRecords(extractRecordList(result));
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : t('admin.courses.states.errorTitle', 'Sections could not be loaded'));
    } finally {
      setIsLoading(false);
    }
  }, [courseId, t]);
  ...
  const handleSave = async (input: CourseSectionFormValues) => {
    const payload = input as unknown as Record<string, unknown>;
    if (editingRecord) {
      const id = String(editingRecord.id ?? '');
      await CourseAdminService.updateSection(id, payload as never);
    } else {
      await CourseAdminService.createSection(courseId.trim() || 'default', payload as never);
    }
  ...
  const handleDelete = async (record: CourseRecord) => {
    ...
    await CourseAdminService.deleteSection(id);
    await loadSections();
  };
  ...
  return (
    <>
        ...
        actions={(
          <>
            ...
            >
              ...
              {t('admin.courses.sections.add', 'Add Section')}
            </button>
          </>
        )}
      >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 dark:border-white/5">
                ...
                <th className="px-4 py-2.5 text-left font-medium text-slate-500">{t('admin.courses.table.sectionNo', 'Section')}</th>
                <th className="px-4 py-2.5 text-left font-medium text-slate-500">{t('admin.courses.table.title', 'Title')}</th>
                <th className="px-4 py-2.5 text-right font-medium text-slate-500">{t('admin.courses.table.lessonCount', 'Lessons')}</th>
                <th className="px-4 py-2.5 text-right font-medium text-slate-500">{t('admin.courses.table.durationSeconds', 'Duration')}</th>
            ...
            <tbody>
              {filteredRecords.length === 0 ? (
                ...
                <tr key={recordKey(record, index)} className="border-b border-slate-50 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5">
                  ...
                  <td className="px-4 py-2.5 font-medium text-slate-700 dark:text-slate-200">{formatCell(record.sectionNo)}</td>
                  <td className="px-4 py-2.5 text-slate-600 dark:text-slate-300">{formatCell(record.title)}</td>
                  <td className="px-4 py-2.5 text-right tabular-nums text-slate-600 dark:text-slate-300">{formatCell(record.lessonCount)}</td>
                  <td className="px-4 py-2.5 text-right tabular-nums text-slate-600 dark:text-slate-300">{formatCell(record.durationSeconds)}</td>
                  ...
                  <td className="px-4 py-2.5">
                    <CourseTableActions>
                      ...
                      <CourseDeleteConfirm
                        title={t('admin.courses.sections.deleteTitle', 'Delete Section')}
                        description={t('admin.courses.sections.deleteDesc', 'Are you sure you want to delete this section? This action cannot be undone.')}
      ...
      <CourseDrawer
        title={editingRecord
          ? t('admin.courses.sections.editTitle', 'Edit Section')
          : t('admin.courses.sections.addTitle', 'Add Section')}
        isOpen={isDrawerOpen}
      ...
      >
        <CourseSectionForm
          ...
          initialValue={editingRecord ? {
            sectionNo: String(editingRecord.sectionNo ?? ''),
