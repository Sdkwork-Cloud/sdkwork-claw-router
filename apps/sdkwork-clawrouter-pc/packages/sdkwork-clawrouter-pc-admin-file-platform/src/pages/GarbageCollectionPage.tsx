...
import { StorageGarbageCollectionForm } from '../forms/StorageGarbageCollectionForm';
import {
  fetchStorageGarbageCollections,
} from '../storageService';
import {
...
export function GarbageCollectionPage() {
  ...
  const section = STORAGE_SECTION_DEFINITIONS.garbageCollection;
  ...
  const loadRecords = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await fetchStorageGarbageCollections();
      setRecords(data);
    } catch (loadError) {
  ...
  return (
    <>
      ...
      >
        <StorageGarbageCollectionForm onCancel={handleDrawerClose} onSuccess={handleFormSuccess} initialData={editingRecord} />
      </StorageDrawer>
    </>
  );
}
