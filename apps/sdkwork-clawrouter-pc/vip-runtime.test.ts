...
import { VipService } from "./packages/sdkwork-clawrouter-pc-vip/src/vipService.ts";
...
test("VIP purchase page remains a dedicated product module backed by standard membership APIs", () => {
  ...
  assert.equal(existsSync(new URL("./packages/sdkwork-clawrouter-pc-console-memberships/package.json", import.meta.url)), true);
  assert.equal(existsSync(new URL("./packages/sdkwork-clawrouter-pc-admin-memberships/package.json", import.meta.url)), true);
  assert.equal(existsSync(new URL("./packages/sdkwork-clawrouter-pc-vip/package.json", import.meta.url)), true);
  assert.equal(existsSync(new URL("./packages/sdkwork-clawrouter-pc-vip/src/VipView.tsx", import.meta.url)), true);
  assert.equal(existsSync(new URL("./packages/sdkwork-clawrouter-pc-vip/src/vipService.ts", import.meta.url)), true);
  assert.equal(existsSync(new URL("./packages/sdkwork-clawrouter-pc-admin-vip", import.meta.url)), false);

  assert.match(appSource, /const VipView = lazyRoute\(\(\) => import\('sdkwork-clawrouter-pc-vip'\), 'VipView'\)/);
  assert.match(appSource, /<Route path="\/vip" element=\{<VipView \/>\} \/>/);
  assert.match(appSource, /const MembershipsView = lazyRoute\(\(\) => import\('sdkwork-clawrouter-pc-console-memberships'\), 'MembershipsView'\)/);
  assert.match(appSource, /<Route path="memberships" element=\{<MembershipsView \/>\} \/>/);
  assert.match(appSource, /const MembershipsAdmin = lazyRoute(?:<[^>]+>)?\(\(\) => import\('sdkwork-clawrouter-pc-admin-memberships'\), 'MembershipsAdmin'\)/);
  ...
  assert.match(packageJson, /"sdkwork-clawrouter-pc-vip": "workspace:\*"/);
  assert.match(packageJson, /"sdkwork-clawrouter-pc-console-memberships": "workspace:\*"/);
  assert.match(packageJson, /"sdkwork-clawrouter-pc-admin-memberships": "workspace:\*"/);
  assert.match(lockfile, /sdkwork-clawrouter-pc-vip:/);
  assert.match(tsconfig, /"sdkwork-clawrouter-pc-vip"/);
  assert.match(tsconfig, /"sdkwork-clawrouter-pc-console-memberships"/);
  assert.match(tsconfig, /"sdkwork-clawrouter-pc-admin-memberships"/);
  assert.match(routeClassification, /route: \/vip[\s\S]*package: sdkwork-clawrouter-pc-vip[\s\S]*api_surface: app/);
  assert.match(routeClassification, /route: \/console\/memberships[\s\S]*package: sdkwork-clawrouter-pc-console-memberships[\s\S]*api_surface: app/);
...
test("VIP service preserves product purchase APIs while using generated app SDK membership paths", () => {
  const viewSource = readPortalFile("./packages/sdkwork-clawrouter-pc-vip/src/VipView.tsx");
  const serviceSource = readPortalFile("./packages/sdkwork-clawrouter-pc-vip/src/vipService.ts");
  const indexSource = readPortalFile("./packages/sdkwork-clawrouter-pc-vip/src/index.ts");
...
test("VIP page localizes public tab, package, and feature copy", () => {
  const viewSource = readPortalFile("./packages/sdkwork-clawrouter-pc-vip/src/VipView.tsx");
  const serviceSource = readPortalFile("./packages/sdkwork-clawrouter-pc-vip/src/vipService.ts");
  const i18nSource = readPortalFile("./packages/sdkwork-clawrouter-pc-i18n/src/resources/admin-commerce/vip.ts");
...
test("VIP points purchase modal creates in-dialog recharge checkout QR instead of routing away", () => {
  const viewSource = readPortalFile("./packages/sdkwork-clawrouter-pc-vip/src/VipView.tsx");
  const vipPackageJson = readPortalFile("./packages/sdkwork-clawrouter-pc-vip/package.json");
  ...
  assert.match(vipPackageJson, /"sdkwork-clawrouter-pc-console-recharge": "workspace:\*"/);
  assert.match(vipPackageJson, /"sdkwork-clawrouter-pc-console-checkout": "workspace:\*"/);
});
...
test("VIP points purchase modal uses current user avatar with localized fallback identity copy", () => {
  ...
  assert.match(vipPackageJson, /"sdkwork-clawrouter-pc-console-user": "workspace:\*"/);
...
test("VIP points purchase product design treats credits as the primary product and money as secondary context", () => {
  const rechargeViewSource = readPortalFile("./packages/sdkwork-clawrouter-pc-console-recharge/src/RechargeView.tsx");
  const viewSource = readPortalFile("./packages/sdkwork-clawrouter-pc-vip/src/VipView.tsx");
  const i18nSource = readPortalFile("./packages/sdkwork-clawrouter-pc-i18n/src/resources/admin-commerce/vip.ts");
...
test("membership console package owns direct membership purchase and a professional entitlement dashboard", () => {
  const source = readPortalFile("./packages/sdkwork-clawrouter-pc-console-memberships/src/MembershipsView.tsx");
  const serviceSource = readPortalFile("./packages/sdkwork-clawrouter-pc-console-memberships/src/membershipService.ts");
  const packageJson = readPortalFile("./packages/sdkwork-clawrouter-pc-console-memberships/package.json");
  ...
  assert.doesNotMatch(source, /sdkwork-clawrouter-pc-vip/);
