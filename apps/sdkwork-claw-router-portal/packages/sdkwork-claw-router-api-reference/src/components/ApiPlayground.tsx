...
export function ApiPlayground({ endpoint, onClose }: ApiPlaygroundProps) {
  ...
  const hiddenHeaders = [
    ...
    { key: 'User-Agent', value: 'ClawRouter/1.0.0' },
  ];
            ...
            <div className="space-y-8">
              ...
              <div className="mt-6">
                ...
                >
                  ...
                  <span>{hiddenHeaders.length} hidden</span>
                </button>
              ...
              {response && (
                <div className="flex items-center gap-4">
                  ...
                  <button
                    ...
                    style={{ marginBottom: '-9px' }}
                    title="Cookies (Not available in browser)"
                  >
                    Cookies
                  </button>
                  <button
                    ...
                    style={{ marginBottom: '-9px' }}
                  >
                    Headers <span className={`ml-1 text-[10px] px-1.5 py-0.5 rounded-full ${activeResponseTab === 'headers' ? 'bg-[#ff6c37]/10 text-[#ff6c37]' : 'bg-slate-100 dark:bg-white/10 text-slate-500'}`}>{response.headers?.length || 0}</span>
                  </button>
                  <button
                    type="button"
                    className="text-[13px] font-semibold pb-2 border-b-2 border-transparent text-slate-400 cursor-not-allowed"
                    style={{ marginBottom: '-9px' }}
                    title="Test Results (Not configured)"
                  >
                    Test Results
                  </button>
                </div>
