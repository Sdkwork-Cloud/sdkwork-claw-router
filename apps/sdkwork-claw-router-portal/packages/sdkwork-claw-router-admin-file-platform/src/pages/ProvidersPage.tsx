...
import { StorageProviderForm } from '../forms/StorageProviderForm';
import {
...
export function ProvidersPage() {
  ...
  const section = STORAGE_SECTION_DEFINITIONS.providers;
  ...
  const handleDelete = async (record: StorageRecord) => {
    const providerId = record.id || record.providerId;
    if (!providerId) return;
    const confirmed = await confirmDelete(t('admin.filePlatform.storage.deleteConfirm', 'Delete this provider?'));
    if (!confirmed) return;
    setDeletingId(providerId);
    try {
      await deleteStorageProvider(providerId);
      await loadRecords();
    } catch (err) {
      setError(err instanceof Error ? err.message : t('admin.filePlatform.storage.deleteError', 'Failed to delete provider'));
    } finally {
      setDeletingId(null);
    }
  };
  ...
  return (
    <>
                ...
                {pagedRecords.map((record, index) => (
                  <tr className="transition-colors hover:bg-slate-50 dark:hover:bg-white/5" key={`${section.id}-${(page - 1) * pageSize + index}`}>
                    {section.columns.map((column) => (
                      ...
                      >
                        {column.key === 'status' || column.key === 'health' ? (
                          <StorageStatusBadge status={record[column.key] || 'unknown'} />
                        ) : (
                          record[column.key] || '-'
                        )}
                      </td>
                    ...
                    <td className="px-4 py-3">
                      <StorageTableActions>
                        ...
                        <StorageIconActionButton label={t('common.actions.delete', 'Delete')} icon={deletingId === (record.id || record.providerId) ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />} tone="danger" onClick={() => handleDelete(record)} disabled={deletingId === (record.id || record.providerId)} />
                      </StorageTableActions>
                    </td>
                  </tr>
                ))}
      ...
      >
        <StorageProviderForm onCancel={handleDrawerClose} onSuccess={handleFormSuccess} initialData={editingRecord} />
      </StorageDrawer>
    </>
  );
}
