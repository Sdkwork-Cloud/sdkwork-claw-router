...
export function BucketsPage() {
  ...
  const section = STORAGE_SECTION_DEFINITIONS.buckets;
  ...
  const pageSize = 20;
  const { confirmDelete, deleteDialog } = useStorageConfirmDelete();

  const loadRecords = useCallback(async () => {
    setIsLoading(true);
    setError(null);
    try {
      const data = await fetchStorageBuckets();
      setRecords(data);
    } catch (loadError) {
  ...
  const handleDelete = async (record: StorageRecord) => {
    const bucketId = record.id || record.bucketId;
    if (!bucketId) return;
    const confirmed = await confirmDelete(t('admin.filePlatform.storage.deleteConfirm', 'Delete this bucket?'));
    if (!confirmed) return;
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('admin.filePlatform.storage.deleteError', 'Failed to delete bucket'));
    } finally {
      setDeletingId(null);
    }
  };
  ...
  const renderCell = (record: StorageRecord, columnKey: string) => {
    ...
    if (columnKey === 'encryption') {
      return <StorageStatusBadge status={value} />;
    }
    if (columnKey === 'storageClass') {
      return <span className="inline-flex rounded bg-slate-100 px-2 py-0.5 text-xs font-medium dark:bg-white/10">{value}</span>;
    }
    return value;
  };
  ...
  return (
    <>
                  ...
                  <tr className="transition-colors hover:bg-slate-50 dark:hover:bg-white/5" key={`${section.id}-${(page - 1) * pageSize + index}`}>
                    ...
                    <td className="px-4 py-3">
                      <StorageTableActions>
                        ...
                        <StorageIconActionButton label={t('common.actions.delete', 'Delete')} icon={deletingId === (record.id || record.bucketId) ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />} tone="danger" onClick={() => handleDelete(record)} disabled={deletingId === (record.id || record.bucketId)} />
                      </StorageTableActions>
                    </td>
                  </tr>
                ))}
      ...
      >
        <StorageBucketForm onCancel={handleDrawerClose} onSuccess={handleFormSuccess} initialData={editingRecord} />
      </StorageDrawer>
    </>
  );
}
