...
import { QuotaPolicyForm } from '../forms/QuotaPolicyForm';
import {
  fetchStorageQuotas,
} from '../storageService';
import {
...
export function QuotasPage() {
  ...
  const section = STORAGE_SECTION_DEFINITIONS.quotas;
  ...
  const loadRecords = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await fetchStorageQuotas();
      setRecords(data);
    } catch (loadError) {
  ...
  return (
    <>
      ...
      >
        <QuotaPolicyForm onCancel={handleDrawerClose} onSuccess={handleFormSuccess} initialData={editingRecord} />
      </StorageDrawer>
    </>
  );
}
