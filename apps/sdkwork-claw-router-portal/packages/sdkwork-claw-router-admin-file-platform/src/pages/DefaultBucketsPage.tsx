...
import { DefaultBucketPolicyForm } from '../forms/DefaultBucketPolicyForm';
import {
  fetchStorageDefaultBuckets,
} from '../storageService';
import {
...
export function DefaultBucketsPage() {
  const { t } = useTranslation();
  const section = STORAGE_SECTION_DEFINITIONS.defaultBuckets;
  ...
  const loadRecords = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await fetchStorageDefaultBuckets();
      setRecords(data);
    } catch (loadError) {
  ...
  return (
    <>
      ...
      >
        <DefaultBucketPolicyForm onCancel={handleDrawerClose} onSuccess={handleFormSuccess} initialData={editingRecord} />
      </StorageDrawer>
    </>
  );
}
