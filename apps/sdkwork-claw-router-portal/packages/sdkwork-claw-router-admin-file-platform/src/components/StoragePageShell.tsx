...
export function StoragePageShell({
  ...
  if (isLoading) {
    return (
      <BusinessStatePanel
        kind="loading"
        title={t('admin.filePlatform.storage.loading', 'Loading storage records...')}
        className="min-h-48"
      />
    );
  }
  ...
  if (error) {
    return (
      <BusinessStatePanel
        kind="error"
        title={t('admin.filePlatform.storage.error', 'Storage data could not be loaded')}
        description={error}
        onRetry={onRefresh}
        className="min-h-48"
      />
