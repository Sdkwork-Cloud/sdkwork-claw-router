...
export function SettingsView() {
  ...
  const loadSettings = useCallback(async (isActive: () => boolean = () => true) => {
    ...
    try {
      ...
      if (isActive()) {
        setLoadError(getSettingsErrorMessage(error, 'Failed to load console settings.'));
      }
    } finally {
      if (isActive()) {
        setLoading(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveError(getSettingsErrorMessage(error, 'Failed to save settings.'));
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="p-4 lg:p-6 w-full mx-auto space-y-6 animate-in fade-in duration-500 min-h-[calc(100vh-72px)] bg-slate-50 dark:bg-[#121212]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-2 border-b border-slate-200 dark:border-white/5 pb-4 mb-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl lg:text-2xl font-bold text-slate-800 dark:text-white tracking-tight">控制台设置</h1>
        </div>
      </div>
        ...
        <div className="flex-1 bg-white dark:bg-[#252525] border border-slate-200 dark:border-white/5 rounded-2xl shadow-sm min-h-[500px]">
          <AnimatePresence mode="wait">
            {loading ? (
              ...
              >
                <div>
                  <h2 className="text-lg font-bold text-slate-800 dark:text-white mb-1">通用设置</h2>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">管理您的语言、时区及跨区请求路由默认偏好。</p>
                  ...
                  <div className="space-y-6 max-w-2xl">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                       <div>
                         <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">系统首选语言</label>
                         <select
                           value={data.language}
                       ...
                       <div>
                         <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">默认报表时区</label>
                         <select
                           value={data.timezone}
                    ...
                    <div className="pt-4 border-t border-slate-200 dark:border-white/5">
                      <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">全局默认回调 URL配置</label>
                      <p className="text-[13px] text-slate-500 mb-3 leading-relaxed">
                        当开启异步多模态任务生成（如视频生成、大批量图像生成）时，如果未在 API 请求体内置顶 Notify URL，平台将默认采用此地址进行状态回推。
                      </p>
                      <input
                        type="url"
              ...
              >
                <div>
                  <h2 className="text-lg font-bold text-slate-800 dark:text-white mb-1">外观与排版体验</h2>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">控制台作为极度沉浸的数据分析及网关监管中心，极力推荐您在暗色模式下获得最专业的多模态聚合操作体验。</p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 max-w-xl">
                    ...
                    >
                      ...
                      <span className={`text-sm font-medium ${!isDark ? 'text-blue-600' : 'text-slate-600 dark:text-slate-300'}`}>浅色模式 (Light)</span>
                    </button>
                    ...
                    >
                      ...
                      <span className={`text-sm font-medium ${isDark ? 'text-blue-600 dark:text-white' : 'text-slate-600 dark:text-slate-300'}`}>深色控制台模式 (Dark Pro)</span>
                    </button>
                  </div>
                </div>
              </motion.div>
              ...
              >
                <div>
                  <h2 className="text-lg font-bold text-slate-800 dark:text-white mb-1">关键事件通知中心</h2>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">配置当业务数据、账单以及大模型 API 网关状态发生异动时，系统与邮件如何通知您。</p>
                  ...
                  <div className="space-y-6 max-w-2xl">
                    ...
                    <div className="bg-slate-50 dark:bg-[#1e1e1e] border border-slate-200 dark:border-white/5 rounded-xl p-6 shadow-sm space-y-7">
                      ...
                      <div className="flex items-start justify-between gap-6">
                        <div>
                          <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200">月度账单出账提醒</h4>
                          <p className="text-[13px] text-slate-500 mt-1.5 leading-relaxed">每月初系统完成并生成上月所有多模态大模型的并发调用清单与扣费统计后，向您发送详尽邮件提醒。</p>
                        </div>
                        <div className="pt-1">
                          <Toggle
                      ...
                      <div className="flex items-start justify-between gap-6">
                        <div>
                          <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200">可用余额/额度熔断告警</h4>
                          <p className="text-[13px] text-slate-500 mt-1.5 leading-relaxed">当您关联的结算账户预估可用余额不足 $50 或单日额度消耗大于 90% 时，立即发送系统紧急站内信与邮件熔断通知。</p>
                        </div>
                        <div className="pt-1">
                          <Toggle
                      ...
                      <div className="flex items-start justify-between gap-6">
                        <div>
                          <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                            网关监控异常跌落报警
                            <span className="px-2 py-0.5 rounded border border-blue-500/30 bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 text-[10px] uppercase font-bold tracking-wider">Pro级</span>
                          </h4>
                          <p className="text-[13px] text-slate-500 mt-1.5 leading-relaxed">当您的所有下发令牌在中继网关发生大面积无法访问（如大量出现上游 429、500/5xx 状态码）时，启用高防监控告警。</p>
                        </div>
                        <div className="pt-1">
                          <Toggle
