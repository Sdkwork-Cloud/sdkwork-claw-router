...
} from "./packages/sdkwork-claw-router-playground/src/components/chat/ChatMarkdownMessage.tsx";
...
} from "./packages/sdkwork-claw-router-playground/src/components/chat/chatLocalStore.ts";
import { resolveReferenceImageCapability } from "./packages/sdkwork-claw-router-playground/src/referenceImageCapability.ts";
import {
...
} from "./packages/sdkwork-claw-router-playground/src/videoReferenceCapability.ts";
import { readRuntimeTextDelta } from "./packages/sdkwork-claw-router-commons/src/runtime.ts";
...
test("chat playground persists unfinished runtime streams and resumes them after refresh", () => {
  const pageSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/ChatPage.tsx");
  const serviceSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/chatService.ts");
  const storeSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/chatLocalStore.ts");
  const typesSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/chatTypes.ts");
  const operationsSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/appRuntimeApiOperations.ts");
...
test("playground chat delegates API key selection to backend runtime policy", () => {
  const pageSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/ChatPage.tsx");
  const inputSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/SimpleChatInput.tsx");
  const typeSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/chatTypes.ts");
  const serviceSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/chatService.ts");
...
test("simple chat input keeps the selected model full name readable after selection", () => {
  const inputSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/SimpleChatInput.tsx");
  const pickerSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/PlaygroundModelPicker.tsx");
...
test("chat playground never submits catalog-only models without runtime routes", () => {
  const pageSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/ChatPage.tsx");
  const inputSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/SimpleChatInput.tsx");
  const selectorSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/chatModelSelection.ts");
...
test("user chat messages use a muted modern send bubble without an avatar", () => {
  ...
  const bubbleSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/ChatMessageBubble.tsx");
  const markdownSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/ChatMarkdownMessage.tsx");
...
test("chat messages use a sanitized GFM markdown response surface", () => {
  const bubbleSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/ChatMessageBubble.tsx");
  const markdownSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/ChatMarkdownMessage.tsx");
  const codeBlockSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/ChatCodeBlock.tsx");
  const packageSource = readPortalFile("./packages/sdkwork-claw-router-playground/package.json");
...
test("playground agent history output uses the same markdown response surface as chat messages", () => {
  const historySource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/ChatHistoryItem.tsx");
...
test("chat message list avoids duplicate error banners once the failed bubble contains the same error", () => {
  const listSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/chat/ChatMessageList.tsx");
...
test("playground SSE runtime errors are translated before display", () => {
  const playgroundSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/pages/Playground.tsx");
  const playgroundCoreI18nSource = readPortalFile("./packages/sdkwork-claw-router-i18n/src/resources/playground/core.ts");
  const playgroundChatI18nSource = readPortalFile("./packages/sdkwork-claw-router-i18n/src/resources/playground/chat.ts");
...
test("image generation reference images sit above the prompt and follow model capacity", () => {
  ...
  const assetMessages = readPortalFile("./packages/sdkwork-claw-router-i18n/src/resources/playground/assets.ts");
...
test("video generation reference assets sit above the prompt and follow model mode capacity", () => {
  ...
  const serviceSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/playgroundGenerationService.ts");
  const videoPopupSource = readPortalFile("./packages/sdkwork-claw-router-playground/src/components/VideoGenerationModePopup.tsx");
  const assetMessages = readPortalFile("./packages/sdkwork-claw-router-i18n/src/resources/playground/assets.ts");
...
test("runtime usage reader follows gateway and provider event envelopes for generated assets", () => {
  const runtimeSource = readPortalFile("./packages/sdkwork-claw-router-commons/src/runtime.ts");
...
test("playground package typecheck includes portal shims for optional appbase peer modules", () => {
  const tsconfig = JSON.parse(readPortalFile("./packages/sdkwork-claw-router-playground/tsconfig.json"));
