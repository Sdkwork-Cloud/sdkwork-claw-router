...
test("admin catalog products route uses a dedicated Weixin-style product list page", () => {
  const indexSource = readPortalFile("./packages/sdkwork-claw-router-admin-catalog/src/index.tsx");
  const productListSource = readPortalFile("./packages/sdkwork-claw-router-admin-catalog/src/ProductListPage.tsx");
  const serviceSource = readPortalFile("./packages/sdkwork-claw-router-admin-catalog/src/catalogService.ts");
...
test("admin catalog product create flow exposes the two-step publish experience", () => {
  ...
  const indexSource = readPortalFile("./packages/sdkwork-claw-router-admin-catalog/src/index.tsx");
  const createSource = readPortalFile("./packages/sdkwork-claw-router-admin-catalog/src/ProductCreatePage.tsx");
...
test("admin catalog category readers handle generated SDK unwrapped responses", async () => {
  const categoryPage = await import("./packages/sdkwork-claw-router-admin-catalog/src/CategoryManagementPage.tsx");
...
test("admin catalog product create category helpers support multiple deep leaf paths", async () => {
  const createPage = await import("./packages/sdkwork-claw-router-admin-catalog/src/ProductCreatePage.tsx");
...
test("admin catalog product create flow models a complete create draft and SDK submit pipeline", async () => {
  ...
  assert.match(createSource, /import \{[\s\S]*readMediaResource[\s\S]*\} from 'sdkwork-claw-router-commons\/runtime'/);
...
test("admin catalog product list normalizes backend product records for table rendering", async () => {
  const productList = await import("./packages/sdkwork-claw-router-admin-catalog/src/ProductListPage.tsx");
