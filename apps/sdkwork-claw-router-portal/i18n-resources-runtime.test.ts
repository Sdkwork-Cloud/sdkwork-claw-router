...
test("claw router i18n resources merge at runtime with aligned admin site navigation keys", async () => {
  const { mergeI18nBundles } = await import("./packages/sdkwork-claw-router-i18n/src/resources/merge.ts");
  const { adminCoreNavigationMessages } = await import("./packages/sdkwork-claw-router-i18n/src/resources/admin/core-navigation.ts");
  const { adminGroupUserMessages } = await import("./packages/sdkwork-claw-router-i18n/src/resources/admin/group-user.ts");
  const { adminModelMessages } = await import("./packages/sdkwork-claw-router-i18n/src/resources/admin/model.ts");
  const { sharedCommonMessages } = await import("./packages/sdkwork-claw-router-i18n/src/resources/shared/common.ts");
