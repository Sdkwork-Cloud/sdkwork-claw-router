...
} from "./packages/sdkwork-claw-router-admin-marketing/src/marketingService.ts";
...
test("admin marketing package owns standardized promotion lifecycle and referral analytics", () => {
  ...
  const legacyMarketingFormExists = existsSync(new URL(
    "./packages/sdkwork-claw-router-admin-marketing/src/marketingForm.ts",
    import.meta.url,
  ));
...
test("admin marketing package path governance keeps recharge and payment capabilities in their business owners", () => {
  ...
  assert.match(routeClassification, /route: \/admin\/memberships\/recharge-packages[\s\S]*package: sdkwork-claw-router-admin-memberships/);
