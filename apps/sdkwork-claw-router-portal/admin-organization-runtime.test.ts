...
test("admin organization route and package are wired into the portal", () => {
  ...
  assert.match(appSource, /import\('sdkwork-claw-router-admin-organization'\)/);
  ...
  assert.equal(
    packageJson.dependencies?.["sdkwork-claw-router-admin-organization"],
    "workspace:*",
  );
  assert.ok(
    packageJson.workspaces?.includes("../../../sdkwork-appbase/sdks/sdkwork-appbase-backend-sdk/*-typescript"),
    "portal workspace must include the sibling appbase backend SDK package path",
  );
  assert.match(
    typecheckSource,
    /\.\.\/\.\.\/\.\.\/sdkwork-appbase\/sdks\/sdkwork-appbase-backend-sdk\/sdkwork-appbase-backend-sdk-typescript\/src\/index\.ts/,
  );
  assert.ok(
    existsSync(new URL("packages/sdkwork-claw-router-admin-organization/package.json", portalRoot)),
    "admin organization package must exist",
  );
});

test("admin organization navigation has translated labels", () => {
  const i18nSource = source("packages/sdkwork-claw-router-i18n/src/resources/admin/core-navigation.ts");
...
test("admin organization page translations are registered", () => {
  const resourceIndex = source("packages/sdkwork-claw-router-i18n/src/resources/index.ts");
  const organizationMessages = source("packages/sdkwork-claw-router-i18n/src/resources/admin/organization.ts");
...
test("admin organization service uses only the appbase backend sdk boundary", () => {
  const service = source("packages/sdkwork-claw-router-admin-organization/src/organizationService.ts");
  const packageJson = JSON.parse(
    source("packages/sdkwork-claw-router-commons/package.json"),
  ) as { dependencies?: Record<string, string> };
  const sdkBoundary = source("packages/sdkwork-claw-router-commons/src/sdk-clients.ts");
...
test("admin organization UI exposes department, position and authorization admin workflows", () => {
  const sourceCode = source("packages/sdkwork-claw-router-admin-organization/src/index.tsx");
...
test("admin organization destructive actions show dependency counts and block unsafe deletes", () => {
  const sourceCode = source("packages/sdkwork-claw-router-admin-organization/src/index.tsx");
  const confirmDialog = source("packages/sdkwork-claw-router-commons/src/components/ConfirmDialog.tsx");
...
test("admin organization service calls appbase backend sdk read and write methods", async () => {
  const { clearStoredAppSessionToken } = await import("./packages/sdkwork-claw-router-commons/src/app-session-token.ts");
  const { resetClawRouterSdkClients } = await import("./packages/sdkwork-claw-router-commons/src/sdk-clients.ts");
  const { OrganizationService } = await import(
    "./packages/sdkwork-claw-router-admin-organization/src/organizationService.ts"
  );
