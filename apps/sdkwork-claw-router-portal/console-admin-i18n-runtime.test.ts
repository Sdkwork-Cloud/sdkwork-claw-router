...
function collectSourceFiles(): string[] {
  ...
  const sharedConsoleAdminFiles = [
    path.join(PACKAGES_ROOT, "sdkwork-claw-router-commons", "src", "components", "Navbar.tsx"),
  ];
...
function collectI18nResourceTranslations(language: "en" | "zh"): Map<string, string> {
  const i18nFile = path.join(PACKAGES_ROOT, "sdkwork-claw-router-i18n", "src", "index.ts");
