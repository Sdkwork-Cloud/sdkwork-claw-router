...
test("admin course center is an independent admin module with header and routes", () => {
  ...
  assert.match(appSource, /import\('sdkwork-claw-router-admin-courses'\)/);
  assert.match(appSource, /const CourseAdmin = lazyRoute<AdminSectionRouteProps>\(\(\) => import\('sdkwork-claw-router-admin-courses'\), 'CourseAdmin'\)/);
  ...
  assert.match(packageJson, /"sdkwork-claw-router-admin-courses": "workspace:\*"/);
...
test("admin course package uses generated backend SDK and owns only course management", () => {
  ...
  const splitPageSource = [
    ...
    "./packages/sdkwork-claw-router-admin-courses/src/pages/CourseCommentsPage.tsx",
    "./packages/sdkwork-claw-router-admin-courses/src/pages/CourseDashboardPage.tsx",
    "./packages/sdkwork-claw-router-admin-courses/src/pages/CourseEngagementPage.tsx",
    "./packages/sdkwork-claw-router-admin-courses/src/pages/CourseLessonsPage.tsx",
    "./packages/sdkwork-claw-router-admin-courses/src/pages/CourseRelationsPage.tsx",
    "./packages/sdkwork-claw-router-admin-courses/src/pages/CourseSectionsPage.tsx",
  ].map(readPortalFile).join("\n");
  const packageSource = `${pageSource}\n${splitPageSource}`;
  const serviceSource = readPortalFile("./packages/sdkwork-claw-router-admin-courses/src/courseAdminService.ts");

  assert.match(packageJson, /"name": "sdkwork-claw-router-admin-courses"/);
