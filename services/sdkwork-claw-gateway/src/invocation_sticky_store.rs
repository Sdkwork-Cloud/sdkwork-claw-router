...
#[derive(Clone)]
pub(crate) enum InvocationStickyObjectRouteStore {
...
impl std::fmt::Debug for InvocationStickyObjectRouteStore {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Sqlite(_) => formatter.write_str("InvocationStickyObjectRouteStore::Sqlite"),
            Self::Postgres(_) => formatter.write_str("InvocationStickyObjectRouteStore::Postgres"),
        }
    }
}
    ...
    fn find_binding<'a>(
        ...
        Box::pin(async move {
            ...
            let binding = match self {
                ...
                .map(sticky_binding_from_sqlite_row),
...
fn sticky_binding_from_sqlite_row(row: sqlx::sqlite::SqliteRow) -> StickyObjectRouteBinding {
...
fn sticky_binding_from_postgres_row(row: sqlx::postgres::PgRow) -> StickyObjectRouteBinding {
    StickyObjectRouteBinding {
        tenant_id: row.get("tenant_id"),
        organization_id: row.get("organization_id"),
        object_type: row.get("object_type"),
