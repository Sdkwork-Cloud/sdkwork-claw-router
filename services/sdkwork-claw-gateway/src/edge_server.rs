    ...
    let Some(root) = &state.config.portal_tool_api_sdk_archive_root else {
        return json_response(
            ...
            json!({
                ...
                "error": "SDK archive generation is not available in the Rust edge server. Configure PORTAL_TOOL_API_SDK_ARCHIVE_ROOT to serve prebuilt SDK ZIP archives produced by the release pipeline.",
            }),
        );
    };
...
fn normalize_portal_csp_origin(value: &str) -> Result<String, String> {
    ...
    {
        return Err(
            "PORTAL_CSP_CONNECT_SRC entries must be HTTP/HTTPS origins without directives"
                .to_owned(),
        );
    }
    let uri = trimmed.parse::<Uri>().map_err(|error| {
        format!("PORTAL_CSP_CONNECT_SRC entry must be an HTTP/HTTPS origin: {error}")
    })?;
    if !matches!(uri.scheme_str(), Some("http" | "https")) || uri.authority().is_none() {
        return Err("PORTAL_CSP_CONNECT_SRC entries must be HTTP/HTTPS origins".to_owned());
    }
    if uri.path() != "/" || uri.query().is_some() {
        return Err(
            "PORTAL_CSP_CONNECT_SRC entries must be origins without path or query".to_owned(),
        );
    }
    Ok(trimmed.to_owned())
}
