...
async fn main() -> anyhow::Result<()> {
    ...
    .map_err(anyhow::Error::msg)?;
    sdkwork_claw_gateway::serve_with_runtime_config(config.bind_addr(), runtime_toml.as_ref()).await
}

pub fn build_edge_server_config(
    runtime_toml: Option<&sdkwork_claw_config::RuntimeTomlConfig>,
) -> anyhow::Result<sdkwork_claw_gateway::EdgeServerConfig> {
    ...
    let tool_api_max_body_bytes = config_u64_or_default(
        "PORTAL_TOOL_API_MAX_BODY_BYTES",
    ...
    edge_config = edge_config
        .with_portal_tool_api_max_body_bytes(tool_api_max_body_bytes.try_into().map_err(|_| {
            anyhow::anyhow!(
                "PORTAL_TOOL_API_MAX_BODY_BYTES must fit in the current platform pointer size"
            )
        })?)
        .map_err(anyhow::Error::msg)?;
    let tool_api_rate_limit_requests = config_u32_or_default(
        "PORTAL_TOOL_API_RATE_LIMIT_REQUESTS",
        runtime_toml.and_then(|config| config.portal.tools.rate_limit_requests),
        120,
    )?;
    let tool_api_rate_limit_window_seconds = config_u64_or_default(
    ...
    let generator_api_key_file = config_optional(
    ...
    let generator_api_key = match env_optional("PORTAL_TOOL_API_SDK_GENERATOR_API_KEY") {
...
fn edge_server_enabled(runtime_toml: Option<&sdkwork_claw_config::RuntimeTomlConfig>) -> bool {
    env_truthy("SDKWORK_CLAW_EDGE_SERVER")
        || runtime_toml
...
fn cors_allowed_origins_from_env_or_toml(
    ...
    if let Some(value) = env_optional(CORS_ALLOWED_ORIGINS) {
        return Ok(value
...
fn portal_csp_frame_src_from_env_or_toml(
    ...
    const CSP_FRAME_SRC: &str = "PORTAL_SECURITY_CSP_FRAME_SRC";
    if let Some(value) = env_optional(CSP_FRAME_SRC) {
        return Ok(value
            .split(|character: char| character == ',' || character.is_whitespace())
            .map(str::trim)
            .filter(|value| !value.is_empty())
