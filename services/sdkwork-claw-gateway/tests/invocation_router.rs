...
fn catalog_with_encoded_image_model_and_hashed_api_key(key_hash: String) -> InMemoryPricingCatalog {
...
async fn invocation_router_routes_extended_model_resource_and_percent_encodes_query_model() {
    ...
    let key_hash = hasher.hash_secret("sk-live-secret").unwrap();
    let dispatcher = Arc::new(CapturingDispatcher::default());
    let router = sdkwork_claw_gateway::invocation_router_with_full_pipeline(
    ...
    assert_eq!(
        Some("model=openrouter%2Fgpt-4o-mini%2Blatest"),
        provider_request.query.as_deref()
    );
    assert_eq!(
        Some(
            "http://provider-proxy.internal/openrouter/v1/images/generations?model=openrouter%2Fgpt-4o-mini%2Blatest"
        ),
        provider_request.url.as_deref()
    );
    let InvocationBody::Json(provider_body) = &provider_request.body else {
...
async fn invocation_router_records_metered_usage_after_settlement() {
    ...
    assert_eq!("req-invocation-router-usage", commands[0].request_id);
...
async fn invocation_router_records_trace_for_free_models_resource() {
    ...
    assert_eq!("req-invocation-router-models-trace", trace.request_id);
...
async fn invocation_router_provider_native_adapter_uses_standard_chain_and_records_usage() {
    ...
    let usage_recorder = Arc::new(RecordingUsageRecorder::default());
    let router =
        sdkwork_claw_gateway::invocation_router_with_full_pipeline_and_provider_adapter_config(
            Arc::new(catalog_with_hashed_api_key(key_hash)),
            hasher,
            Arc::new(sdkwork_claw_gateway::InvocationHttpDispatcher::new()),
            Some(secret_resolver()),
            None,
            Some(usage_recorder.clone()),
    ...
    assert_eq!(
        Some("req-provider-native-adapter"),
        calls[0]
            .body
    ...
    assert!(commands.iter().all(|command| {
        command.request_id == "req-provider-native-adapter"
...
async fn invocation_router_fallback_preserves_explicit_provider_native_routes() {
    ...
    let invocation_router = sdkwork_claw_gateway::invocation_router_with_full_pipeline(
        Arc::new(catalog_with_hashed_api_key(key_hash)),
        hasher,
        dispatcher.clone(),
        Some(secret_resolver()),
...
async fn invocation_http_dispatcher_forwards_provider_header_auth_after_sanitizing_inbound_keys() {
    ...
    let response = sdkwork_claw_gateway::InvocationHttpDispatcher::new()
...
async fn invocation_http_dispatcher_enforces_account_timeout() {
    ...
    let error = sdkwork_claw_gateway::InvocationHttpDispatcher::new()
        .dispatch(&invocation, &account)
        .await
        .unwrap_err();
