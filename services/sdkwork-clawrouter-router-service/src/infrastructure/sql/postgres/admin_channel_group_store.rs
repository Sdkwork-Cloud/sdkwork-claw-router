...
impl PostgresAdminChannelGroupStore {
    pub fn new(pool: PgPool) -> Self {
        Self { pool }
    }
}

impl AdminChannelGroupStore for PostgresAdminChannelGroupStore {
...
async fn replace_channel_bindings(
    ...
    for (index, item) in command.items.iter().enumerate() {
        ...
        sqlx::query(
            ...
            VALUES
                ($1, $2, $3, 1, $4, $5::timestamptz, $6::timestamptz, 0, $7, $8, $9, $10, $11::jsonb, $12)
        ...
        .execute(&mut **tx)
        .await
        .map_err(|error| store_error("failed to upsert channel group channel binding", error))?;
    }
...
async fn insert_channel_group(
    ...
    sqlx::query_scalar(
        ...
        VALUES
            ($1, $2, $3, 1, $4, $5::timestamptz, $6::timestamptz, 0, $7, $8, '', $9, $10, 1, $11, $12, $13::numeric, $14, $15::numeric, $16, $17, '{}'::jsonb, '{}'::jsonb, $18)
        RETURNING id
        "#,
    )
...
async fn update_channel_group(
    ...
    let result = sqlx::query(
        ...
        SET group_name = COALESCE($1, group_name),
            ...
            price_reference_mode = COALESCE($4, price_reference_mode),
            rate_multiplier = COALESCE($5::numeric, rate_multiplier),
            official_price_multiplier = COALESCE($6::numeric, official_price_multiplier),
            group_type = COALESCE($7, group_type),
            capacity_limit = COALESCE($8, capacity_limit),
...
async fn sync_channel_group_relationship_status(
    ...
    if status == 0 {
        sqlx::query(
            ...
            UPDATE ai_channel_group_member
            SET status = 0,
                metadata = jsonb_set(COALESCE(metadata, '{}'::jsonb), '{disabledByParent}', 'true'::jsonb, true),
                updated_at = $1::timestamptz,
                version = COALESCE(version, 0) + 1
            WHERE tenant_id = $2
              AND organization_id = $3
              AND channel_group_id = $4
              AND status = 1
              AND deleted_at IS NULL
            "#,
        )
        ...
        sqlx::query(
            ...
            UPDATE ai_channel_group_member
            SET status = 1,
                metadata = COALESCE(metadata, '{}'::jsonb) - 'disabledByParent',
                updated_at = $1::timestamptz,
                version = COALESCE(version, 0) + 1
            WHERE tenant_id = $2
              AND organization_id = $3
              ...
              AND status = 0
              AND deleted_at IS NULL
              AND COALESCE(metadata ->> 'disabledByParent', 'false') = 'true'
            "#,
        )
        .bind(requested_at)
        .bind(tenant_id)
...
async fn upsert_pricing_plan_binding(
    ...
    sqlx::query(
        ...
        VALUES
            ($1, $2, $3, 1, 1, $4::timestamptz, $5::timestamptz, 0, $6, $7, $8, $9, $10, 1, $11::numeric, 1, $12::timestamptz, $13)
        "#,
    )
    .bind(binding_uuid)
    .bind(tenant_id)
...
async fn insert_config_snapshot(
    ...
    sqlx::query(
        ...
        VALUES
            ($1, $2, $3, $4, $5, 1, $6, $7, $8, 'ai_channel_group', $9::jsonb, $10::jsonb, $11, $12::timestamptz, $13, $14)
        "#,
    )
    .bind(snapshot_uuid)
    .bind(tenant_id)
...
fn channel_group_select_sql(predicate: &str) -> String {
    format!(
        ...
        SELECT
            ...
            COALESCE(g.rate_multiplier, 1)::text AS rate_multiplier,
            COALESCE(g.official_price_multiplier, 1)::text AS official_price_multiplier,
            g.group_type,
...
fn channel_binding_select_sql() -> &'static str {
    r#"
        SELECT
            ...
            COALESCE(
                (
                    SELECT jsonb_agg(selected.code ORDER BY selected.code)
                    FROM (
                ...
                '[]'::jsonb
            )::text AS resource_codes_json,
            COALESCE(
                (
                    SELECT jsonb_agg(selected.code ORDER BY selected.code)
                    FROM (
                        SELECT DISTINCT COALESCE(NULLIF(r.api_code, ''), NULLIF(gr.resource_code, ''), gr.resource_group_code) AS code
                        FROM ai_channel_group_resource gr
                        LEFT JOIN ai_resource r
                          ON r.resource_code = gr.resource_code
                         AND r.tenant_id = gr.tenant_id
...
fn item_from_row(row: sqlx::postgres::PgRow) -> DomainResult<AdminChannelGroupItem> {
...
fn group_type_cell(row: &sqlx::postgres::PgRow) -> DomainResult<String> {
