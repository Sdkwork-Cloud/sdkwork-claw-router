...
async fn resolve_auth_settings_scope(
    ...
    let tenant_row = match tenant_code {
        Some(code) if !code.trim().is_empty() => sqlx::query(
            "SELECT id FROM iam_tenant WHERE code = $1 AND status = 'active' AND deleted_at IS NULL ORDER BY id LIMIT 1",
        )
        .bind(code.trim())
        .fetch_optional(pool)
        .await,
    ...
    .map_err(|error| store_error("failed to load auth settings IAM tenant", error))?;
    ...
    let organization_row = match organization_code {
        Some(code) if !code.trim().is_empty() => sqlx::query(
            "SELECT id FROM iam_organization WHERE tenant_id = $1 AND code = $2 AND status = 'active' AND deleted_at IS NULL ORDER BY id LIMIT 1",
        )
        ...
        .await,
        _ => sqlx::query(
            "SELECT id FROM iam_organization WHERE tenant_id = $1 AND status = 'active' AND deleted_at IS NULL ORDER BY id LIMIT 1",
        )
        .bind(tenant_id.to_string())
        .fetch_optional(pool)
        .await,
    }
    .map_err(|error| store_error("failed to load auth settings IAM organization", error))?;
