...
fn row_to_prompt(row: &sqlx::postgres::PgRow) -> DomainResult<AdminPromptItem> {
    Ok(AdminPromptItem {
...
fn row_to_prompt_version(row: &sqlx::postgres::PgRow) -> DomainResult<AdminPromptVersionItem> {
...
fn row_to_prompt_binding(row: &sqlx::postgres::PgRow) -> DomainResult<AdminPromptBindingItem> {
...
fn tags_cell(row: &sqlx::postgres::PgRow, column: &str) -> DomainResult<Vec<String>> {
...
fn optional_integer_cell(row: &sqlx::postgres::PgRow, column: &str) -> DomainResult<Option<i64>> {
    if let Ok(value) = row.try_get::<Option<i64>, _>(column) {
        return Ok(value);
    }
    if let Ok(value) = row.try_get::<i64, _>(column) {
