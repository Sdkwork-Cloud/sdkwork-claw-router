use sdkwork_clawrouter_app_iam_directory_repository_sqlx::{
    ...
    AppIamDirectorySubject,
    PostgresAppIamDirectoryReadStore as RepositoryPostgresAppIamDirectoryReadStore,
...
impl AppIamDirectoryReadStore for PostgresAppIamDirectoryReadStore {
    ...
    fn list_department_assignments<'a>(
        ...
        Box::pin(async move {
            RepositoryAppIamDirectoryReadStore::list_department_assignments(&self.0, subject, query)
                .await
                .map_err(|error| DomainError::new(error.to_string()))
        })
    }
    ...
    fn list_position_assignments<'a>(
        ...
        Box::pin(async move {
            RepositoryAppIamDirectoryReadStore::list_position_assignments(&self.0, subject, query)
                .await
                .map_err(|error| DomainError::new(error.to_string()))
        })
    }
