...
WHERE p.tenant_id = ?1
  ...
  AND p.status = 1
ORDER BY pr.profile_version DESC, pr.id DESC
LIMIT 1
"#;
...
SELECT
    ...
    match_expression AS match_expression,
...
VALUES
    (?1, ?2, ?3, 1, 1, ?4, ?4, 0, '{}', ?5, 'Console Routing Strategy', 1, ?6, 1, ?7, 'USD')
...
VALUES
    (?1, ?2, ?3, 1, 1, ?4, ?4, 0, '{}', ?5, ?6, 'Console Routing Strategy', 2, 100, ?7, ?4, ?8)
"#;
...
VALUES
    (?1, ?2, ?3, 1, 1, ?4, ?4, 0, '{}', ?5, ?6, ?7, ?8, ?9)
"#;

#[derive(Debug, Clone)]
pub struct SqliteAppRoutingStrategyStore {
    pool: SqlitePool,
}

impl SqliteAppRoutingStrategyStore {
    pub fn new(pool: SqlitePool) -> Self {
        Self { pool }
    }
}

impl AppRoutingStrategyStore for SqliteAppRoutingStrategyStore {
...
fn row_to_mapping_rule(row: sqlx::sqlite::SqliteRow) -> DomainResult<AppRoutingMappingRule> {
    let source_model =
...
fn required_string_cell(row: &sqlx::sqlite::SqliteRow, column: &str) -> DomainResult<String> {
