...
#[derive(Debug, Clone)]
pub struct SqliteAdminCatalogStore {
    pool: SqlitePool,
}

impl SqliteAdminCatalogStore {
    pub fn new(pool: SqlitePool) -> Self {
        Self { pool }
    }
}

impl AdminCatalogStore for SqliteAdminCatalogStore {
    ...
    if is_update {
        let result = sqlx::query(
            ...
            SET category_no = ?1,
                ...
                updated_at = ?6
            WHERE id = ?7
              AND tenant_id = CAST(?8 AS TEXT)
              AND (organization_id = CAST(?9 AS TEXT) OR organization_id IS NULL)
            "#,
        )
        ...
        sqlx::query(
            ...
            VALUES
                (?1, CAST(?2 AS TEXT), CAST(?3 AS TEXT), ?4, ?5, ?6, ?7, ?8, ?9, ?9)
...
async fn delete_category(
    ...
    let result = sqlx::query(
        ...
        WHERE id = ?2
          ...
          AND (organization_id = CAST(?4 AS TEXT) OR organization_id IS NULL)
        "#,
    )
    .bind(&command.requested_at)
    .bind(&command.category_id)
        ...
        SELECT
            ...
            (
                ...
                WHERE s.tenant_id = p.tenant_id
                  AND s.spu_id = p.id
                ORDER BY CAST(s.price_amount AS REAL) ASC, s.id ASC
                LIMIT 1
            ) AS default_sku_id,
            (
    ...
    if is_update {
        let result = sqlx::query(
            ...
            WHERE id = ?8
              ...
              AND (organization_id = CAST(?10 AS TEXT) OR organization_id IS NULL)
            "#,
        )
        .bind(&command.spu_no)
        .bind(&command.title)
...
async fn replace_product_categories(
    ...
    for (index, category_id) in command.category_ids.iter().enumerate() {
        ...
        sqlx::query(
            ...
            VALUES
                (?1, CAST(?2 AS TEXT), CAST(?3 AS TEXT), ?4, ?5, ?6, ?7, 'active', ?8, ?8)
    ...
    if is_update {
        let result = sqlx::query(
            ...
            SET sku_no = ?1,
                ...
                updated_at = ?10
            WHERE id = ?11
              AND tenant_id = CAST(?12 AS TEXT)
              AND (organization_id = CAST(?13 AS TEXT) OR organization_id IS NULL)
            "#,
        )
        .bind(&command.sku_no)
        .bind(&command.product_id)
...
async fn delete_sku(pool: &SqlitePool, command: DeleteAdminSkuCommand) -> DomainResult<bool> {
    let result = sqlx::query(
        r#"
        UPDATE commerce_product_sku
        SET sales_status = 'archived',
...
async fn replace_sku_attributes(
    ...
    for attribute in &command.attributes {
        ...
        sqlx::query(
            ...
            VALUES
                (?1, CAST(?2 AS TEXT), CAST(?3 AS TEXT), ?4, ?5, ?6, ?7, ?8, ?8)
...
async fn create_attribute(
    ...
    sqlx::query(
        ...
        VALUES
            (?1, CAST(?2 AS TEXT), CAST(?3 AS TEXT), ?4, ?5, ?6, ?7, 0, ?8, ?8)
...
async fn list_category_attributes(
    ...
    let rows = sqlx::query(
        ...
        WHERE ca.tenant_id = CAST(?1 AS TEXT)
          AND (ca.organization_id = CAST(?2 AS TEXT) OR ca.organization_id IS NULL)
    ...
    if is_update {
        let result = sqlx::query(
            ...
            SET category_id = ?1,
                ...
                updated_at = ?8
            WHERE id = ?9
              AND tenant_id = CAST(?10 AS TEXT)
              AND (organization_id = CAST(?11 AS TEXT) OR organization_id IS NULL)
            "#,
        )
        .bind(&command.category_id)
        ...
        sqlx::query(
            ...
            VALUES
                (?1, CAST(?2 AS TEXT), CAST(?3 AS TEXT), ?4, ?5, ?6, ?7, ?8, ?9, ?10, ?11, ?11)
...
async fn load_product(
    ...
    let row = sqlx::query(
        ...
        SELECT
            ...
            (SELECT s.price_amount FROM commerce_product_sku s WHERE s.tenant_id = p.tenant_id AND s.spu_id = p.id ORDER BY CAST(s.price_amount AS REAL), s.id LIMIT 1) AS min_price_amount,
            (SELECT s.currency_code FROM commerce_product_sku s WHERE s.tenant_id = p.tenant_id AND s.spu_id = p.id ORDER BY CAST(s.price_amount AS REAL), s.id LIMIT 1) AS currency_code
        FROM commerce_product_spu p
        WHERE p.id = ?1
          AND p.tenant_id = CAST(?2 AS TEXT)
          AND (p.organization_id = CAST(?3 AS TEXT) OR p.organization_id IS NULL)
        "#,
    )
    .bind(product_id)
    .bind(subject.tenant_id)
...
async fn load_category_attribute(
    ...
    let row = sqlx::query(
        ...
        JOIN commerce_product_attribute a
         ...
         AND a.id = ca.attribute_id
        WHERE ca.id = ?1
          AND ca.tenant_id = CAST(?2 AS TEXT)
          AND (ca.organization_id = CAST(?3 AS TEXT) OR ca.organization_id IS NULL)
        "#,
    )
    .bind(binding_id)
    .bind(subject.tenant_id)
...
fn product_record_from_row(row: &sqlx::sqlite::SqliteRow) -> DomainResult<AdminCatalogJsonRecord> {
    let mut item = Map::new();
    insert_string(&mut item, "id", string_cell(row, "id")?);
...
async fn sku_attributes(
    ...
    let rows = sqlx::query(
        ...
        WHERE sav.tenant_id = CAST(?1 AS TEXT)
          AND sav.sku_id = ?2
...
fn csv_string_array_cell(row: &sqlx::sqlite::SqliteRow, column: &str) -> DomainResult<Vec<Value>> {
