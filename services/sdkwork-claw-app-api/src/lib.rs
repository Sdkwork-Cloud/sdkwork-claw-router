...
use sdkwork_claw_product::application::{
    ...
    PasswordHasher, PaymentAggregateRuntimeStore, Pbkdf2Sha256PasswordHasher, RuntimeStreamBus,
};
...
use sdkwork_claw_product::infrastructure::sql::postgres::{
    ...
    PostgresAppSessionEventStore, PostgresAppSkillsReadStore, PostgresAppStoreReadStore,
    PostgresAppUserProfileReadStore, PostgresCatalogLoadError, PostgresCourseStore,
...
type AppIamDirectoryStore = Arc<dyn AppIamDirectoryReadStore + Send + Sync>;
...
fn router_with_api_key_management_store_and_database_status(
    ...
    router = match app_iam_directory_read_store {
        Some(read_store) => router.merge(
            sdkwork_claw_product::api::app_iam_directory_router_with_read_store(read_store).layer(
                from_fn_with_state(
                    subject_boundary_config.clone(),
                    sdkwork_claw_http::app_request_subject_boundary,
                ),
...
pub async fn router_with_sqlite_product_catalog(
    ...
    let app_iam_directory_read_store = Arc::new(SqliteAppIamDirectoryReadStore::new(pool.clone()));
    let membership_store = Arc::new(SqliteCommerceMembershipStore::new(pool.clone()));
    ...
    let appbase_wallet_router =
        sdkwork_commerce_http::app_account_wallet_router_with_sqlite_pool(pool.clone());
    let appbase_commerce_router =
    ...
    Ok(router_with_api_key_management_store_and_database_status(
        ...
        Some(appbase_foundation_router),
        ...
        Some(app_iam_directory_read_store),
...
pub async fn router_with_postgres_product_catalog(
    ...
    let app_iam_directory_read_store =
        Arc::new(PostgresAppIamDirectoryReadStore::new(pool.clone()));
    let membership_store = Arc::new(PostgresCommerceMembershipStore::new(pool.clone()));
    ...
    let appbase_wallet_router =
        sdkwork_commerce_http::app_account_wallet_router_with_postgres_pool(pool.clone());
    let appbase_commerce_router =
...
pub async fn router_with_sqlite_shared_runtime(
    ...
    Ok(router_with_api_key_management_store_and_database_status(
        Arc::new(SqlitePricingCatalogLoader::with_api_key_secret_codec(
            pool.clone(),
            api_key_secret_codec.clone(),
        )),
        Arc::new(SqliteGatewayApiKeyCommandStore::new(
...
pub async fn router_with_postgres_shared_runtime(
    ...
    Ok(router_with_api_key_management_store_and_database_status(
        Arc::new(PostgresPricingCatalogLoader::with_api_key_secret_codec(
            pool.clone(),
            api_key_secret_codec.clone(),
        )),
        Arc::new(PostgresGatewayApiKeyCommandStore::new(
...
async fn router_with_database_config_api_key_trusted_subject_app_session_and_optional_provider_secret_map_config_and_startup_install_mode(
    ...
    match config.engine {
        DatabaseEngine::Sqlite => {
            ...
            let appbase_foundation_router =
                sdkwork_commerce_http::app_commerce_foundation_router_with_sqlite_pool(
                    pool.clone(),
                );
            let appbase_wallet_router =
                sdkwork_commerce_http::app_account_wallet_router_with_sqlite_pool(pool.clone());
        ...
        DatabaseEngine::Postgres => {
            ...
            let appbase_foundation_router =
                sdkwork_commerce_http::app_commerce_foundation_router_with_postgres_pool(
                    pool.clone(),
                );
            let appbase_wallet_router =
                sdkwork_commerce_http::app_account_wallet_router_with_postgres_pool(pool.clone());
...
async fn sqlite_model_ranking_schema_ready(pool: &SqlitePool) -> Result<bool, sqlx::Error> {
    ...
    let model_column_count: i64 = sqlx::query_scalar(
        ...
        WHERE name IN ('catalog_key', 'vendor_code', 'capability', 'rank_score')
        "#,
    )
    .fetch_one(pool)
    .await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn sqlite_model_ranking_schema_ready_accepts_global_model_schema_without_region_code() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
