...
fn build_payment_callback_command(
    ...
    let event_id =
        callback_header(headers, &["x-sdkwork-event-id", "x-event-id"]).unwrap_or_else(|| {
            sha256_text(&format!(
                "{provider_key}|{nonce}|{}|{payload_digest}",
...
fn validate_payment_callback(
    ...
    let parsed = parse_payment_callback_payload(provider_key, raw)?;
...
fn parse_payment_callback_payload(
    ...
    if trimmed.starts_with('<') {
        return parse_xml_payment_callback(provider_key, trimmed);
    }
...
fn parse_payment_provider(raw_provider: &str) -> Result<(i64, String), String> {
    let normalized = raw_provider.trim().to_ascii_lowercase();
    match normalized.as_str() {
        "wechat" | "wechatpay" | "wechat_pay" | "wxpay" => Ok((1, "wechat".to_owned())),
        "alipay" | "ali" => Ok((2, "alipay".to_owned())),
        "stripe" | "card" => Ok((7, "stripe".to_owned())),
        _ => Err(format!("unsupported payment provider: {raw_provider}")),
    }
}
