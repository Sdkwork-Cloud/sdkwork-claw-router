...
fn effective_response_body<'a>(
    invocation: &Invocation,
    response: &'a InvocationDispatchResponse,
) -> Option<&'a Value> {
    let body = response.body.as_ref()?;
    if invocation.dispatch.mode == DispatchMode::InternalProviderAdapter {
        return body.get("body").or(Some(body));
    }
    Some(body)
}
...
fn sticky_response_object_id(object_type: &str, response_body: &Value) -> Option<String> {
    ...
    if response_body
        .get("object")
        .and_then(Value::as_str)
        .is_some_and(|object| sticky_response_object_matches(object_type, object))
    {
