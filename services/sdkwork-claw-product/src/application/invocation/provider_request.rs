...
fn build_adapter_provider_request(
    ...
    Ok(InvocationProviderRequest {
        ...
        query: None,
        headers,
        body: build_adapter_body(invocation, account, resolved_secret)?,
    })
}
...
fn build_adapter_body(
    ...
    Ok(InvocationBody::Json(json!({
        "invocation": {
            ...
            "standardPath": invocation.request.path.as_str(),
            "shape": adapter_shape_code(&invocation.dispatch.invocation_shape),
            "stream": adapter_invocation_shape_streams(&invocation.dispatch.invocation_shape)
        },
        "subject": {
...
fn adapter_shape_code(shape: &super::InvocationShape) -> &'static str {
    match shape {
        super::InvocationShape::Json => "sync_json",
        super::InvocationShape::SseStream => "sse_stream",
        super::InvocationShape::ByteStream => "byte_stream",
        super::InvocationShape::Empty => "sync_json",
    }
}

fn adapter_invocation_shape_streams(shape: &super::InvocationShape) -> bool {
    matches!(
        shape,
        super::InvocationShape::SseStream | super::InvocationShape::ByteStream
    )
}
