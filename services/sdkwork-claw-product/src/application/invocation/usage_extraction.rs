...
fn extract_composite_usage_from_body(
    ...
    let input_tokens = integer_field(usage, &["prompt_tokens", "input_tokens"]).unwrap_or(0);
    let output_tokens = integer_field(usage, &["completion_tokens", "output_tokens"]).unwrap_or(0);
...
fn streaming_usage_body(invocation: &Invocation) -> Result<Option<Value>, InvocationError> {
    ...
    let Some(bytes) = response.body_bytes.as_ref() else {
        return Ok(None);
    };
    let text = std::str::from_utf8(bytes).map_err(|error| {
        usage_error(format!("streaming provider response is not UTF-8: {error}"))
    })?;
    Ok(openai_sse_usage_body(text))
}
