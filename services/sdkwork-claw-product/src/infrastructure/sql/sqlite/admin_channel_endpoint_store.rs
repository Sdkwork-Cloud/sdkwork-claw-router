...
#[derive(Debug, Clone)]
pub struct SqliteAdminChannelEndpointStore {
    pool: SqlitePool,
}

impl SqliteAdminChannelEndpointStore {
    pub fn new(pool: SqlitePool) -> Self {
        Self { pool }
    }
}

impl AdminChannelEndpointStore for SqliteAdminChannelEndpointStore {
...
async fn insert_channel_endpoint(
    ...
    let result = sqlx::query(
        ...
        SELECT
            ?, a.tenant_id, a.organization_id, 1, ?, ?, ?, 0, '{}',
            a.id,
            a.provider_code,
            COALESCE(NULLIF(a.channel_code, ''), a.provider_code || '-' || CAST(a.id AS TEXT)),
    ...
    last_insert_rowid(tx).await.map(Some)
}
...
async fn update_channel_endpoint_core(
    ...
    sqlx::query(
        ...
        SET vendor_code = COALESCE(?, vendor_code),
            ...
            effective_from = CASE WHEN ? THEN ? ELSE effective_from END,
            effective_to = CASE WHEN ? THEN ? ELSE effective_to END,
            updated_at = ?,
    ...
    .bind(present_flag(command.effective_from.is_some()))
    .bind(
        command
            .effective_from
            .as_ref()
    ...
    .bind(present_flag(command.effective_to.is_some()))
    .bind(
        command
            .effective_to
            .as_ref()
...
fn item_from_row(row: sqlx::sqlite::SqliteRow) -> DomainResult<AdminChannelEndpointItem> {
...
async fn last_insert_rowid(tx: &mut Transaction<'_, Sqlite>) -> DomainResult<i64> {
    sqlx::query_scalar("SELECT last_insert_rowid()")
        .fetch_one(&mut **tx)
        .await
        .map_err(|error| store_error("failed to read inserted channel endpoint id", error))
}
