...
ORDER BY p.rank_level ASC, p.id ASC
LIMIT 500
"#;
...
ORDER BY rb.created_at DESC, rb.id ASC
LIMIT 500
"#;

#[derive(Debug, Clone)]
pub struct SqliteAppIamDirectoryReadStore {
    pool: SqlitePool,
}
...
fn row_to_organization(row: &sqlx::sqlite::SqliteRow) -> DomainResult<AppIamOrganizationItem> {
...
) -> DomainResult<AppIamOrganizationMembershipItem> {
...
fn row_to_department(row: &sqlx::sqlite::SqliteRow) -> DomainResult<AppIamDepartmentItem> {
...
fn row_to_department_assignment(
    row: &sqlx::sqlite::SqliteRow,
) -> DomainResult<AppIamDepartmentAssignmentItem> {
...
fn row_to_position(row: &sqlx::sqlite::SqliteRow) -> DomainResult<AppIamPositionItem> {
...
fn row_to_position_assignment(
    row: &sqlx::sqlite::SqliteRow,
) -> DomainResult<AppIamPositionAssignmentItem> {
...
fn row_to_role_binding(row: &sqlx::sqlite::SqliteRow) -> DomainResult<AppIamRoleBindingItem> {
...
async fn table_exists(pool: &SqlitePool, table_name: &str) -> DomainResult<bool> {
...
fn require_subject(
    subject: Option<AppIamDirectorySubject>,
) -> DomainResult<AppIamDirectorySubject> {
    subject.ok_or_else(|| {
        DomainError::new("trusted request subject is required for app IAM directory")
    })
}
...
fn option_string_cell(row: &sqlx::sqlite::SqliteRow, column: &str) -> Option<String> {
