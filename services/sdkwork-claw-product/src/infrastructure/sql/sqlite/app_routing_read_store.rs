...
SELECT
    ...
    json_extract(COALESCE(k.metadata, '{}'), '$.copyableKeyCiphertext') AS copyable_key_ciphertext,
...
SELECT
    ...
    CAST(COALESCE(t.error_type, '') AS TEXT) AS error_type,
    ...
    COALESCE(t.streaming, 0) AS streaming,
...
FROM (
    SELECT
        substr(CAST(COALESCE(started_at, created_at) AS TEXT), 1, 10) AS bucket_time,
...
WITH trace_by_model AS (
    SELECT
        ...
        CAST(COALESCE(AVG(t.latency_ms), 0) AS INTEGER) AS avg_latency_ms
    FROM ai_request_trace t
    LEFT JOIN (
        SELECT
            tenant_id,
...
#[derive(Clone)]
pub struct SqliteAppRoutingReadStore {
    pool: SqlitePool,
    api_key_secret_codec: Option<Arc<dyn ApiKeySecretCodec + Send + Sync>>,
}

impl SqliteAppRoutingReadStore {
...
impl AppRoutingReadStore for SqliteAppRoutingReadStore {
...
fn row_to_request_trace(row: sqlx::sqlite::SqliteRow) -> DomainResult<AppRoutingRequestTraceItem> {
...
fn row_to_usage_data(row: sqlx::sqlite::SqliteRow) -> AppRoutingUsageData {
    AppRoutingUsageData {
...
fn row_to_model_stats(row: sqlx::sqlite::SqliteRow) -> AppRoutingModelStats {
