...
SELECT
    ...
    CAST(COALESCE(vpc.change_amount / 10.0, CASE WHEN c.amount IS NULL THEN 0 ELSE c.amount / 100.0 END, 0) AS TEXT) AS amount,
...
WHERE uc.tenant_id = ?1
  ...
  AND uc.user_id = ?3
ORDER BY COALESCE(uc.acquire_at, uc.created_at) DESC, uc.id DESC
LIMIT 100
"#;

const LOAD_RECHARGE_HISTORY: &str = r#"
SELECT
    COALESCE(p.id, o.id, vr.id) AS id,
    p.id AS payment_id,
...
WHERE o.tenant_id = ?1
  ...
  AND o.user_id = ?3
ORDER BY COALESCE(vr.recharge_time, p.success_time, p.created_at, o.pay_success_time, o.created_at) DESC, o.id DESC
LIMIT 100
"#;
...
SELECT
    ...
    COALESCE(get_limit, 0) AS get_limit,
    COALESCE(stackable, 0) AS stackable
FROM plus_coupon
WHERE redeem_code = ?
  AND status = 1
...
async fn ensure_points_account(
    ...
    let id: i64 = sqlx::query_scalar("SELECT last_insert_rowid()")
        .fetch_one(&mut **tx)
        .await
        .map_err(|error| store_error("failed to read points account id", error))?;
...
async fn insert_user_coupon(
    ...
    .bind(if coupon.stackable { 1_i32 } else { 0_i32 })
    .execute(&mut **tx)
    .await
    ...
    sqlx::query_scalar("SELECT last_insert_rowid()")
        .fetch_one(&mut **tx)
        .await
        .map_err(|error| store_error("failed to read user coupon id", error))
}
...
fn recharge_history_status(row: &sqlx::sqlite::SqliteRow) -> Result<&'static str, DomainError> {
    let order_status =
