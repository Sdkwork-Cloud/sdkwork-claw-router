...
fn require_subject(
    subject: Option<AppGatewayTracesSubject>,
) -> DomainResult<AppGatewayTracesSubject> {
    subject.ok_or_else(|| {
        DomainError::new("trusted request subject is required for app gateway traces")
    })
}
...
fn gateway_http_status(value: i64) -> DomainResult<i64> {
...
fn gateway_latency_ms(value: i64) -> DomainResult<i64> {
    if value >= 0 {
        Ok(value)
    } else {
        Err(DomainError::new(format!(
...
fn gateway_health_status(row: &sqlx::sqlite::SqliteRow) -> DomainResult<i64> {
...
fn gateway_deployment_mode(row: &sqlx::sqlite::SqliteRow) -> DomainResult<i64> {
    if string_cell(row, "gateway_id").trim().is_empty() {
        return Ok(0);
    }
...
fn missing_integer_cell_error(column: &str) -> DomainError {
    match column {
        "health_status" => {
            DomainError::new("missing gateway trace health_status from database row")
        }
        "deployment_mode" => {
            DomainError::new("missing gateway trace deployment_mode from database row")
        }
        column => DomainError::new(format!("missing gateway trace {column} from database row")),
    }
}
