...
use crate::infrastructure::sql::sql_admin_analytics::{
...
use crate::ports::{
    ...
    AdminAnalyticsSnapshot, AdminAnalyticsSubject, AdminAnalyticsSummary, AdminAnalyticsTrendPoint,
...
fn build_snapshot(
    time_range: crate::ports::AdminAnalyticsTimeRange,
...
) -> Result<AdminAnalyticsSummary, DomainError> {
...
async fn load_trend(
    ...
    query: &AdminAnalyticsQuery,
    subject: AdminAnalyticsSubject,
) -> Result<Vec<AdminAnalyticsTrendPoint>, DomainError> {
...
async fn load_user_aggregates(
    ...
    query: &AdminAnalyticsQuery,
    subject: AdminAnalyticsSubject,
) -> Result<Vec<UserAggregate>, DomainError> {
    let sql = format!(
        r#"
        SELECT
            user_id,
...
) -> Result<Vec<ModelAggregate>, DomainError> {
    let sql = format!(
        r#"
        SELECT
            model,
...
async fn load_model_failed_requests(
    ...
    query: &AdminAnalyticsQuery,
    subject: AdminAnalyticsSubject,
) -> Result<Vec<ModelFailedRequest>, DomainError> {
    let sql = format!(
        r#"
        SELECT
            model,
...
async fn load_distribution(
    ...
    value_expr: &str,
    limit: usize,
) -> Result<Vec<AdminAnalyticsPieItem>, DomainError> {
    let sql = format!(
        r#"
        SELECT
            name,
...
fn period_expression(time_range: crate::ports::AdminAnalyticsTimeRange) -> &'static str {
    match time_range {
        crate::ports::AdminAnalyticsTimeRange::Hourly => "strftime('%Y-%m-%d %H:00', occurred_at)",
        crate::ports::AdminAnalyticsTimeRange::Weekly => "strftime('%Y-W%W', occurred_at)",
        crate::ports::AdminAnalyticsTimeRange::Monthly => "strftime('%Y-%m', occurred_at)",
        crate::ports::AdminAnalyticsTimeRange::Yearly => "strftime('%Y', occurred_at)",
        crate::ports::AdminAnalyticsTimeRange::Daily => "substr(occurred_at, 1, 10)",
    }
}
