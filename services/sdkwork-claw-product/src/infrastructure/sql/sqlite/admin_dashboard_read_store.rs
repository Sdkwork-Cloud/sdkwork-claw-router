...
async fn load_user_consumption(
    ...
    tenant_id: i64,
    organization_id: i64,
) -> Result<Vec<AdminPieChartItem>, DomainError> {
...
async fn load_traffic(
    ...
    tenant_id: i64,
    organization_id: i64,
) -> Result<Vec<AdminDashboardTrafficItem>, DomainError> {
...
async fn load_recent_usage(
    ...
    tenant_id: i64,
    organization_id: i64,
) -> Result<Vec<AdminDashboardRecentUsageItem>, DomainError> {
...
fn pie_chart_item(
    ...
    value_column: &str,
    index: usize,
) -> Result<AdminPieChartItem, DomainError> {
    Ok(AdminPieChartItem {
...
fn recent_usage_item(
    row: sqlx::sqlite::SqliteRow,
) -> Result<AdminDashboardRecentUsageItem, DomainError> {
...
fn required_string_cell(
    ...
    if value.trim().is_empty() {
        return Err(DomainError::new(format!("missing {field_name}")));
    }
    Ok(value)
}
