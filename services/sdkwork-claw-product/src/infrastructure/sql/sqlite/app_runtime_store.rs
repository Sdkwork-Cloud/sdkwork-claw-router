...
#[derive(Debug, Clone)]
pub struct SqliteAppRuntimeStore {
    pool: SqlitePool,
}

impl SqliteAppRuntimeStore {
    pub fn new(pool: SqlitePool) -> Self {
        Self { pool }
    }
}

impl AppRuntimeStore for SqliteAppRuntimeStore {
...
async fn create_artifact(
    ...
    let row = sqlx::query(
        "SELECT * FROM ai_runtime_artifact WHERE uuid = ?1 AND tenant_id = ?2 AND organization_id = ?3 AND user_id = ?4",
    )
        .bind(&command.artifact_uuid)
        .bind(command.subject.tenant_id)
        .bind(command.subject.organization_id)
...
async fn load_invocation_row_by_uuid(
    ...
    sqlx::query(invocation_select_sql())
        .bind(subject.tenant_id)
        .bind(subject.organization_id)
        .bind(subject.user_id)
        .bind(invocation_id)
...
fn invocation_select_sql() -> &'static str {
    r#"
    SELECT *
    FROM ai_runtime_invocation
    WHERE tenant_id = ?1
...
fn row_to_invocation(row: sqlx::sqlite::SqliteRow) -> DomainResult<AppRuntimeInvocationItem> {
...
fn row_to_event(row: sqlx::sqlite::SqliteRow) -> DomainResult<AppRuntimeEventItem> {
...
fn row_to_artifact(row: sqlx::sqlite::SqliteRow) -> DomainResult<AppRuntimeArtifactItem> {
...
fn sql_error(error: sqlx::Error) -> DomainError {
    DomainError::new(format!("sqlite app runtime store error: {error}"))
}
