...
#[derive(Debug, Clone)]
pub struct SqliteAdminProviderSecretStore {
    pool: SqlitePool,
}

impl SqliteAdminProviderSecretStore {
    pub fn new(pool: SqlitePool) -> Self {
        Self { pool }
    }
}

impl AdminProviderSecretStore for SqliteAdminProviderSecretStore {
...
async fn insert_provider_secret(
    ...
    sqlx::query_scalar("SELECT last_insert_rowid()")
        .fetch_one(&mut **tx)
        .await
        .map_err(|error| store_error("failed to read provider secret id", error))
}
...
async fn insert_config_snapshot(
    ...
    sqlx::query(
        ...
        VALUES
            (?, ?, ?, ?, ?, ?, 1, ?, 'integration_provider_account', ?, ?, ?, ?, ?)
        "#,
    )
