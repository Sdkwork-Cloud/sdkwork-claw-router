...
WHERE s.status = 1
  ...
  AND s.owner_id = ?3
  AND (?4 IS NULL OR substr(CAST(s.period_start AS TEXT), 1, 4) = ?4 OR s.period LIKE (?4 || '%'))
GROUP BY
    s.id,
    s.statement_no,
    s.period,
...
WHERE i.status = 1
  ...
  AND s.owner_id = ?3
  AND (?4 IS NULL OR substr(CAST(s.period_start AS TEXT), 1, 4) = ?4 OR s.period LIKE (?4 || '%'))
ORDER BY s.period_end DESC, i.statement_id DESC, i.item_type ASC, i.model ASC
"#;
...
WHERE status = 1
  ...
  AND (?4 IS NULL OR substr(CAST(occurred_at AS TEXT), 1, 4) = ?4)
GROUP BY day
ORDER BY day ASC
LIMIT 366
"#;

pub struct SqliteSettlementsDashboardReadStore {
    pool: SqlitePool,
}

impl SqliteSettlementsDashboardReadStore {
    pub fn new(pool: SqlitePool) -> Self {
        Self { pool }
    }
}

impl SettlementsDashboardReadStore for SqliteSettlementsDashboardReadStore {
...
fn row_to_bill(row: sqlx::sqlite::SqliteRow) -> Result<SettlementBill, DomainError> {
    Ok(SettlementBill {
...
fn usage_label(row: &sqlx::sqlite::SqliteRow, modality: i64) -> String {
...
fn whole_decimal_string_cell(row: &sqlx::sqlite::SqliteRow, column: &str) -> String {
    let value = string_cell(row, column);
    let value = value.trim();
    if value.is_empty() {
        return "0".to_owned();
