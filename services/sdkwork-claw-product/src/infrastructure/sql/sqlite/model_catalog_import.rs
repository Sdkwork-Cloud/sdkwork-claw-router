...
async fn load_family_ids(
    ...
    let rows = sqlx::query(
        "SELECT id, vendor_code, region_code, family_code FROM ai_model_family WHERE tenant_id = 0 AND organization_id = 0",
    )
    .fetch_all(&mut *conn)
    .await?;
    Ok(rows
        ...
        for family in &vendor.families {
            if let Some(default_model) = &family.default_model {
                ...
                sqlx::query(
                    ...
                    WHERE tenant_id = 0 AND organization_id = 0 AND vendor_code = ? AND region_code = ? AND family_code = ?
                    "#,
                )
