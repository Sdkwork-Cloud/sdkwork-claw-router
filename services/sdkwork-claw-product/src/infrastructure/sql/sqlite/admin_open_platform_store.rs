...
#[derive(Clone)]
pub struct SqliteAdminOpenPlatformStore {
    pool: SqlitePool,
    api_key_secret_codec: Option<Arc<dyn ApiKeySecretCodec + Send + Sync>>,
}

impl std::fmt::Debug for SqliteAdminOpenPlatformStore {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("SqliteAdminOpenPlatformStore")
...
impl SqliteAdminOpenPlatformStore {
...
impl AdminOpenPlatformStore for SqliteAdminOpenPlatformStore {
    fn list_providers<'a>(
        &'a self,
...
async fn list_accounts(
    ...
    let rows = sqlx::query(
        ...
        SELECT
            ...
            aes_key_ref,
            default_entry_id,
            COALESCE(qr_default, 0) AS qr_default,
            status,
...
async fn find_qr_default_entry(
    ...
    let row = sqlx::query(
        ...
        SELECT
            ...
            COALESCE(a.qr_default, 0) AS account_qr_default,
...
async fn insert_account(
    ...
    sqlx::query_scalar("SELECT last_insert_rowid()")
        .fetch_one(&mut **tx)
        .await
        .map_err(|error| store_error("failed to read open platform account id", error))
}
...
async fn update_account(
    ...
    .bind(if next_qr_default { 1 } else { 0 })
    .bind(
        command
            .status
            .as_deref()
...
async fn load_account_metadata_by_id(
    ...
    let metadata = sqlx::query_scalar(
        r#"
        SELECT CAST(COALESCE(metadata, '{}') AS TEXT)
        FROM open_platform_account
        WHERE id = ?
          AND tenant_id = ?
          AND organization_id = ?
    ...
    Ok(metadata.unwrap_or_else(|| "{}".to_owned()))
}
...
async fn insert_entry(
    ...
    sqlx::query_scalar("SELECT last_insert_rowid()")
        .fetch_one(&mut **tx)
        .await
        .map_err(|error| store_error("failed to read open platform entry id", error))
}
...
async fn insert_pay_binding(
    ...
    sqlx::query_scalar("SELECT last_insert_rowid()")
        .fetch_one(&mut **tx)
        .await
        .map_err(|error| store_error("failed to read open platform pay binding id", error))
}
...
fn provider_from_row(row: &sqlx::sqlite::SqliteRow) -> DomainResult<AdminOpenPlatformProviderItem> {
...
fn manifest_from_row(row: &sqlx::sqlite::SqliteRow) -> DomainResult<AdminOpenPlatformManifestItem> {
...
fn account_from_row(row: &sqlx::sqlite::SqliteRow) -> DomainResult<AdminOpenPlatformAccountItem> {
...
fn entry_from_row(row: &sqlx::sqlite::SqliteRow) -> DomainResult<AdminOpenPlatformEntryItem> {
