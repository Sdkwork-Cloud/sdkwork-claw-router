...
async fn list_announcements(
    ...
    let rows = sqlx::query(
        ...
        ORDER BY COALESCE(m.published_at, m.updated_at, m.created_at) DESC, m.id DESC
        LIMIT 200
        "#,
    )
...
async fn insert_notification_message(
    ...
    let id: i64 = sqlx::query_scalar("SELECT last_insert_rowid()")
        .fetch_one(&mut **tx)
        .await
        .map_err(|error| store_error("failed to read announcement notification id", error))?;
...
async fn update_notification_message(
    ...
    let result = sqlx::query(
        ...
        SET title = COALESCE(?, title),
            ...
            show_as_popup = COALESCE(?, show_as_popup),
            published_at = CASE
                WHEN ? = 0 THEN NULL
        ...
        WHERE id = ?
          ...
          AND organization_id = ?
          AND scope_type = ?
          AND message_code = CAST('announcement:' || id AS TEXT)
