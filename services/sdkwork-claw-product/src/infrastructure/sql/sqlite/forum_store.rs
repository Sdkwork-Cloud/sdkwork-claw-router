    ...
    fn load_comment_detail<'a>(
        ...
        Box::pin(async move {
            ...
            let sql = format!(
                ...
                scope_filter = sqlite_scope_filter("plus_comments"),
            );
...
async fn increment_feed_view_count(
    ...
    sqlx::query(
        ...
        WHERE id = ?3
          ...
          AND {scope_filter}
        "#,
            scope_filter = sqlite_scope_filter("plus_feeds"),
        )
        .as_str(),
    )
    .bind(scope.tenant_id)
...
async fn load_forum_overview(
    ...
    let sql = format!(
        ...
        "#,
        feed_scope_filter = sqlite_scope_filter("plus_feeds"),
        comment_scope_filter = sqlite_scope_filter("plus_comments"),
    );
    let row = sqlx::query(sql.as_str())
        .bind(scope.tenant_id)
        .bind(scope.organization_id)
