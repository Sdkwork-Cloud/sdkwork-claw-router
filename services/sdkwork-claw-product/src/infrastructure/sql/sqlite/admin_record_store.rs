...
mod tests {
    ...
    fn list_logs_projects_user_agent_from_trace_metadata() {
        assert!(
            LIST_ADMIN_RECORD_LOGS.contains("json_extract(t.metadata, '$.userAgent')"),
            "admin record SQLite SQL must project the full User-Agent from trace metadata"
        );
        assert!(
            LIST_ADMIN_RECORD_LOGS.contains("AS user_agent"),
            "admin record SQLite SQL must expose a user_agent column for API serialization"
        );
    }
