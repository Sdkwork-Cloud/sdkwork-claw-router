...
#[derive(Debug, Clone)]
pub struct SqliteAdminMonitorReadStore {
    pool: SqlitePool,
}
...
fn node_from_row(row: sqlx::sqlite::SqliteRow) -> DomainResult<AdminMonitorNode> {
...
fn alert_from_row(row: sqlx::sqlite::SqliteRow) -> DomainResult<AdminMonitorAlert> {
...
) -> DomainResult<AdminMonitorPerformanceDatum> {
...
fn required_decimal_cell(row: &sqlx::sqlite::SqliteRow, column: &str) -> DomainResult<f64> {
