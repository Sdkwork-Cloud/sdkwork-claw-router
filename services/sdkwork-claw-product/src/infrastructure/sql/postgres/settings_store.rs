...
SELECT
    ...
    COALESCE(p.notification_preferences::text, '{}') AS notifications_json
FROM (
    SELECT $1::bigint AS tenant_id, $2::bigint AS organization_id, $3::bigint AS user_id, $4::varchar AS endpoint_code
) subject
...
VALUES
    ($1, $2, $3, $4, 1, $4, 1, 1, $5::timestamp AT TIME ZONE 'UTC', $5::timestamp AT TIME ZONE 'UTC', 0, '{}'::jsonb, $6, $7, $8::jsonb)
...
VALUES
    ($1, $2, $3, $4, 1, $4, 1, 1, $5::timestamp AT TIME ZONE 'UTC', $5::timestamp AT TIME ZONE 'UTC', 0, $6::jsonb, $7, 'Console Settings Webhook', $8, $9::jsonb, 'hmac-sha256', $10::jsonb, 0)
...
async fn load_settings(pool: &PgPool, subject: SettingsSubject) -> DomainResult<SettingsData> {
...
mod tests {
    ...
    #[test]
    fn settings_load_query_scopes_preferences_and_webhook_to_app_subject() {
        for predicate in [
            "p.tenant_id = subject.tenant_id",
            "p.organization_id = subject.organization_id",
            "p.user_id = subject.user_id",
            "w.tenant_id = subject.tenant_id",
            "w.organization_id = subject.organization_id",
            "w.endpoint_code = subject.endpoint_code",
        ] {
            assert!(
                LOAD_SETTINGS.contains(predicate),
                "settings Postgres SQL must include subject predicate {predicate}"
            );
        }
    }

    #[test]
    fn settings_upserts_use_subject_scoped_conflict_targets_and_clear_soft_delete_state() {
        for (sql, conflict_target) in [
            (
            ...
            (
                ...
                "ON CONFLICT(tenant_id, organization_id, endpoint_code)",
            ),
        ] {
            assert!(
                sql.contains(conflict_target),
                "settings Postgres upsert must use scoped conflict target {conflict_target}"
            );
            assert!(
                sql.contains("deleted_at = NULL") && sql.contains("deleted_by = NULL"),
                "settings Postgres upsert must restore soft-deleted rows without leaking stale delete state"
            );
        }
    }
}
