...
async fn list_monitor_nodes(
    pool: &PgPool,
    query: AdminMonitorQuery,
) -> DomainResult<Vec<AdminMonitorNode>> {
    let rows = sqlx::query(
        r#"
        SELECT
            i.id,
...
async fn list_monitor_alerts(
    pool: &PgPool,
    query: AdminMonitorQuery,
) -> DomainResult<Vec<AdminMonitorAlert>> {
    let rows = sqlx::query(
        r#"
        SELECT
...
async fn list_monitor_performance(
    pool: &PgPool,
    query: AdminMonitorQuery,
) -> DomainResult<Vec<AdminMonitorPerformanceDatum>> {
    let rows = sqlx::query(
        r#"
        SELECT
...
fn node_from_row(row: sqlx::postgres::PgRow) -> DomainResult<AdminMonitorNode> {
...
fn alert_from_row(row: sqlx::postgres::PgRow) -> DomainResult<AdminMonitorAlert> {
...
fn performance_from_row(row: sqlx::postgres::PgRow) -> DomainResult<AdminMonitorPerformanceDatum> {
...
fn node_status_label(status: i64) -> DomainResult<String> {
    match status {
        1 => Ok("online"),
        2 => Ok("warning"),
        0 => Ok("offline"),
...
fn severity_label(severity: i64) -> DomainResult<String> {
    match severity {
...
fn required_integer_cell(row: &sqlx::postgres::PgRow, column: &str) -> DomainResult<i64> {
    row.try_get::<Option<i64>, _>(column)
        ...
        .ok_or_else(|| DomainError::new(format!("missing monitor {column} from database row")))
}

fn required_decimal_cell(row: &sqlx::postgres::PgRow, column: &str) -> DomainResult<f64> {
    row.try_get::<Option<String>, _>(column)
        .ok()
        .flatten()
        .and_then(|value| value.parse().ok())
        .ok_or_else(|| DomainError::new(format!("invalid monitor {column} from database row")))
}
