...
WITH ledger_entries AS (
    SELECT
        ...
        CASE
            ...
            WHEN CAST(COALESCE(NULLIF(CAST(h.amount AS TEXT), ''), '0') AS NUMERIC) > 0 OR COALESCE(h.points_change, 0) > 0 THEN 'recharge'
            ELSE 'consume'
        END AS normalized_type,
        CASE
            WHEN ABS(CAST(COALESCE(NULLIF(CAST(h.amount AS TEXT), ''), '0') AS NUMERIC)) > 0 THEN CAST(h.amount AS TEXT)
            WHEN COALESCE(h.points_change, 0) != 0 THEN CAST(h.points_change AS TEXT)
            ELSE '0'
        END AS amount,
        CASE
            WHEN CAST(COALESCE(NULLIF(CAST(h.balance_after AS TEXT), ''), '0') AS NUMERIC) != 0 THEN CAST(h.balance_after AS TEXT)
...
#[derive(Debug, Clone)]
pub struct PostgresAdminFinanceStore {
    pool: PgPool,
}

impl PostgresAdminFinanceStore {
    pub fn new(pool: PgPool) -> Self {
        Self { pool }
    }
}

impl AdminFinanceStore for PostgresAdminFinanceStore {
...
fn related_cell_present(row: &sqlx::postgres::PgRow, column: &str) -> bool {
    if row
