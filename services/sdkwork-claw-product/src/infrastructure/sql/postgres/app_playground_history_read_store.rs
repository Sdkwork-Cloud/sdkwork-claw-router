...
use crate::ports::{
    ...
    AppPlaygroundHistorySubject, AppPlaygroundMediaItem,
};

const LOAD_PLAYGROUND_HISTORY: &str = r#"
SELECT
...
#[derive(Debug, Clone)]
pub struct PostgresAppPlaygroundHistoryReadStore {
    pool: PgPool,
}

impl PostgresAppPlaygroundHistoryReadStore {
    pub fn new(pool: PgPool) -> Self {
        Self { pool }
    }
}

impl AppPlaygroundHistoryReadStore for PostgresAppPlaygroundHistoryReadStore {
    fn load_playground_history<'a>(
        ...
        Box::pin(async move {
            ...
            let rows = sqlx::query(LOAD_PLAYGROUND_HISTORY)
...
fn row_to_history_item(row: sqlx::postgres::PgRow) -> DomainResult<AppPlaygroundHistoryItem> {
...
fn require_subject(
    subject: Option<AppPlaygroundHistorySubject>,
) -> DomainResult<AppPlaygroundHistorySubject> {
    subject.ok_or_else(|| {
        DomainError::new("trusted request subject is required for app playground history")
    })
}
...
fn item_type_label(value: i64) -> DomainResult<&'static str> {
    match value {
        ...
        value => Err(DomainError::new(format!(
            "invalid playground history item kind from database row: {value}"
        ))),
    }
}
...
fn status_label(value: i64) -> DomainResult<&'static str> {
    match value {
        ...
        4 => Ok("cancelled"),
        value => Err(DomainError::new(format!(
            "invalid playground history status from database row: {value}"
        ))),
    }
}
...
fn required_integer_cell(
    ...
    optional_integer_cell(row, column).ok_or_else(|| {
        DomainError::new(format!(
            "missing playground history {source} from database row"
        ))
    })
}
