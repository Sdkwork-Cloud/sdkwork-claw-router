...
ranked_channel AS (
    SELECT
        ...
        COALESCE(NULLIF(c.base_url_override, ''), NULLIF(px.endpoint, '')) AS channel_url,
...
SELECT
    ...
    COALESCE(NULLIF(rc.channel_url, ''), NULLIF(p.base_url_template, ''), '') AS url,
...
fn row_to_provider(row: sqlx::postgres::PgRow) -> DomainResult<AppProviderItem> {
...
fn integration_type_code(row: &sqlx::postgres::PgRow) -> DomainResult<String> {
