...
impl AdminModelStore for PostgresAdminModelStore {
    fn list_vendors<'a>(
        &'a self,
...
async fn apply_sdkwork_models_catalog_refresh(
    ...
    crate::infrastructure::sql::postgres::model_catalog_import::import_postgres_model_catalog_tx(
        tx, catalog,
    )
    .await
...
async fn insert_pricing_import_snapshot(
    ...
    sqlx::query_scalar(
        ...
        VALUES
            ($1, $2, $3, $4, $5, 1, $6::jsonb, 1, $7, $8, 'database', $9, $10, 0, 'USD', $11::timestamptz)
        RETURNING id
        "#,
    )
...
async fn upsert_model_catalog_sync_run(
    ...
    sqlx::query_scalar(
        ...
        VALUES
            ($1, $2, $3, 'manual_refresh', $4, 1, 1, $5::jsonb, $6, 'mixed', NULL, 1, $7::timestamptz, $8::timestamptz, $9::timestamptz, $10, $11, $12, $13, $14, $15, $16, 0, $17::jsonb)
        RETURNING id
        "#,
    )
    .bind(&sync_run_uuid)
