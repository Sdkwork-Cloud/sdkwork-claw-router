...
async fn load_snapshot(
    pool: &PgPool,
    query: AdminAnalyticsQuery,
) -> DomainResult<AdminAnalyticsSnapshot> {
...
async fn load_summary(
    ...
    end_time: Option<&str>,
) -> DomainResult<AnalyticsSummaryRow> {
...
async fn load_trend(
    ...
    start_time: Option<&str>,
    end_time: Option<&str>,
) -> DomainResult<Vec<AnalyticsTrendRow>> {
...
async fn load_user_rankings(
    ...
    limit: i64,
    order_by: &str,
) -> DomainResult<Vec<AnalyticsUserRankRow>> {
    let sql = format!(
        r#"
        SELECT
            COALESCE(CAST(NULLIF(owner_id, 0) AS TEXT), CAST(NULLIF(user_id, 0) AS TEXT), NULLIF(owner_name_snapshot, ''), 'unknown') AS user_id,
...
async fn load_model_rankings(
    ...
    limit: i64,
    order_by: &str,
) -> DomainResult<Vec<AnalyticsModelRankRow>> {
    let sql = format!(
        r#"
        SELECT
...
async fn load_user_model_distributions(
    ...
    start_time: Option<&str>,
    end_time: Option<&str>,
) -> DomainResult<Vec<(String, Vec<AnalyticsPieRow>)>> {
...
async fn load_pie_rows(
    ...
    start_time: Option<&str>,
    end_time: Option<&str>,
) -> DomainResult<Vec<AnalyticsPieRow>> {
    let rows = sqlx::query(sql)
        .bind(tenant_id)
        .bind(organization_id)
        .bind(start_time)
...
async fn load_modality_distribution(
    ...
    rows.into_iter()
        .map(|row| {
            Ok(AnalyticsPieRow {
                name: model_modality::label(optional_integer_cell(&row, "modality")).to_owned(),
                value: decimal_cell(&row, "value")?,
            })
        })
        .collect()
...
fn decimal_cell(row: &sqlx::postgres::PgRow, column: &str) -> DomainResult<f64> {
    let value = string_cell(row, column);
    let parsed = value
        .trim()
        .parse::<f64>()
        .map_err(|_| DomainError::new(format!("invalid admin analytics {column}: {value}")))?;
    if parsed.is_finite() {
        Ok(parsed)
    } else {
...
fn sql_error(error: sqlx::Error) -> DomainError {
    DomainError::new(format!(
        "postgres admin analytics read store error: {error}"
    ))
}
