...
impl ForumFeedReadStore for PostgresForumStore {
    fn load_feeds<'a>(
        &'a self,
        ...
        Box::pin(async move {
            ...
            let sql = format!(
                ...
                SELECT
                    ...
                    COALESCE(f.is_top, false) AS is_top,
                    COALESCE(f.is_hot, false) AS is_hot,
                    COALESCE(f.is_recommended, false) AS is_recommended,
                    COALESCE(to_char((f.created_at AT TIME ZONE 'UTC'), 'YYYY-MM-DD"T"HH24:MI:SS"Z"'), '') AS created_at,
                ...
                WHERE COALESCE(f.status, 0) = $6
                  ...
                  AND ($9::int8 IS NULL OR COALESCE(f.category_id, 0) = $9)
                  AND ($10::text IS NULL OR lower(COALESCE(f.title, '') || ' ' || COALESCE(f.summary, '')) LIKE $10)
                  AND {scope_filter}
                ORDER BY {order_by}
                LIMIT $11 OFFSET $12
                "#,
...
impl ForumFeedCommandStore for PostgresForumStore {
    fn create_feed<'a>(
        &'a self,
        ...
        Box::pin(async move {
            ...
            sqlx::query(
                ...
                VALUES
                    ($1, $2, $3, $3, 0, $4, $5, 1, $6,
                     $7, $8, $9, $10, $1, $11::jsonb, $12::jsonb,
                     $13::jsonb, $14, $15, $3, $16::jsonb, $17, 0, 0,
                     0, 0, 0, 0, 0, 0, 0)
                "#,
            )
            .bind(feed_id)
...
impl ForumCommentReadStore for PostgresForumStore {
    fn load_comments<'a>(
        &'a self,
        content_type: String,
        content_id: i64,
...
impl ForumCommentCommandStore for PostgresForumStore {
    fn create_comment<'a>(
        &'a self,
...
fn comment_item_from_row(row: &sqlx::postgres::PgRow) -> ForumCommentItem {
    let user_id = integer_cell(row, "user_id");
    ForumCommentItem {
