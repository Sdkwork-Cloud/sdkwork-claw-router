...
use crate::infrastructure::sql::read_model::is_missing_postgres_read_model;
use crate::ports::{
...
SELECT
    ...
    CAST(COALESCE(p.point_amount, 0) AS TEXT) AS bonus
...
WHERE (p.tenant_id = $1 OR p.tenant_id = 0)
  ...
  AND p.status = 1
  AND (p.valid_from IS NULL OR p.valid_from <= ($3::timestamp AT TIME ZONE 'UTC'))
  AND (p.valid_to IS NULL OR p.valid_to >= ($3::timestamp AT TIME ZONE 'UTC'))
...
SELECT
    ...
    CAST(COALESCE(point_amount, 0) AS TEXT) AS bonus,
    CAST(recharge_type AS TEXT) AS recharge_type
FROM plus_vip_recharge_pack
WHERE (tenant_id = $1 OR tenant_id = 0)
  AND (organization_id = $2 OR organization_id = 0)
  AND status = 1
  AND price = $3::numeric
  AND (valid_from IS NULL OR valid_from <= ($4::timestamp AT TIME ZONE 'UTC'))
  AND (valid_to IS NULL OR valid_to >= ($4::timestamp AT TIME ZONE 'UTC'))
...
SELECT
    CAST(pr.id AS TEXT) AS product_id,
    CAST(s.id AS TEXT) AS sku_id,
    CAST(COALESCE(pr.category_id, 0) AS TEXT) AS category_id,
    COALESCE(NULLIF(s.name, ''), NULLIF(s.title, ''), NULLIF(pr.title, ''), 'Points recharge') AS product_name,
    COALESCE(NULLIF(CAST(s.specs AS TEXT), ''), '{}') AS sku_spec
FROM plus_sku s
...
WHERE (s.tenant_id = $1 OR s.tenant_id = 0)
  ...
  AND pr.status = 1
ORDER BY
    CASE WHEN s.price = $3::numeric THEN 0 ELSE 1 END,
    pr.id ASC,
    s.id ASC
LIMIT 1
"#;
...
#[derive(Debug, Clone)]
pub struct PostgresRechargeStore {
    pool: PgPool,
}

impl PostgresRechargeStore {
    pub fn new(pool: PgPool) -> Self {
        Self { pool }
    }
}

impl RechargeStore for PostgresRechargeStore {
...
async fn insert_order(
    ...
    sqlx::query_scalar(
        ...
        VALUES
            ($1, $2, $3, 1, $4::timestamp AT TIME ZONE 'UTC', $4::timestamp AT TIME ZONE 'UTC', 0, $5, 4, 1, $6, $6, $7, $8, $9::numeric, 0, $10, 1, $11, $12, $9::numeric, 0, 0, 0, 0, 'CNY', $13, 'CONSOLE', $14, $15::timestamp AT TIME ZONE 'UTC', 0, $16, $17)
        RETURNING id
        "#,
    )
...
async fn insert_order_item(
    ...
    sqlx::query(
        ...
        VALUES
            ($1, $2, $3, 1, $4::timestamp AT TIME ZONE 'UTC', $4::timestamp AT TIME ZONE 'UTC', 0, $5, $6, 'VIRTUAL', $7, $8, 1, $9::numeric, $9::numeric, $7, $10, $11, 0, 0, 0, 'CNY', 0, 0, $12, $13)
        "#,
    )
...
async fn insert_payment(
    ...
    sqlx::query(
        ...
        VALUES
            ($1, $2, $3, 1, $4::timestamp AT TIME ZONE 'UTC', $4::timestamp AT TIME ZONE 'UTC', 0, $5, 'POINTS', $6, $7, $8, $9, $10, 1, $11::numeric, $12::timestamp AT TIME ZONE 'UTC', $13, $14, $15::jsonb, $16::jsonb, $17::jsonb)
        "#,
    )
...
async fn insert_vip_recharge(
    ...
    sqlx::query(
        ...
        VALUES
            ($1, $2, $3, 1, $4::timestamp AT TIME ZONE 'UTC', $4::timestamp AT TIME ZONE 'UTC', 0, $5, $6::numeric, $7, $8, $4::timestamp AT TIME ZONE 'UTC', $9, 3, $10, $11, $12)
        "#,
    )
...
fn empty_rows_when_read_model_is_missing(
    error: sqlx::Error,
) -> Result<Vec<sqlx::postgres::PgRow>, DomainError> {
...
mod tests {
    ...
    #[test]
    fn recharge_package_queries_cast_integer_projection_aliases_for_stable_postgres_mapping() {
        for projection in [
            "CAST(p.id AS TEXT) AS id",
            "CAST(COALESCE(p.point_amount, 0) AS TEXT) AS bonus",
            "CAST(id AS TEXT) AS id",
            ...
            assert!(
                [LOAD_RECHARGE_PACKAGES, LOAD_RECHARGE_PACK_FOR_AMOUNT]
                    .join("\n")
                    .contains(projection),
                "recharge package Postgres SQL must project {projection} to avoid int4/int8 row mapping drift"
            );
        }
    }

    #[test]
    fn recharge_method_and_product_queries_cast_integer_projection_aliases_for_stable_postgres_mapping(
    ) {
        for (sql, projection) in [
            (LOAD_RECHARGE_METHOD, "CAST(id AS TEXT) AS id"),
            (
                LOAD_RECHARGE_PRODUCT_SKU,
                "CAST(pr.id AS TEXT) AS product_id",
            ),
            (LOAD_RECHARGE_PRODUCT_SKU, "CAST(s.id AS TEXT) AS sku_id"),
            (
                LOAD_RECHARGE_PRODUCT_SKU,
                "CAST(COALESCE(pr.category_id, 0) AS TEXT) AS category_id",
            ),
            ...
            assert!(
                ...
                "recharge Postgres SQL must project {projection} to avoid int4/int8 row mapping drift"
            );
        }
    }

    #[test]
    fn recharge_queries_scope_catalog_reads_and_writes_to_app_subject() {
        for predicate in [
            "(p.tenant_id = $1 OR p.tenant_id = 0)",
            "(p.organization_id = $2 OR p.organization_id = 0)",
            "(tenant_id = $1 OR tenant_id = 0)",
            "(organization_id = $2 OR organization_id = 0)",
            "(s.tenant_id = $1 OR s.tenant_id = 0)",
            "(s.organization_id = $2 OR s.organization_id = 0)",
            "(pr.tenant_id = $1 OR pr.tenant_id = 0)",
            "(pr.organization_id = $2 OR pr.organization_id = 0)",
        ] {
            assert!(
                [
                    LOAD_RECHARGE_PACKAGES,
                    LOAD_RECHARGE_PACK_FOR_AMOUNT,
                    LOAD_RECHARGE_METHOD,
                    LOAD_RECHARGE_PRODUCT_SKU,
                ]
                .join("\n")
                .contains(predicate),
                "recharge Postgres SQL must include catalog scope predicate {predicate}"
            );
        }
    }
