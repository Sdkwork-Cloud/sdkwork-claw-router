    ...
    fn usage_logs_query_uses_api_key_group_name_for_display_and_search() {
        for sql in [LOAD_USAGE_LOGS, LOAD_USAGE_LOGS_TOTAL] {
            assert!(
                ...
                "usage logs Postgres SQL must join the API key group table"
            );
            assert!(
            ...
            assert!(
                ...
                "usage logs Postgres keyword search must include the maintained group name"
            );
        }
        assert!(
            ...
            "usage logs Postgres SQL must project the maintained group name with snapshot fallback"
        );
    }
