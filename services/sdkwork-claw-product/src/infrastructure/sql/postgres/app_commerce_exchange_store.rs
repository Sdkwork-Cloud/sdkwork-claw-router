...
#[derive(Debug, Clone)]
pub struct PostgresAppCommerceExchangeStore {
    pool: PgPool,
}

impl PostgresAppCommerceExchangeStore {
    pub fn new(pool: PgPool) -> Self {
        Self { pool }
    }
}

impl AppCommerceExchangeReadStore for PostgresAppCommerceExchangeStore {
...
WHERE ((tenant_id = $1 AND organization_id = $2) OR (tenant_id = 0 AND organization_id = 0))
  AND config_key = $3
ORDER BY CASE WHEN tenant_id = $4 AND organization_id = $5 THEN 0 ELSE 1 END,
         updated_at DESC,
         id DESC
LIMIT 500
"#;
