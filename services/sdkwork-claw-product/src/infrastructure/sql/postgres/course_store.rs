...
async fn load_courses(
    ...
    let sql = format!(
        ...
        WHERE COALESCE(c.status, 0) = $3
          ...
          AND ($4::int8 IS NULL OR COALESCE(c.level, 0) = $4)
          AND ($5::text IS NULL OR lower(COALESCE(c.category, '')) = $5)
          AND (
                $6::text IS NULL
             OR lower(COALESCE(c.title, '') || ' ' || COALESCE(c.description, '') || ' ' || COALESCE(c.category, '') || ' ' || COALESCE(c.tags::text, '')) LIKE $6
          )
          AND {scope_filter}
        ORDER BY COALESCE(c.published_at, c.updated_at, c.created_at) DESC NULLS LAST, c.id DESC
        LIMIT $7 OFFSET $8
        "#,
        select = COURSE_SELECT_COLUMNS,
        scope_filter = postgres_scope_filter("c"),
    );
    ...
    let sql = format!(
        ...
        WHERE COALESCE(c.status, 0) = $3
          ...
          AND (
             ...
             OR ($5::int8 IS NOT NULL AND c.id = $5)
          )
          AND {scope_filter}
        LIMIT 1
        "#,
...
async fn load_lessons(
    ...
    let rows = sqlx::query(
        ...
        SELECT
            ...
            COALESCE(free_preview, false) AS free_preview
...
async fn load_categories(
    ...
    let sql = format!(
        ...
        "#,
        course_scope_filter = postgres_scope_filter("c"),
        category_scope_filter = postgres_scope_filter("cat"),
    );
    let rows = sqlx::query(sql.as_str())
        .bind(scope.tenant_id)
        .bind(scope.organization_id)
