...
fn postgres_admin_app_writes_appbase_snowflake_ids_explicitly() {
    for expected in [
        ...
        "INSERT INTO platform_app (id, uuid, tenant_id, organization_id",
    ] {
...
fn postgres_admin_app_scopes_every_mutation_to_trusted_tenant_and_organization() {
    for expected in [
        "FROM platform_app WHERE id = $1 AND tenant_id = $2 AND organization_id = $3",
...
fn postgres_admin_app_matches_canonical_media_resource_schema() {
    let platform_app_table = POSTGRES_SCHEMA
        .split("CREATE TABLE IF NOT EXISTS platform_app")
        .nth(1)
        .unwrap()
    ...
    let c_category_table = POSTGRES_SCHEMA
        ...
        .next()
        .unwrap();
    let platform_app_template_table = POSTGRES_SCHEMA
        .split("CREATE TABLE IF NOT EXISTS platform_app_template")
        .nth(1)
        .unwrap()
        .split("CREATE UNIQUE INDEX IF NOT EXISTS uk_platform_app_template_no")
        .next()
        .unwrap();
    assert_sql_contains(platform_app_table, "config JSONB NOT NULL DEFAULT '{}'::jsonb");
    assert_sql_contains(platform_app_table, "status INTEGER NOT NULL DEFAULT 1");
    assert_sql_contains(platform_app_table, "icon_media_resource_id VARCHAR(128)");
    assert_sql_contains(platform_app_table, "icon_object_blob_id BIGINT");
    assert_sql_contains(platform_app_table, "icon_resource_snapshot JSONB");
    assert_sql_contains(platform_app_table, "artifact_media_resource_id VARCHAR(128)");
    assert_sql_contains(platform_app_table, "artifact_object_blob_id BIGINT");
    assert_sql_contains(platform_app_table, "artifact_resource_snapshot JSONB");
    assert_sql_not_contains(platform_app_table, "icon_url");
    assert_sql_not_contains(platform_app_table, "download_url");
    ...
    assert_sql_contains(
        platform_app_template_table,
        "icon_media_resource_id VARCHAR(128)",
    );
    assert_sql_contains(platform_app_template_table, "icon_object_blob_id BIGINT");
    assert_sql_contains(platform_app_template_table, "icon_resource_snapshot JSONB");
    assert_sql_contains(
        platform_app_template_table,
        "cover_media_resource_id VARCHAR(128)",
    );
    assert_sql_contains(platform_app_template_table, "cover_object_blob_id BIGINT");
    assert_sql_contains(platform_app_template_table, "cover_resource_snapshot JSONB");
    assert_sql_not_contains(platform_app_template_table, "icon_url");
    assert_sql_not_contains(platform_app_template_table, "cover_url");
    assert_sql_not_contains(platform_app_table, "market_status");
    assert_sql_not_contains(platform_app_table, "app_key");
    assert_sql_not_contains(POSTGRES_ADMIN_APP_STORE, "platform_app.market_status");
    assert_sql_not_contains(POSTGRES_ADMIN_APP_STORE, "platform_app.app_key");
    assert_sql_contains(POSTGRES_ADMIN_APP_STORE, "DELETE FROM platform_app");
}
