...
#[tokio::test]
async fn openai_compatible_relay_posts_provider_model_and_upstream_secret() {
...
async fn openai_compatible_relay_uses_configured_retryable_statuses_without_default_fallback() {
    ...
    assert_eq!("provider_error", response.body["error"]["code"]);
...
async fn openai_compatible_relay_does_not_retry_non_retryable_upstream_status() {
    ...
    assert_eq!("invalid_api_key", response.body["error"]["code"]);
