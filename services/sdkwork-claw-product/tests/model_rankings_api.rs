...
async fn app_model_rankings_route_reports_service_unavailable_when_read_store_is_not_configured() {
    let router = sdkwork_claw_product::api::app_model_rankings_router();
    let response = router
        .oneshot(
            Request::builder()
                .method("GET")
...
async fn admin_model_rankings_route_requires_trusted_subject_before_unconfigured_read_store() {
    let router = sdkwork_claw_product::api::admin_model_rankings_router();
    let response = router
        .oneshot(
            Request::builder()
                .method("GET")
...
async fn admin_model_ranking_status_route_requires_trusted_subject() {
    let router = sdkwork_claw_product::api::admin_model_rankings_router_with_read_store(Arc::new(
        StubModelRankingsReadStore,
    ));
    let response = router
        .oneshot(
            Request::builder()
