...
#[tokio::test]
async fn sqlite_admin_access_group_store_prefers_resource_group_for_group_backed_resource_code() {
    let pool = schema_sqlite_pool().await;
    seed_access_group_channel_fixture(&pool).await;
    let store = SqliteAdminAccessGroupStore::new(pool.clone());
    let subject = AdminAccessGroupSubject {
...
#[tokio::test]
async fn sqlite_admin_access_group_store_keeps_resource_authorization_normalized_and_cascades_group_delete(
) {
    let pool = schema_sqlite_pool().await;
    seed_access_group_channel_fixture(&pool).await;
    let store = SqliteAdminAccessGroupStore::new(pool.clone());
    ...
    let deleted = store
        .delete_access_group(sdkwork_claw_product::ports::DeleteAdminAccessGroupCommand {
            subject,
            group_id: 10,
...
#[tokio::test]
async fn sqlite_admin_access_group_store_syncs_relationship_status_when_group_status_changes() {
    let pool = schema_sqlite_pool().await;
    seed_access_group_channel_fixture(&pool).await;
    let store = SqliteAdminAccessGroupStore::new(pool.clone());
    let subject = AdminAccessGroupSubject {
    ...
    let disabled = store
        .update_access_group(update_access_group_status_command(
            subject,
            "disabled",
            "2026-05-25 10:07:00",
        ))
        .await
        .unwrap()
        .expect("access group should update");
...
#[tokio::test]
async fn sqlite_admin_access_group_store_keeps_replaced_relationships_disabled_when_group_is_disabled(
) {
    let pool = schema_sqlite_pool().await;
    seed_access_group_channel_fixture(&pool).await;
    let store = SqliteAdminAccessGroupStore::new(pool.clone());
...
#[tokio::test]
async fn sqlite_admin_access_group_store_records_routing_config_version_for_group_and_binding_changes(
) {
    let pool = schema_sqlite_pool().await;
    seed_access_group_channel_fixture(&pool).await;
    let store = SqliteAdminAccessGroupStore::new(pool.clone());
    ...
    assert_eq!(
        vec![
            "replace_access_group_channel_bindings".to_owned(),
            "update_access_group".to_owned()
        ],
        event_actions
    );
}
...
fn update_access_group_status_command(
    subject: AdminAccessGroupSubject,
    status: &str,
    requested_at: &str,
) -> UpdateAdminAccessGroupCommand {
