...
use sdkwork_claw_product::application::{EntityUuidGenerator, InMemoryRuntimeStreamBus};
...
async fn app_runtime_prefers_gateway_chat_completions_over_local_relay_when_both_are_configured() {
    ...
    let relay_requests = Arc::new(Mutex::new(Vec::new()));
    let router =
        sdkwork_claw_product::api::app_runtime_router_with_store_and_gateway_client_chat_stream_relay(
            store.clone(),
            Arc::new(SequentialUuidGenerator::new(vec!["runtime-event-uuid-1"])),
            Arc::new(TestRuntimeCatalog::without_model_provider_route(
                "openai/gpt-5.5",
...
impl sdkwork_claw_product::ports::PricingCatalog for TestRuntimeCatalog {
    fn list_models(
        ...
        let mut model = sdkwork_claw_product::domain::AiModel::new(
            &self.model,
            &self.model,
            &self.vendor_code,
            vec!["chat", "response"],
    ...
    fn list_api_keys(&self) -> Vec<sdkwork_claw_product::domain::GatewayApiKey> {
        if let Some(api_keys) = &self.api_keys {
            return api_keys
                .iter()
                .map(|fixture| {
                    sdkwork_claw_product::domain::GatewayApiKey::new(
                        fixture.id,
                        fixture.group_id,
                        "sk-app",
                        "hash",
        ...
        let api_key = sdkwork_claw_product::domain::GatewayApiKey::new(101, 10, "sk-app", "hash")
            .with_owner(10, 20, 30);
    ...
    fn find_vendor(
        ...
        (vendor_code == self.vendor_code).then(|| {
            sdkwork_claw_product::domain::ModelVendorDefinition::new(
                &self.vendor_code,
                sdkwork_claw_product::domain::ModelVendor::from_code(&self.vendor_code),
                &self.vendor_code,
            )
        })
    }
...
impl AppRuntimeGatewayClient for RecordingGatewayRuntimeClient {
    ...
    ) -> sdkwork_claw_product::ports::AppRuntimeFuture<'a, AppRuntimeGatewayResponse> {
