...
async fn main() -> anyhow::Result<()> {
    ...
    let config = sdkwork_claw_config::RuntimeConfig::from_optional_parts(
        sdkwork_claw_app::SERVICE_NAME,
        "SDKWORK_CLAW_APP_API_BIND",
        "0.0.0.0:18082",
        std::env::var("SDKWORK_CLAW_APP_API_BIND").ok().or_else(|| {
            runtime_toml
                .as_ref()
    ...
    .map_err(anyhow::Error::msg)?;
    sdkwork_claw_app::serve_with_runtime_config(config.bind_addr(), runtime_toml.as_ref()).await
}
