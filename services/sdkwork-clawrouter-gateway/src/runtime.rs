        ...
        DatabaseEngine::Sqlite => {
            ...
            if startup_install_mode.should_ensure() {
                let install_report = DatabaseInstaller::for_sqlite(pool.clone())
                    ...
                    .ensure_installed()
                    .await?;
                log_bootstrap_admin_report("sdkwork-clawrouter-gateway", &install_report);
            }
...
async fn build_embedded_sdkwork_api_gateway_router(
    ...
    sdkwork_api_gateway::build_sdkwork_api_gateway_router_with_embedded_routers(
        config,
        [
            (
...
fn log_gateway_runtime_catalog_snapshot_summary(
    ...
    if phase == "refresh" {
        tracing::debug!(
            service = "sdkwork-clawrouter-gateway",
