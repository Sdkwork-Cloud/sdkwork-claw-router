...
mod tests {
    ...
    #[test]
    fn env_optional_with_legacy_reads_legacy_alias_when_canonical_missing() {
        const CANONICAL: &str = "SDKWORK_CLAW_EDGE_ENV_TEST_CANONICAL_ONLY";
        const LEGACY: &str = "SDKWORK_CLAW_EDGE_ENV_TEST_LEGACY_ONLY";
        unsafe {
            std::env::remove_var(CANONICAL);
    ...
    #[test]
    fn config_u32_or_default_with_legacy_reads_canonical_env_before_config() {
        const CANONICAL: &str = "SDKWORK_CLAW_EDGE_ENV_TEST_RATE";
        const LEGACY: &str = "SDKWORK_CLAW_EDGE_ENV_TEST_RATE_LEGACY";
        unsafe {
            std::env::set_var(CANONICAL, "240");
            std::env::remove_var(LEGACY);
        }
        assert_eq!(
            config_u32_or_default_with_legacy(CANONICAL, LEGACY, Some(60), 120).unwrap(),
            240,
        );
        unsafe {
            std::env::remove_var(CANONICAL);
    ...
    #[test]
    fn config_bool_or_default_with_legacy_accepts_truthy_env_values() {
        const CANONICAL: &str = "SDKWORK_CLAW_EDGE_ENV_TEST_HSTS";
        const LEGACY: &str = "SDKWORK_CLAW_EDGE_ENV_TEST_HSTS_LEGACY";
        unsafe {
            std::env::set_var(CANONICAL, "yes");
            std::env::remove_var(LEGACY);
        }
        assert!(config_bool_or_default_with_legacy(
            CANONICAL,
            LEGACY,
            Some(false),
            false,
