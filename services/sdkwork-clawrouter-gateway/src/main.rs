...
async fn main() -> anyhow::Result<()> {
    ...
    if edge_server_enabled(runtime_toml.as_ref()) || all_in_one_runtime_enabled() {
        let config = runtime_config_from_env_or_toml(
            sdkwork_clawrouter_gateway::SERVICE_NAME,
        ...
        if all_in_one_runtime_enabled() {
            return sdkwork_clawrouter_gateway::serve_all_in_one_edge_server_with_runtime_config(
                config.bind_addr(),
                edge_config,
                runtime_toml.as_ref(),
            )
        ...
        return sdkwork_clawrouter_gateway::serve_edge_server_with_runtime_config(
            config.bind_addr(),
            edge_config,
            runtime_toml.as_ref(),
        )
    ...
    .map_err(anyhow::Error::msg)?;
    sdkwork_clawrouter_gateway::serve_with_runtime_config(config.bind_addr(), runtime_toml.as_ref())
        .await
}

pub fn build_edge_server_config(
    runtime_toml: Option<&sdkwork_claw_config::RuntimeTomlConfig>,
) -> anyhow::Result<sdkwork_clawrouter_gateway::EdgeServerConfig> {
    let mut edge_config = sdkwork_clawrouter_gateway::EdgeServerConfig::try_new(
