...
) -> Result<Router, sdkwork_clawrouter_gateway::GatewayRouterError> {
    sdkwork_clawrouter_gateway::runtime::router_with_database_api_key_provider_configs_usage_settlement_worker_config_and_startup_install_mode(
...
async fn seeded_gateway_router_with_provider_adapter_config(
    ...
    sdkwork_clawrouter_gateway::runtime::router_with_database_api_key_provider_configs_adapter_config_and_startup_install_mode(
        catalog.database_config().unwrap(),
        Some(catalog.api_key_security_config().unwrap()),
        None,
        provider_secret_map_config,
...
fn openai_compatible_passthrough_path_manifest_stays_complete() {
    let paths = sdkwork_clawrouter_gateway::openai_compatible_passthrough_paths();
    let method_paths = sdkwork_clawrouter_gateway::openai_method_passthrough_paths();
...
fn gateway_openapi_v1_paths_are_product_routes_or_openai_passthrough_routes() {
    ...
    let openapi_paths = spec["paths"]
        ...
        .map(String::as_str)
        .collect::<BTreeSet<_>>();
    let passthrough_paths = sdkwork_clawrouter_gateway::openai_compatible_passthrough_paths()
        .iter()
        .chain(sdkwork_clawrouter_gateway::openai_method_passthrough_paths().iter())
        .chain(sdkwork_clawrouter_gateway::stored_chat_completion_passthrough_paths().iter())
        .copied()
        .collect::<BTreeSet<_>>();
...
fn gateway_openapi_vendor_paths_are_runtime_provider_routes_without_public_manifest() {
    ...
    let runtime_providers = sdkwork_clawrouter_gateway::provider_native_passthrough_providers()
        .iter()
        .copied()
        .collect::<BTreeSet<_>>();
...
async fn gateway_mounts_provider_native_passthrough_boundaries_without_404() {
    let router = sdkwork_clawrouter_gateway::router();
...
async fn gateway_provider_native_passthrough_keeps_official_standard_provider_direct_when_only_non_standard_adapter_route_exists(
    ...
    .unwrap();
    let router = sdkwork_clawrouter_gateway::router_with_provider_passthrough_and_adapter_config(
        passthrough_config,
        Some(adapter_config),
    );
...
async fn gateway_forwards_configured_vendor_prefixed_vidu_passthrough_request() {
    ...
    .unwrap();
    let router = sdkwork_clawrouter_gateway::router_with_provider_passthrough_config(config);
