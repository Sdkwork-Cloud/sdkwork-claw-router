...
async fn edge_server_exposes_own_health_check_without_upstream_probe() {
    let router = sdkwork_clawrouter_gateway::edge_server_router(
...
async fn edge_server_can_dispatch_to_in_process_upstreams() {
    ...
    let router = sdkwork_clawrouter_gateway::edge_server_router_with_in_process_upstreams(
        sdkwork_clawrouter_gateway::EdgeServerConfig::try_new(
            "http://127.0.0.1:1",
            "http://127.0.0.1:1",
            "http://127.0.0.1:1",
...
async fn edge_server_can_serve_portal_dist_without_node_server() {
    ...
    let config = sdkwork_clawrouter_gateway::EdgeServerConfig::try_new(
        &gateway.base_url,
        &admin.base_url,
        &app.base_url,
        "http://127.0.0.1:3901",
    ...
    let router = sdkwork_clawrouter_gateway::edge_server_router(config);
...
fn edge_server_config_rejects_non_origin_forward_targets() {
    assert!(sdkwork_clawrouter_gateway::EdgeServerConfig::try_new(
