...
async fn edge_server_proxies_real_sqlite_gateway_admin_and_app_services() {
    ...
    let edge_router = sdkwork_clawrouter_gateway::edge_server_router(
        sdkwork_clawrouter_gateway::EdgeServerConfig::try_new(
            &gateway.base_url,
            &admin.base_url,
            &app.base_url,
            &portal.base_url,
...
async fn all_in_one_edge_router_serves_sqlite_gateway_admin_and_app_without_service_ports() {
    ...
    let edge_router = sdkwork_clawrouter_gateway::edge_server_router_with_in_process_upstreams(
        sdkwork_clawrouter_gateway::EdgeServerConfig::try_new(
            "http://127.0.0.1:1",
            "http://127.0.0.1:1",
            "http://127.0.0.1:1",
        ...
        .unwrap(),
        sdkwork_clawrouter_gateway::EdgeInProcessUpstreams::new(
            gateway_router,
            admin_router,
            app_router,
        ),
...
async fn edge_server_proxies_app_router_console_routing_api_through_generated_sdk_paths() {
    ...
    let app_router = sdkwork_claw_http::service_router_with_contract_routes(
        "sdkwork-clawrouter-app-api-server-routing-smoke",
