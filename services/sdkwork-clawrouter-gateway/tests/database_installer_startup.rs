...
use sdkwork_clawrouter_gateway::runtime::router_with_database_and_api_key_config;
...
async fn gateway_env_startup_can_skip_installer_when_workspace_already_ensured_database() {
    ...
    let _router = sdkwork_clawrouter_gateway::runtime::router_from_env()
        .await
        .unwrap();
...
async fn gateway_env_startup_rejects_zero_config_server_placeholder_postgres() {
    ...
    let router_result = sdkwork_clawrouter_gateway::runtime::router_from_env().await;
...
fn unique_sqlite_url() -> String {
    ...
    path.push(format!(
        "sdkwork-clawrouter-gateway-startup-{millis}-{counter}.sqlite"
    ));
...
fn unique_runtime_config_path() -> std::path::PathBuf {
    ...
    let mut path = std::env::temp_dir();
    path.push(format!(
        "sdkwork-clawrouter-gateway-runtime-{millis}-{counter}"
    ));
