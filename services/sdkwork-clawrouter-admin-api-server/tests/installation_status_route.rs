...
async fn fresh_sqlite_install_refreshes_model_rankings_from_usage_and_serves_admin_and_app_reads() {
    ...
    let admin_router =
        sdkwork_clawrouter_admin_api_server::router_with_database_and_api_key_config(
            database_config,
            Some(api_key_config),
            Some(trusted_subject_config().unwrap()),
            Some(app_session_config().unwrap()),
...
fn unique_sqlite_url() -> String {
    ...
    path.push(format!(
        "sdkwork-clawrouter-admin-api-server-installation-status-{millis}-{counter}.sqlite"
    ));
