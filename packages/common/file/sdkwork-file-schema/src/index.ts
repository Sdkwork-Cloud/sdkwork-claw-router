...
export const SDKWORK_FILE_SCHEMA_TABLES: readonly FileSchemaTable[] = [
  ...
  table(SDKWORK_FILE_TABLES.uploadSession, [
    tenantColumn,
    nullableText("organization_id"),
    text("owner_user_id"),
    nullableText("app_id"),
    nullableText("space_id"),
    nullableText("parent_node_id"),
    nullableText("slot_code"),
    nullableText("target_type"),
    nullableText("target_id"),
    text("filename_original"),
    text("filename_normalized"),
    text("content_type_claimed"),
    nullableText("content_type_detected"),
    bigint("size_bytes_claimed"),
    nullableBigint("size_bytes_actual"),
    nullableText("checksum_algorithm_claimed"),
    nullableText("checksum_value_claimed"),
    text("upload_mode"),
    bigint("provider_id"),
    bigint("bucket_id"),
    text("object_key"),
    nullableText("multipart_upload_id"),
    nullableBigint("part_size_bytes"),
    nullableInteger("total_parts"),
    integer("uploaded_parts", "0"),
    bigint("uploaded_bytes", "0"),
    bigint("resumable_offset", "0"),
    text("status", "'created'"),
    nullableText("client_fingerprint"),
    text("idempotency_key"),
    timestamp("expires_at"),
    nullableTimestamp("completed_at"),
    nullableTimestamp("aborted_at"),
  ], {
    checks: [
      enumCheck("upload_mode", SDKWORK_FILE_UPLOAD_MODES, "ck_upload_session_upload_mode"),
      enumCheck("status", SDKWORK_FILE_UPLOAD_STATUSES, "ck_upload_session_status"),
      "size_bytes_claimed > 0",
      "uploaded_parts >= 0",
      "uploaded_bytes >= 0",
      "resumable_offset >= 0",
    ],
    indexes: [
      idx("idx_upload_session_cleanup", ["tenant_id", "status", "expires_at", "id"]),
      idx("idx_upload_session_storage_location", ["tenant_id", "provider_id", "bucket_id", "status", "expires_at", "id"]),
      idx("idx_upload_session_bucket", ["tenant_id", "bucket_id", "status", "expires_at", "id"]),
      idx("idx_upload_session_target_slot", ["tenant_id", "target_type", "target_id", "slot_code"]),
      idx("uq_upload_session_tenant_idempotency", ["tenant_id", "idempotency_key"], undefined, true),
    ],
    foreignKeys: [
      fk("fk_upload_session_provider_tenant", ["tenant_id", "provider_id"], "object_provider(tenant_id, id)"),
      fk("fk_upload_session_bucket_tenant", ["tenant_id", "bucket_id"], "object_bucket(tenant_id, id)"),
    ],
    uniques: [{ columns: ["tenant_id", "id"], name: "uq_upload_session_tenant_id" }],
  }),

  table(SDKWORK_FILE_TABLES.uploadPresignGrant, [
    tenantColumn,
    bigint("session_id"),
    text("grant_type"),
    text("http_method"),
    nullableInteger("part_number"),
    timestamp("url_expires_at"),
    jsonb("allowed_headers_json"),
    jsonb("required_headers_json"),
    bigint("content_length_min", "0"),
    nullableBigint("content_length_max"),
    timestamp("issued_at", "now()"),
    text("issued_by"),
  ], {
    checks: ["content_length_min >= 0", "(part_number IS NULL OR (part_number >= 1 AND part_number <= 10000))"],
    foreignKeys: [fk("fk_upload_presign_grant_session_tenant", ["tenant_id", "session_id"], "upload_session(tenant_id, id)")],
    indexes: [
      idx("idx_upload_presign_grant_tenant_session_part", ["tenant_id", "session_id", "part_number", "issued_at"]),
    ],
  }),

  table(SDKWORK_FILE_TABLES.uploadPart, [
    tenantColumn,
    bigint("session_id"),
    integer("part_number"),
    bigint("size_bytes_claimed"),
    nullableBigint("size_bytes_actual"),
    nullableText("etag"),
    nullableText("checksum_algorithm"),
    nullableText("checksum_value"),
    nullableTimestamp("uploaded_at"),
    nullableTimestamp("verified_at"),
    text("status", "'created'"),
  ], {
    checks: [
      { expression: "part_number >= 1 AND part_number <= 10000", name: "ck_upload_part_number" },
      "size_bytes_claimed >= 0",
      "(size_bytes_actual IS NULL OR size_bytes_actual >= 0)",
    ],
    foreignKeys: [fk("fk_upload_part_session_tenant", ["tenant_id", "session_id"], "upload_session(tenant_id, id)")],
    indexes: [idx("idx_upload_part_tenant_session_part", ["tenant_id", "session_id", "part_number"])],
    uniques: [{ columns: ["tenant_id", "session_id", "part_number"], name: "uq_upload_part_session_part" }],
  }),

  table(SDKWORK_FILE_TABLES.uploadCompletionAttempt, [
    tenantColumn,
    bigint("session_id"),
    text("idempotency_key"),
    text("status"),
    nullableText("error_code"),
    nullableText("error_message"),
    timestamp("attempted_at", "now()"),
  ], {
    foreignKeys: [fk("fk_upload_completion_attempt_session_tenant", ["tenant_id", "session_id"], "upload_session(tenant_id, id)")],
    indexes: [idx("idx_upload_completion_attempt_tenant_session", ["tenant_id", "session_id", "attempted_at"])],
    uniques: [{ columns: ["tenant_id", "session_id", "idempotency_key"], name: "uq_upload_completion_attempt_idempotency" }],
  }),
  ...
  table(SDKWORK_FILE_TABLES.storageQuotaReservation, [
    ...
    foreignKeys: [
      fk("fk_storage_quota_reservation_upload_session_tenant", ["tenant_id", "upload_session_id"], "upload_session(tenant_id, id)"),
    ],
    indexes: [
      idx("idx_storage_quota_reservation_scope", ["tenant_id", "organization_id", "user_id", "space_id", "status", "expires_at"]),
      idx("idx_storage_quota_reservation_upload", ["tenant_id", "upload_session_id", "status"]),
...
const REQUIRED_SCHEMA_CHECKS = [
  ...
  { checkName: "ck_upload_session_upload_mode", tableName: SDKWORK_FILE_TABLES.uploadSession },
  { checkName: "ck_upload_session_status", tableName: SDKWORK_FILE_TABLES.uploadSession },
...
const REQUIRED_SCHEMA_INDEXES = [
  ...
  requiredIndex(SDKWORK_FILE_TABLES.uploadSession, "idx_upload_session_cleanup", ["tenant_id", "status", "expires_at", "id"]),
  requiredIndex(SDKWORK_FILE_TABLES.uploadSession, "idx_upload_session_storage_location", ["tenant_id", "provider_id", "bucket_id", "status", "expires_at", "id"]),
  requiredIndex(SDKWORK_FILE_TABLES.uploadSession, "idx_upload_session_bucket", ["tenant_id", "bucket_id", "status", "expires_at", "id"]),
  requiredIndex(SDKWORK_FILE_TABLES.uploadSession, "idx_upload_session_target_slot", ["tenant_id", "target_type", "target_id", "slot_code"]),
  requiredIndex(SDKWORK_FILE_TABLES.uploadSession, "uq_upload_session_tenant_idempotency", ["tenant_id", "idempotency_key"], { unique: true }),
  requiredIndex(SDKWORK_FILE_TABLES.uploadPresignGrant, "idx_upload_presign_grant_tenant_session_part", ["tenant_id", "session_id", "part_number", "issued_at"]),
  requiredIndex(SDKWORK_FILE_TABLES.uploadPart, "idx_upload_part_tenant_session_part", ["tenant_id", "session_id", "part_number"]),
  requiredIndex(SDKWORK_FILE_TABLES.uploadCompletionAttempt, "idx_upload_completion_attempt_tenant_session", ["tenant_id", "session_id", "attempted_at"]),
  ...
  requiredIndex(SDKWORK_FILE_TABLES.storageQuotaReservation, "idx_storage_quota_reservation_upload", ["tenant_id", "upload_session_id", "status"]),
...
const REQUIRED_TENANT_PARENT_UNIQUES = [
  ...
  requiredUnique(SDKWORK_FILE_TABLES.uploadSession, "uq_upload_session_tenant_id", ["tenant_id", "id"]),
...
const REQUIRED_TENANT_CONSISTENT_FOREIGN_KEYS = [
  ...
  requiredForeignKey(SDKWORK_FILE_TABLES.uploadSession, "fk_upload_session_provider_tenant", ["tenant_id", "provider_id"], "object_provider(tenant_id, id)"),
  requiredForeignKey(SDKWORK_FILE_TABLES.uploadSession, "fk_upload_session_bucket_tenant", ["tenant_id", "bucket_id"], "object_bucket(tenant_id, id)"),
  requiredForeignKey(SDKWORK_FILE_TABLES.uploadPresignGrant, "fk_upload_presign_grant_session_tenant", ["tenant_id", "session_id"], "upload_session(tenant_id, id)"),
  requiredForeignKey(SDKWORK_FILE_TABLES.uploadPart, "fk_upload_part_session_tenant", ["tenant_id", "session_id"], "upload_session(tenant_id, id)"),
  requiredForeignKey(SDKWORK_FILE_TABLES.uploadCompletionAttempt, "fk_upload_completion_attempt_session_tenant", ["tenant_id", "session_id"], "upload_session(tenant_id, id)"),
  ...
  requiredForeignKey(SDKWORK_FILE_TABLES.storageQuotaReservation, "fk_storage_quota_reservation_upload_session_tenant", ["tenant_id", "upload_session_id"], "upload_session(tenant_id, id)"),
...
function nullableInteger(name: string): FileSchemaColumn {
  return { name, nullable: true, type: "integer" };
}
