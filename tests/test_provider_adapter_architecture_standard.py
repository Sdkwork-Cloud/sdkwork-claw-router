...
def test_gateway_does_not_depend_on_concrete_provider_adapter_packages():
    gateway_cargo = (ROOT / "services/sdkwork-clawrouter-gateway/Cargo.toml").read_text(
        encoding="utf-8"
    )
...
def test_provider_native_passthrough_uses_explicit_registry_hit_before_adapter():
    passthrough = (ROOT / "services/sdkwork-clawrouter-gateway/src/passthrough.rs").read_text(
        encoding="utf-8"
    )
    matcher = (
